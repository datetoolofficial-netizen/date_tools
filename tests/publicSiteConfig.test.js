...
describe('public settings projection', () => {
    ...
    it('publishes only the safe PWA update announcement fields', () => {
        ...
        expect(projected.pwaUpdatePrompt).toEqual({ enabled: true, version: '0.3.39' });
        ...
        expect(normalizePwaUpdatePrompt({ enabled: true, version: 'stale-version' }).version).toBe('0.3.39');
    });
});
