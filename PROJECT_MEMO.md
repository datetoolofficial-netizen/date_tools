...
### تجهيز نشر 0.3.61 وفحص الأمان وGoogle - جارٍ
...
- كشف صفحة Code Scanning تنبيهين مرتفعين جديدين في `scripts/post-deploy-health.mjs` بسبب تعبيرين منتظمين عامين لفحص `robots.txt` و`sitemap.xml`؛ استُبدلا بمطابقة نصية محددة لا تغير نطاق الفحص.
- نجاح ESLint وفحص الأسرار و209 اختبارات وحدة، ونجاح `npm audit --omit=dev --audit-level=high` بلا ثغرات معروفة.
...
1. **تنبيهان مرتفعان من CodeQL في فحص ما بعد النشر**
   - الأعراض: `Missing regular expression anchor` في السطرين الخاصين بـ`robots.txt` و`sitemap.xml`.
   - السبب: استخدام تعبيرين منتظمين للبحث داخل استجابة نصية كاملة رغم أن المطلوب مطابقة سلاسل ثابتة.
   - الحل: استخدام `String(...).toLowerCase().includes(...)` بقيم ثابتة محددة، مع نجاح الاختبار المخصص وجميع اختبارات الوحدة.
   - الحالة: محلول محليًا؛ ينتظر الدفع وإعادة فحص CodeQL حتى يغلق التنبيهان.
...
- الإنتاج ما زال يعرض `0.3.60`؛ نشر `0.3.61` ينتظر إكمال تفويض Wrangler المحدود في Cloudflare.
- لم يُرسل طلب مراجعة جديد إلى AdSense، ولم يبدأ تحقق إصلاح جديد في Search Console.

المتبقي المحفوظ:

- إكمال تفويض Cloudflare بصلاحيات `account:read` و`user:read` و`workers:write` و`zone:read` فقط، ثم نشر `0.3.61` وتشغيل فحص الصحة.
- دفع إصلاح CodeQL والتوثيق، ثم التحقق من إغلاق التنبيهين ونجاح Quality Gate.
