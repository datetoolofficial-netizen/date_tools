...
* توجيه التصميم: طلب المستخدم إصلاح الأخطاء البرمجية عند إعادة البناء، وليس إعادة تصميم الواجهات. لا تُغيّر الألوان أو التدرجات أو الأحجام أو الهيكل البصري دون طلب صريح؛ مرجع ألوان الصفحات العامة المعتمد هو ما قبل الإصدار `0.3.60`.
...
الإصدار المنشور للتطبيق هو 0.3.66.
نسخة منصة الإدارة المنشورة هي 0.1.54.
نسخة بوابة المعلنين المنشورة هي 1.0.7.
يوجد سجل إصدارات رسمي في VERSION_LOG.md.
إصلاح رابط توثيق بريد Firebase منشور، وأُرسلت رسالة توثيق جديدة من صفحة أمان الإدارة بعد التصحيح.
```
...
### إصلاح عرض التاريخ الهجري ونتيجة فرق التاريخ - الإصدار 0.3.45

تم إنجازه:

- إضافة `hijriSuffix` مترجمة بقيمة `هـ` للعربية و`AH` للإنجليزية بدل إلحاق قيمة غير معرّفة بنتيجة التحويل.
- استخدام لاحقة التقويم المترجمة في التحويل والعمر الهجري ونصوص مشاركة فرق التاريخ.
- إخفاء قسم "معلومة إضافية" من نتيجة فرق التاريخ الميلادي والهجري مع إبقاء زر المشاركة.
- إبقاء التاريخ الأول والثاني داخل قالب نص المشاركة دون عرضهما مرة ثانية أسفل النتيجة.

الأخطاء المكتشفة:

1. **ظهور undefined بعد السنة الهجرية**
   - الأعراض: ظهرت نتيجة مثل `18 ربيع الأول 1448undefined`.
   - السبب: استخدام `i18n[lang].hijriSuffix` دون تعريف المفتاح في العربية أو الإنجليزية.
   - الحل: تعريف المفتاح للغتين واستخدامه بدل اللاحقة العربية الثابتة في جميع المواضع ذات الصلة.
   - الحالة: منجز ومختبر.

2. **تكرار التاريخين في بطاقة معلومات نتيجة فرق التاريخ**
   - الأعراض: ظهرت بطاقة "معلومة إضافية" أسفل النتيجة تعرض التاريخ الأول والثاني رغم وجودهما في نص المشاركة.
   - السبب: استخدام نموذج بطاقة قصة التاريخ نفسه لفرق التاريخ وللنتائج التي تحتاج شرحًا إضافيًا.
   - الحل: السماح لبطاقة النتيجة بإخفاء المحتوى الوصفي الفارغ مع إبقاء إجراء المشاركة، وتمرير وصف فارغ لنتيجتي الفرق.
   - الحالة: منجز ومختبر.
...
- `package.json`
- `package-lock.json`
- `tests/datePresentation.test.js`
- `VERSION_LOG.md`
- `PROJECT_MEMO.md`
...
- نجحت 67 حالة اختبار في 19 ملفًا، ونجح ESLint وبناء Next.js الكامل وتوليد 31 صفحة.
- أثبت اختبار المتصفح المحلي ظهور `18 ربيع الأول 1448 هـ` دون `undefined`.
- أثبت اختبار فرق التاريخ اختفاء المحتوى الإضافي وبقاء زر المشاركة، وأثبت الاختبار الآلي استمرار وجود التاريخين في نص المشاركة.
- نُشر الإصدار `0.3.45` عبر Cloudflare Build المرتبط بفرع `master` بعد نجاح البناء للالتزام `570566a`.
- أعادت واجهة الإصدار الحية `version: 0.3.45` بتاريخ `2026-08-31`، وبقي الفحص التفاعلي المحلي للنتيجتين ناجحًا دون `undefined` أو معلومات مكررة.
- تعذر النشر المباشر من الطرفية لغياب `CLOUDFLARE_API_TOKEN` في جلسة Wrangler غير التفاعلية، لذلك استُخدم رمز البناء المحدود المرتبط بـCloudflare بنجاح دون كشف أو إضافة أي سر للمستودع.

### توحيد صفحات الأدوات الفرعية - الإصدار 0.3.46

تم إنجازه:

- إلغاء عرض الصفحات المستقلة الوهمية لمسارات الأدوات الفرعية التسعة، لأن كل مسار كان يعيد بناء صفحة العائلة كاملة ثم ينزل إلى قسم واحد بالجافا.
- إبقاء الروابط العامة الأصلية مثل `/age-calculator` و`/time-converter` و`/current-weather` محفوظة في الإدارة وحقول SEO.
- إضافة تحويل دائم `308` من كل رابط فرعي إلى صفحة العائلة والقسم الداخلي المطابق، مثل `/age-calculator` إلى `/#age-calculator`.
- إزالة المسارات الفرعية التسعة من sitemap والإبقاء على صفحات الأدوات الرئيسية `/` و`/clock` و`/weather`.
- حذف قسم «أدوات مرتبطة» من الصفحات الرئيسية وحذف النصوص والأنماط التي كانت مخصصة للصفحات الفرعية المنفردة.
- إبقاء `subtoolSeo` العربي والإنجليزي وصور المشاركة وأسماء الأدوات وحقول المعاينة دون حذف أو فقدان بيانات.

الأخطاء المكتشفة:

1. **مسارات فرعية تعرض الصفحة الرئيسية نفسها**
   - الأعراض: فتح `/age-calculator` أو أي مسار فرعي يعيد تحميل جميع أدوات العائلة ثم ينزل إلى الأداة المطلوبة.
   - السبب: مسار `[slug]` كان يعرض عميل الصفحة الرئيسية كاملًا مع `initialSectionId` ويمنح النسخة المكررة canonical وSchema مستقلين.
   - الحل: إزالة العرض المستقل واستخدام تحويلات `308` إلى الأقسام الداخلية في الصفحات الرئيسية.
   - الحالة: منجز ومغطى بالاختبارات.

2. **تكرار تسع صفحات متقاربة في sitemap**
   - الأعراض: كانت صفحات الأدوات الفرعية تُرسل إلى Google كوثائق مستقلة رغم تطابق واجهتها التفاعلية مع صفحة العائلة.
   - السبب: توليد إدخال sitemap لكل عنصر في `TOOL_SECTION_ROUTE_ENTRIES`.
   - الحل: توليد صفحات العائلات الثلاث فقط والاحتفاظ بالمسارات القديمة كتحويلات دائمة خارج sitemap.
   - الحالة: منجز ومغطى بالاختبارات.
...
- `package.json`
- `package-lock.json`
- `tests/toolRouteConsolidation.test.js`
- `tests/publicSiteConfig.test.js`
- `tests/pwaVersionCheck.test.js`
- `VERSION_LOG.md`
- `PROJECT_MEMO.md`
...
- نجحت 71 حالة اختبار في 20 ملفًا، ونجح ESLint وبناء Next.js الكامل وتوليد 31 صفحة.
- أثبت فحص الإنتاج المحلي أن `/age-calculator?source=admin` يعيد `308` إلى `/?source=admin#age-calculator` مع حفظ معاملات الرابط.
...
### صفحات SEO مستقلة للأدوات الفرعية - الإصدار 0.3.47

تم إنجازه:

- إعادة المسارات الفرعية التسعة كصفحات فعلية تعيد `200` بدل تحويلها إلى أقسام داخل صفحات العائلات.
- إبقاء حقول `subtoolSeo` العربية والإنجليزية في الإدارة واستخدام عنوان البحث والوصف و`H1` وCanonical الخاص بكل أداة.
- إضافة وضع `standaloneSectionId` إلى صفحات التاريخ والوقت والطقس لإعادة استخدام منطق الأدوات نفسه مع عرض الأداة المطلوبة فقط.
- جعل صفحات الطقس المستقلة تعرض البحث المشترك ثم النتيجة المتخصصة في حالة الطقس أو نصيحة الخروج أو التوقع، بينما تعرض صفحة البحث مجموعة النتائج الكاملة.
- إضافة محتوى إرشادي فريد لكل أداة بالعربية والإنجليزية، وإخراجه في HTML الأولي قبل اكتمال تحميل إعدادات الواجهة.
- إعادة المسارات التسعة إلى sitemap مع تاريخ تعديل ودرجة أولوية مناسبة.
- إعادة Metadata وSchema من نوع `WebApplication` و`BreadcrumbList` لكل صفحة مستقلة.
- إصلاح `SiteShell` حتى لا يستبدل عنوان الصفحة الفرعية بعنوان صفحة العائلة بعد تحميل الإعدادات أو تغيير اللغة.
- إبقاء قسم «أدوات مرتبطة» محذوفًا حسب طلب المستخدم وعدم إعادته مع الصفحات المستقلة.

الأخطاء المكتشفة:

1. **التحويل الدائم يمنع الأدوات الفرعية من المنافسة منفردة**
   - الأعراض: كانت `/age-calculator` وبقية المسارات تتحول إلى صفحة العائلة، لذلك يجمع Google إشاراتها مع الصفحة النهائية بدل فهرستها بعنوان مستقل.
   - السبب: اعتماد تحويلات `308` بعد تفسير سابق بأن المطلوب إلغاء الصفحات الفرعية كليًا.
   - الحل: إنشاء صفحات مستقلة حقيقية تعرض أداة واحدة ومحتوى فريدًا مع Canonical وMetadata مستقلين.
   - الحالة: منجز ومختبر محليًا.

2. **استبدال عنوان الأداة بعد تحميل إعدادات الموقع**
   - الأعراض: ظهر عنوان حاسبة العمر الصحيح في HTML ثم استبدله `SiteShell` بعنوان صفحة التاريخ العامة داخل المتصفح.
   - السبب: اختيار إعدادات SEO من بادئة المسار فقط وعدم التعرف على المسارات الفرعية التسعة.
   - الحل: مطابقة `pathname` مع `TOOL_SECTION_ROUTE_ENTRIES` واستخدام سجل `subtoolSeo` الخاص بالأداة.
   - الحالة: منجز ومختبر تفاعليًا.
...
- `package.json`
- `package-lock.json`
- `tests/toolStandaloneRoutes.test.js`
- `tests/publicSiteConfig.test.js`
- `tests/pwaVersionCheck.test.js`
- `VERSION_LOG.md`
- `PROJECT_MEMO.md`
...
- نجحت 72 حالة اختبار في 20 ملفًا، ونجح ESLint وبناء Next.js الكامل وتوليد 31 صفحة.
- أعادت المسارات التسعة محليًا حالة `200` ولكل مسار عنوان بحث وCanonical مطابقان لحقوله في الإدارة.
- أثبت الفحص التفاعلي أن صفحات حاسبة العمر وتحويل الوقت تعرض أداة واحدة فقط مع ثبات العنوان الصحيح بعد اكتمال التحميل.
- ظهر المحتوى الفريد وSchema الخاص بحاسبة العمر في HTML الأولي، ولم يظهر معرّف أداة تحويل التاريخ داخل صفحتها.
- أصبحت sitemap المحلية تضم 17 عنوانًا، منها صفحات العائلات الثلاث والمسارات الفرعية التسعة.
- نُشر الإصدار `0.3.47` عبر بناء Cloudflare المرتبط بفرع `master` للالتزام `97bba50`، وأعادت واجهة الإصدار الحية `version: 0.3.47`.
- أعادت المسارات الفرعية التسعة حيًا حالة `200` دون تحويل، مع عنوان بحث وCanonical مستقلين ومطابقين لكل رابط.
- أصبحت sitemap الحية تضم 17 عنوانًا، وتأكد وجود المسارات الفرعية التسعة كاملة إلى جانب صفحات العائلات الرئيسية.
- تعرض الإنتاج عناوين مستقلة مثل «حاسبة العمر بالهجري والميلادي - احسب عمرك بدقة» و«تحويل التاريخ من هجري إلى ميلادي والعكس» و«حالة الطقس الآن ودرجة الحرارة المحسوسة».

### الأدوات المشابهة في الصفحات الفردية - الإصدار 0.3.48

تم إنجازه:

- إضافة قسم «أدوات مشابهة» في الصفحات الفردية التسع فقط، بعد المحتوى الإرشادي الخاص بكل أداة.
- عرض الأدوات الأخرى من العائلة نفسها واستبعاد الأداة المفتوحة؛ صفحتا الوقت تعرضان رابطًا واحدًا، وصفحات التاريخ رابطين، وصفحات الطقس ثلاثة روابط.
- جعل كل بطاقة رابطًا مباشرًا إلى الصفحة المستقلة للأداة الأخرى، مثل `/age-calculator` إلى `/date-converter` و`/date-difference`، دون تحويل أو Hash.
- استخدام أسماء الأدوات وأوصاف `subtoolSeo` المحفوظة في الإدارة بحسب لغة الموقع، مع نصوص واجهة عربية وإنجليزية مستقلة.
- إضافة تصميم متجاوب مميز بألوان عائلات التاريخ والوقت والطقس، دون تجاوز عرض الشاشة أو إضافة قسم حاوي متداخل.
- إبقاء صفحات العائلات الرئيسية `/` و`/clock` و`/weather` بلا قسم الأدوات المشابهة حتى يظل القسم إعلانًا سياقيًا خاصًا بالصفحات الفردية.

الأخطاء المكتشفة:

1. **غياب مسار داخلي واضح بين صفحات الأدوات المستقلة**
   - الأعراض: كانت كل صفحة فردية تنتهي دون اقتراح أدوات أخرى من العائلة نفسها بعد حذف قسم الروابط القديم.
   - السبب: لم يكن هناك قسم مخصص للصفحات المستقلة بعد إعادة تفعيلها في الإصدار `0.3.47`.
   - الحل: إنشاء قسم روابط سياقية مباشر يعتمد على سجل المسارات وإعدادات SEO المترجمة ويستبعد الصفحة الحالية تلقائيًا.
   - الحالة: منجز ومختبر محليًا.
...
```powershell
npm test -- --run tests/toolStandaloneRoutes.test.js tests/publicSiteConfig.test.js tests/pwaVersionCheck.test.js
npm test -- --run
npm run lint
npm run build
...
- نجحت 73 حالة اختبار في 20 ملفًا، ونجح ESLint وبناء Next.js الكامل وتوليد 31 صفحة.
- أثبت الفحص التفاعلي المحلي أن صفحة حاسبة العمر تعرض رابطَي تحويل التاريخ وحساب المدة فقط، وأن النصوص والأوصاف تتحول إلى الإنجليزية بصورة صحيحة.
- أثبت فحص الجوال بعرض 390 بكسل عدم وجود تجاوز أفقي، وعرض صفحة الوقت رابطًا واحدًا وصفحة الطقس ثلاثة روابط دون تكرار فعلي في شجرة الصفحة.
- نُشر الإصدار `0.3.48` عبر بناء Cloudflare المرتبط بفرع `master` للالتزام `05ce722`، وأعادت واجهة الإصدار الحية `version: 0.3.48` بتاريخ `2026-09-01`.
- أثبت الفحص الحي أن `/age-calculator` تعرض `/date-converter` و`/date-difference`، وأن `/time-converter` تعرض `/timezone-difference`، وأن `/current-weather` تعرض أدوات الطقس الثلاث الأخرى.
- أثبت الفحص الحي وجود نسخة واحدة فقط من القسم في كل صفحة فردية وعدم وجوده في صفحات العائلات `/` و`/clock` و`/weather`، مع عدم وجود تجاوز أفقي في صفحة حاسبة العمر.

### مسارات الأدوات الإنجليزية المستقلة - الإصدار 0.3.49

تم إنجازه:

- إضافة صفحات إنجليزية مستقلة لعائلات الأدوات: `/en` و`/en/clock` و`/en/weather`.
- إضافة الصفحات الفردية الإنجليزية التسع تحت `/en`، ومنها `/en/age-calculator` و`/en/time-converter` و`/en/current-weather`.
- إعادة استخدام مكونات ومنطق الأدوات نفسه دون نسخ حسابات التاريخ أو الوقت أو استدعاءات الطقس.
- جعل المسار يحدد اللغة؛ روابط `/en` تبدأ بالإنجليزية و`LTR` حتى لو كانت لغة محفوظة سابقة مختلفة.
- تحويل زر اللغة إلى تنقل بين رابطين متناظرين، مثل `/age-calculator` و`/en/age-calculator`، مع حفظ الاستعلام وHash عند وجودهما.
- تحويل روابط الهوية والتنقل الرئيسية والأدوات المشابهة إلى `/en` عند استخدام النسخة الإنجليزية.
- إنشاء Canonical مستقل لكل نسخة وإضافة `hreflang` للقيم `ar-SA` و`en-US` و`x-default` في النسختين.
- توليد Metadata وOpen Graph وSchema `WebApplication` وBreadcrumbs باللغة الإنجليزية ومن حقول الإدارة الإنجليزية.
- إضافة روابط الأدوات الإنجليزية الاثني عشر إلى sitemap مع بدائل اللغة المتبادلة، فارتفع عدد عناوين sitemap المحلية من 17 إلى 29.
- إضافة ترويسة `Content-Language` بحسب المسار، وتحديث Schema الموقع العام ليعلن دعم العربية والإنجليزية.
- عدم إنشاء مسارات إنجليزية للصفحات المدارة مثل الخصوصية والشروط؛ عناوينها مترجمة حاليًا لكن محتواها الإنجليزي الكامل لم يُنشأ بعد حسب قرار المستخدم السابق.

الأخطاء المكتشفة:

1. **اللغة الإنجليزية كانت تستخدم الرابط العربي نفسه**
   - الأعراض: تبديل اللغة يغير النص داخل الصفحة فقط، لذلك لا توجد وثيقة إنجليزية مستقلة يمكن أن تنافس بعنوانها في Google.
   - السبب: اعتماد اللغة على `localStorage` وحالة React دون تمثيلها في بنية URL أو Metadata.
   - الحل: إضافة مسارات `/en` فعلية وربط اللغة والتنقل وCanonical وSchema وSitemap بالمسار.
   - الحالة: منجز ومختبر محليًا.

2. **احتمال تضارب اللغة المحفوظة مع لغة الرابط**
   - الأعراض: كان يمكن للغة محفوظة في المتصفح أن تستبدل لغة الصفحة بعد التحميل.
   - السبب: قراءة `site_lang` دون منح المسار أولوية.
   - الحل: جعل لغة مسارات الأدوات هي المصدر الحاسم، واستخدام اللغة المحفوظة فقط في الصفحات التي لا تملك مسارًا مترجمًا مستقلًا.
   - الحالة: منجز ومختبر تفاعليًا.

3. **ومضة لغة مخالفة للرابط أثناء الانتقال**
   - الأعراض: عند الضغط على زر العربية داخل `/en/age-calculator` كان النص يتحول إلى العربية قبل اكتمال انتقال الرابط بنحو ثانية، بينما يبقى Canonical الإنجليزي مؤقتًا.
   - السبب: تحديث حالة اللغة محليًا قبل تنفيذ `router.push` وانتظار الصفحة المناظرة.
   - الحل: إبقاء لغة الصفحة الحالية ثابتة أثناء التنقل، وترك المسار الجديد يحدد اللغة بعد اكتمال الانتقال.
   - الحالة: منجز في الإصدار `0.3.50` ويخضع للفحص النهائي.

الملفات المتأثرة:

- `app/en/page.jsx`
- `app/en/clock/page.jsx`
- `app/en/weather/page.jsx`
- `app/en/[slug]/page.jsx`
- `app/en/EnglishToolPage.jsx`
- `app/localizedToolRoutes.js`
- `app/toolSeoServer.js`
- `app/SiteShell.jsx`
- `app/Header.jsx`
- `app/components/ToolSeoContent.jsx`
...
- نجحت 75 حالة اختبار في 20 ملفًا، ونجح ESLint وبناء Next.js الكامل وتوليد 34 صفحة ثابتة/نظامية.
- أعادت المسارات الإنجليزية الاثنا عشر محليًا حالة `200` وعنوانًا وCanonical إنجليزيين مع `Content-Language: en`.
- أظهرت `/en/age-calculator` نصًا إنجليزيًا في HTML الأولي وSchema بلغة `en-US` وروابط بديلة للعربية والإنجليزية و`x-default`.
- أثبت الفحص التفاعلي أن الصفحة الإنجليزية تستخدم `lang=en` و`dir=ltr` وروابط هيدر وأدوات مشابهة تحت `/en`، وأن زر اللغة ينتقل إلى الصفحة العربية المناظرة ويعيد `RTL`.
- أثبت فحص `/en/weather` على عرض 390 بكسل عدم وجود تجاوز أفقي.
- تضم sitemap المحلية 29 عنوانًا، منها 12 عنوانًا إنجليزيًا.
- نُشر الإصدار `0.3.49` مبدئيًا وأثبت وجود المسارات الإنجليزية الاثني عشر وSitemap وMetadata الصحيحة، ثم رُصدت ومضة تبديل اللغة وعولجت في `0.3.50` قبل إغلاق المهمة.
- نُشر الإصدار `0.3.50` عبر بناء Cloudflare المرتبط بفرع `master` للالتزام `dfae46a`، وأعادت واجهة الإصدار الحية `version: 0.3.50` بتاريخ `2026-09-01`.
- أثبت الفحص الحي النهائي أن `/en/age-calculator` بقيت إنجليزية مع Canonical الإنجليزي حتى اكتمال التنقل، ثم انتقل الرابط والمحتوى وCanonical معًا إلى `/age-calculator` بالعربية دون ومضة متعارضة.
- بقيت المسارات الإنجليزية الاثنا عشر بحالة `200`، واحتفظت sitemap بـ29 عنوانًا منها 12 عنوانًا إنجليزيًا بعد نشر التصحيح.

حالة قائمة التحسينات المحفوظة للرجوع إليها لاحقًا:

- **منجز:** مسارات إنجليزية مستقلة مع `hreflang` وCanonical وSitemap وتنقل ثنائي اللغة.
- **متبقي:** أمثلة عملية وطريقة الحساب وحدود الدقة ومصادر البيانات داخل كل صفحة أداة.
- **متبقي:** تحميل مكتبة AdSense مرة واحدة، حجز أبعاد مواضع الإعلانات، وتأجيل الإعلانات الواقعة أسفل الشاشة.
- **متبقي:** حاسبة أيام العمل مع إعداد عطلة نهاية الأسبوع.
- **متبقي:** حاسبة إضافة أو طرح مدة من تاريخ.
- **متبقي:** عد تنازلي لموعد مع تصدير `.ics`.
- **متبقي:** منظم اجتماع بين عدة مدن وساعات العمل المشتركة.
- **متبقي:** أداة الشروق والغروب وطول النهار والساعة الذهبية.
- **متبقي:** توقعات الطقس بالساعة إذا كانت واجهة البيانات والتكلفة مناسبتين.
- **متبقي:** قياس Core Web Vitals الفعلي لكل مسار ومراقبة `LCP` و`INP` و`CLS`.
- **مؤجل عمدًا:** إنشاء صفحات آلية لكل مدينة أو تاريخ، وإضافة تقييمات أو Schema غير مستندة إلى محتوى حقيقي.

### مزامنة إعدادات الواجهة في بيئة التطوير المحلية - غير منشور

تم إنجازه:

- إبقاء خادم التطوير محصورًا على `127.0.0.1` مع السماح له بالاتصال بالخدمات الخارجية اللازمة.
- تعطيل كاش إعدادات الموقع العام في وضع `development` فقط، مع إبقاء كاش الإنتاج لمدة خمس دقائق كما هو.
- مسح كاش `sessionStorage` وإرسال إشارة محلية بعد نجاح `saveSiteConfig` أو `saveSiteConfigSection` حتى تعيد التبويبات العامة المفتوحة قراءة الإعدادات فورًا.
- التحقق مباشرة من Firebase أن `dateTop` و`dateMiddle` مفعّلان، ثم إثبات أن API المحلي وHTML الصفحة يعرضان قيمة بانر «أعلن معنا» الحديثة.

الأخطاء المكتشفة:

1. **تأخر ظهور إعدادات الإدارة في التطوير المحلي**
   - الأعراض: تفعيل بانر «أعلن معنا» وحفظ نصه لا ينعكس على الصفحة المحلية رغم نجاح الحفظ في Firebase.
   - السبب: استخدام كاش Next.js لخمس دقائق مع كاش `sessionStorage` لخمس دقائق أيضًا، فكانت بيئة التطوير تعيد نسخة سابقة من `settings/public`.
   - الحل: تجاوز الكاش في التطوير فقط وإشعار تبويبات الواجهة العامة بعد كل حفظ ناجح.
   - الحالة: منجز ومختبر محليًا، وغير منشور حسب طلب المستخدم.
...
npx eslint app/SiteShell.jsx app/firebase.js app/api/site-config/route.js app/components/PublicAdSlot.jsx
npm test
Invoke-RestMethod http://127.0.0.1:3000/api/site-config
Invoke-WebRequest http://127.0.0.1:3000/
```

الحالة:

- أعاد API المحلي `dateTop=True` و`dateMiddle=True` والنص المحفوظ `اعلن معنا للتجربة`.
- أعادت الصفحة المحلية حالة `200` واحتوى HTML الفعلي على `public-ad-house` ونص البانر.
- نجح ESLint للملفات المتأثرة، ونجحت 75 حالة اختبار في 20 ملفًا.
- لم يُشغّل أي نشر أو رفع إلى Cloudflare أو GitHub، والتغييرات ما زالت محلية فقط.

### عزل بيئة التطوير عن الإنتاج بالكامل - غير منشور

تم إنجازه:

- إنشاء مشروع Firebase مستقل باسم `date-tools-development` دون Gemini أو Google Analytics.
- تسجيل تطبيق ويب مستقل داخل مشروع التطوير وحفظ إعداداته في `.dev.vars` المحلية المستبعدة من Git.
- إنشاء قاعدة Cloud Firestore قياسية مستقلة في منطقة الدمام `me-central2` ونشر `firestore.rules` الحالية عليها فقط.
- تفعيل تسجيل الدخول بالبريد وكلمة المرور داخل مشروع التطوير.
- إنشاء حساب مدير تطوير مستقل وإضافة مستند `admins/{uid}` بدور `super_admin` داخل Firestore التطويري.
- إنشاء حساب خدمة مخصص باسم `datetools-dev-server` ومنحه دور `Cloud Datastore User` فقط داخل مشروع التطوير.
- حفظ مفتاح حساب الخدمة في `.dev.vars` وحذف ملف JSON المنزّل بعد الدمج، مع التأكد أن الملف المحلي مستبعد من Git.
- منع `npm run dev` من البدء إذا كانت قيم Firebase ناقصة أو تشير إلى مشروع الإنتاج `date-tool-official`.
- تحميل `.dev.vars` فعليًا داخل عملية Next.js بدل الاكتفاء بفحص الملف دون تمرير قيمه.
- تحويل جميع مسارات REST والخادم التي كانت تستخدم معرّف Firebase ثابتًا إلى معرّف البيئة المختارة.
- إضافة alias باسم `development` في `.firebaserc` مع إبقاء `default` على مشروع الإنتاج دون تغيير.
- إضافة `wrangler.dev.jsonc` باسم Worker محلي `datetools-development-local` وR2 محلي `datetools-media-development-local` دون routes أو موارد إنتاج.
- إضافة أمر `npm run preview:dev` لبناء وتشغيل معاينة Cloudflare المحلية الكاملة مع Firebase التطويري وR2 المحلي.
- إضافة أمر `npm run preview:dev:start` لتشغيل مخرجات المعاينة الموجودة مباشرة دون إعادة بناء.
- تشغيل المعاينة الحالية كعملية خلفية مخفية مع حفظ سجلاتها داخل `.local-config` المستبعد من Git، حتى لا تتوقف بانتهاء جلسة Codex.
- إبقاء Turnstile متجاوزًا فقط على `localhost` و`127.0.0.1` عند غياب مفاتيح التطوير، مع بقاء حماية الإنتاج كما هي.
- تنظيف خوادم Next/Workerd المحلية القديمة التي كانت تمسك مخرجات OpenNext وتمنع إعادة البناء على Windows.

الأخطاء المكتشفة:

1. **بيئة التطوير المحلية كانت متصلة ببيانات Firebase الإنتاجية**
   - الأعراض: تعديل الإعدادات محليًا ينعكس على الموقع المنشور وتظهر بيانات الإنتاج نفسها في الواجهة المحلية.
   - السبب: إعداد Firebase Web ومعرّف REST كانا ثابتين على `date-tool-official`.
   - الحل: إنشاء مشروع Firebase مستقل، جعل الإعدادات معتمدة على متغيرات البيئة، وإضافة حواجز توقف التشغيل عند اكتشاف مشروع الإنتاج.
   - الحالة: منجز ومختبر محليًا.

2. **فحص `.dev.vars` لم يكن يمرر القيم إلى Next.js**
   - الأعراض: ينجح فحص العزل ثم تعمل عملية `next dev` دون قيم Firebase التطويرية.
   - السبب: `next dev` لا يقرأ `.dev.vars` تلقائيًا.
   - الحل: تشغيل الفحص وNext.js عبر `node --env-file=.dev.vars`.
   - الحالة: منجز ومغطى باختبار.

3. **معاينة Cloudflare المحلية كانت تستخدم ملف إنتاج واحدًا**
   - الأعراض: لا توجد طريقة واضحة لاختبار R2 محليًا دون الاعتماد على `wrangler.jsonc` الخاص بالإنتاج.
   - السبب: وجود أمر preview عام فقط.
   - الحل: إضافة `wrangler.dev.jsonc` وأمر `preview:dev` بأسماء موارد محلية مختلفة صراحة.
   - الحالة: منجز ومختبر على `127.0.0.1:8787`.

4. **مجلد OpenNext كان مقفولًا بعمليات اختبار قديمة**
   - الأعراض: فشل بناء المعاينة بخطأ `EPERM` عند حذف `.open-next`.
   - السبب: بقاء خوادم `next start` وعمليات `workerd` قديمة على المنافذ `3010` إلى `3012`.
   - الحل: إيقاف عمليات المشروع المحلية القديمة فقط ثم تنظيف مجلد المخرجات وإعادة البناء.
   - الحالة: منجز.

5. **رابط المعاينة توقف بعد انتهاء مهمة Codex**
   - الأعراض: أعاد المتصفح `ERR_CONNECTION_REFUSED` على `127.0.0.1:8787` رغم نجاح المعاينة سابقًا.
   - السبب: خادم Wrangler كان مرتبطًا بجلسة التنفيذ المؤقتة وانتهى بانتهائها.
   - الحل: إضافة أمر تشغيل سريع وتشغيله كعملية خلفية مخفية مستقلة، مع التحقق من المنفذ والصفحة والاتصال بـFirebase.
   - الحالة: منجز؛ المنفذ `8787` يستمع حاليًا.
...
- `app/api/statistics/route.js`
- `app/api/support/route.js`
- `scripts/verify-development-environment.mjs`
- `tests/developmentIsolation.test.js`
- `PROJECT_MEMO.md`

الأوامر المستخدمة:

```powershell
npx --yes firebase-tools firestore:locations --project date-tools-development
npx --yes firebase-tools firestore:databases:create '(default)' --location me-central2 --edition standard --project date-tools-development
npx --yes firebase-tools deploy --only firestore:rules --project date-tools-development
npm run dev -- -p 3000
npm run preview:dev
npm run preview:dev:start
npm test
npx eslint app/firebaseEnvironment.js scripts/verify-development-environment.mjs tests/developmentIsolation.test.js
git diff --check
```

الحالة:

- نجحت 77 حالة اختبار في 21 ملفًا، ونجح ESLint وبناء Next.js وOpenNext الكامل.
- تعمل المعاينة السريعة عبر `http://127.0.0.1:3000` عند تشغيل `npm run dev -- -p 3000`.
- تعمل معاينة Cloudflare الكاملة حاليًا عبر `http://127.0.0.1:8787` وتشمل R2 محليًا.
- تعمل المعاينة الحالية كعملية خلفية مخفية وتكتب سجلاتها في `.local-config/preview.out.log` و`.local-config/preview.err.log`.
- أعادت الصفحة المحلية `200` دون بانر الإنتاج، وأعاد `/api/public-campaigns` حالة `200` مع قائمة فارغة من Firestore التطويري.
- أعاد مسار ملف R2 غير موجود حالة `404 media_not_found` بدل `503`، ما يثبت ارتباط bucket المحلي.
- بقي `wrangler.jsonc` ومشروع `date-tool-official` وWorker `datetools` وR2 الإنتاجي دون تعديل أو نشر.
- لم يُشغّل `git push` أو Cloudflare deploy، وجميع تغييرات الكود ما زالت محلية فقط حسب طلب المستخدم.

### إصلاح عرض إدارة الإعلانات على الجوال - غير منشور

تم إنجازه:

- إعادة بناء `/admin/ad-settings` كمحتوى إدارة داخلي يعتمد على `AdminShell` العام بدل إنشاء غلاف إدارة وقائمة جانبية وشريط علوي وتوثيق مستخدم للمرة الثانية داخل الصفحة.
- وضع جدول مواضع الإعلانات داخل سيكشن مركزي مستقل ومتناسق مع `tools-section-card`، مع رأس سيكشن واضح وحاوية جدول داخلية ذات حدود وتدوير موحدين.
- إبقاء مواضع الإعلانات على هيئة جدول حقيقي في الجوال وسطح المكتب، وتقليل عرضه الأدنى من `900/920px` إلى `720px` مع تمرير أفقي محصور داخل حاوية الجدول فقط.
- استخدام مكوّن `AdminEnableToggle` المشترك نفسه لتشغيل Google والنص التسويقي، وتوحيد أزرار العرض وتعديل الكود والتفاصيل مع `tools-item-actions` من حيث الحجم والألوان والمسافات.
- حصر عرض تخطيط الإدارة والقائمة الجانبية داخل عرض شاشة الجوال لمنع الانزياح والتمرير الأفقي.
- إضافة زر إغلاق واضح للقائمة الجانبية على الجوال، ودعم إغلاقها بزر `Escape` أو بالنقر على الخلفية المعتمة.
- منع تمرير الصفحة الخلفية أثناء فتح القائمة، ثم إعادة التمرير مباشرة بعد إغلاقها.
- تحديث اختبار تنقل الإدارة ليتحقق من ملكية `AdminShell` للقائمة وعدم إعادة نسخها داخل صفحة إعدادات الإعلانات.
- إعادة بناء معاينة OpenNext المحلية وتشغيلها على `127.0.0.1:8787` دون نشر خارجي.

الأخطاء المكتشفة:

1. **جدول إدارة الإعلانات يتجاوز عرض الجوال**
   - الأعراض: بقاء حد أدنى بعرض `900px` للجدول، ما يجعل المحتوى مقصوصًا ويدفع المستخدم إلى تمرير أفقي مربك.
   - السبب: كان عرض الجدول العريض يتسرب إلى عرض المستند في اتجاه RTL ولم تكن للجدول حاوية داخلية مستقلة داخل السيكشن.
   - الحل: وضع الجدول داخل حاوية تمرير مستقلة بعرض `100%`، وخفض الحد الأدنى إلى `720px`، وتطبيق متغيرات رأس الجدول والخلايا الموحدة في المشروع.
   - الحالة: منجز ومختبر محليًا.

2. **القائمة الجانبية تغطي صفحة الجوال دون تحكم إغلاق كافٍ**
   - الأعراض: ظهور القائمة فوق المحتوى مع صعوبة فهم طريقة إغلاقها وتحرك الخلفية أثناء بقائها مفتوحة.
   - السبب: الاعتماد على الخلفية المعتمة وحدها للإغلاق وعدم قفل تمرير الصفحة.
   - الحل: إضافة زر إغلاق، ودعم `Escape`، وقفل تمرير الخلفية، وتقييد عرض القائمة إلى `86vw` بحد أقصى `280px`.
   - الحالة: منجز ومختبر محليًا.

3. **صفحة إعدادات الإعلانات تكرر تخطيط الإدارة داخل التخطيط العام**
   - الأعراض: شريط جانبي ثانٍ يغطي المحتوى، ومساحة صفحة ضيقة ومنزاحة، واختلاف واضح عن تصميم بقية صفحات الإدارة.
   - السبب: احتوت الصفحة على `legacy-admin-shell` وSidebar وTopNav وتحقق صلاحية مستقل، رغم أن Layout الإدارة يغلّفها مسبقًا بـ`AdminShell`.
   - الحل: حذف الغلاف والتوثيق والتنقل المكررة، وترك الصفحة تعرض محتواها فقط داخل التخطيط المركزي العام.
   - الحالة: منجز ومختبر محليًا.
...
```powershell
npx eslint app/admin/ad-settings/page.jsx app/admin/AdminEnableToggle.jsx app/admin/AdminShell.jsx
git diff --check
npm run build
npm run preview:dev
npm test
...
- نجح ESLint وفحص الفروقات وبناء Next.js وبناء OpenNext الكامل.
- نجحت 78 حالة اختبار في 21 ملفًا.
- أثبت الفحص المرئي على عرض متصفح الجوال الفعلي أن السيكشن والصفحة بعرض واحد ومتمركزان، وأن الجدول بقي `table` مع تمرير أفقي داخل الحاوية فقط.
- أثبت قياس DOM أن عرض المستند `614px` وعرض تمريره `614px` دون أي تجاوز أفقي، وأن حاوية الجدول بعرض `518px` وتحتوي جدولًا بعرض `720px` داخليًا.
- ظهرت أزرار الإجراءات الثلاثة بالحجم العام `38x38` وبألوان العرض البنفسجي وتعديل الكود الأصفر والتفاصيل الأزرق.
- أثبت اختبار القائمة أن الخلفية المعتمة تغطي عرض الشاشة كاملًا، وأن القائمة بعرض `280px` وتغلق من الزر مع إعادة تمرير الصفحة.
- أثبت فحص عرض `1280px` أن الجدول بقي `table` وصفوفه بقيت `table-row` دون تغيير في نسخة سطح المكتب.
- تعمل المعاينة المحلية المحدثة على `http://127.0.0.1:8787`، ولم يُشغّل أي نشر أو رفع إلى Cloudflare أو GitHub.

### إعادة بناء الصفحة الرئيسية للوحة الإدارة كلوحة إحصائية شاملة - غير منشور

تم إنجازه:

- إعادة بناء `/admin` بالكامل كمحتوى داخلي يعتمد على `AdminShell` العام، وحذف القائمة الجانبية والشريط العلوي والتوثيق والفوتر المكررة داخل الصفحة.
- إضافة هيدر تشغيل مختصر يعرض اسم المدير ودوره ووقت آخر تحديث وزرًا لإعادة قراءة البيانات دون إعادة تحميل الصفحة.
- إضافة ثمانية مؤشرات رئيسية للزيارات، واستخدام الأدوات، وظهور الإعلانات، ونقراتها، والحملات النشطة والمعلقة، والمعلنين، والتذاكر المفتوحة.
- إضافة لوحة توزيع استخدام أدوات العمر وتحويل التاريخ وحساب المدة والساعة والطقس مع النسب وأشرطة مقارنة مرئية.
- إضافة ملخص حالة المحتوى والعرض يشمل الصفحات والروابط والسوشيال ومواضع Google والنصوص التسويقية ومجموعات الأدوات.
- إضافة لوحة أداء الإعلانات والحملات تشمل CTR والإجماليات وحالات الحملات ومشاهداتها ونقراتها.
- إضافة أحدث نشاطات الحملات والتذاكر وروابط إجراءات سريعة إلى الأقسام الأكثر استخدامًا.
- قراءة الحملات والمعلنين والتذاكر كبيانات اختيارية حسب صلاحية المدير، وإظهار شرطة وعبارة توضيحية بدل رقم صفري مضلل عند عدم توفر الصلاحية.
- استخدام تصميم متجاوب بثماني بطاقات موزعة على أربعة أعمدة في الشاشات الواسعة وعمودين في الجوال، وتحويل اللوحات المزدوجة إلى عمود واحد تلقائيًا.
- تحديث اختبار تخطيط الإدارة ليتأكد أن `/admin` و`/admin/ad-settings` لا تعيدان إنشاء التنقل العام داخل محتوى الصفحة.

الأخطاء المكتشفة:

1. **الصفحة الرئيسية تكرر تخطيط الإدارة داخل `AdminShell`**
   - الأعراض: وجود هيكل إدارة ثانٍ داخل الصفحة، وتعقيد الاستجابة للجوال، وتحميل تحقق دخول وقائمة تنقل وفوتر مكررين.
   - السبب: بقاء الصفحة من مرحلة ما قبل إضافة Layout الإدارة الدائم.
   - الحل: جعل الصفحة مكوّن محتوى فقط واستخدام `useAdminShell` لاسم المدير ودوره، وترك التوثيق والتنقل والغلاف للمكوّن العام.
   - الحالة: منجز ومختبر محليًا.

2. **الإحصاءات السابقة كانت مقتصرة على التاريخ وأربعة مواضع إعلانية قديمة**
   - الأعراض: عدم ظهور استخدام الساعة والطقس أو حالة الحملات والمعلنين والتذاكر والإعدادات، ووجود مسميات مواضع قديمة لا تمثل النظام الحالي.
   - السبب: اعتماد الصفحة القديمة على مجموعة مؤشرات ثابتة لم تعد تغطي بنية المنصة الحالية.
   - الحل: بناء ملخصات ديناميكية من `statistics/main` وإعدادات الموقع ومجموعات الحملات والمعلنين وتذاكر الدعم الحالية.
   - الحالة: منجز ومختبر محليًا.
...
```powershell
npx eslint app/admin/page.jsx app/admin/AdminShell.jsx tests/adminNoReload.test.js
npm test
git diff --check
npm run preview:dev
Invoke-WebRequest http://127.0.0.1:8787/admin
```

الحالة:

- نجح ESLint وفحص الفروقات، ونجحت 78 حالة اختبار في 21 ملفًا.
- نجح بناء Next.js الكامل وتوليد 34 صفحة، ثم نجح بناء OpenNext وتشغيل معاينة Cloudflare المحلية.
- أعاد `/admin` محليًا حالة `200` وحمّل حزمة الصفحة `app/admin/page-adf263fbe90912f6.js` المتضمنة عناوين اللوحة الإحصائية الجديدة.
- حمّلت الصفحة ملف CSS المحدث المتضمن `admin-overview-page` و`admin-overview-kpi-grid` وقواعد الاستجابة للجوال.
- احتفظ التبويب المفتوح مؤقتًا بواجهة قديمة من ذاكرة تنقل المتصفح؛ النسخة المقدمة من الخادم هي الجديدة وتظهر بعد تحديث التبويب.
- لم يُشغّل أي نشر أو رفع إلى Cloudflare أو GitHub، والتغييرات محلية فقط.

### استكمال الترجمة وتحديث النتائج والمحتوى الإنجليزي - الإصدار 0.3.51

تم إنجازه:

- جعل نتائج حساب العمر وتحويل التاريخ وحساب المدة تعاد صياغتها تلقائيًا باللغة الجديدة عند تبديل اللغة بعد ظهور النتيجة، دون احتساب زيارة استخدام جديدة.
- جعل نتيجة تحويل الساعة وفرق التوقيت يعاد عرضهما تلقائيًا بصياغة `AM/PM` ونص فرق الساعات باللغة الحالية.
- ترجمة أسماء الأيام وبانر تاريخ اليوم والمواعيد الأساسية الحالية: حساب المواطن والراتب والتقاعد، مع أولوية لحقل `nameEn` الإداري عند وجوده.
- إضافة قوالب مشاركة وأسئلة شائعة إنجليزية افتراضية صحيحة للتاريخ والساعة والطقس بدل الرجوع إلى النص العربي عند غياب ترجمة محفوظة.
- ربط صفحات المحتوى المُدارة بلغة `SiteContext`، ودعم `titleEn` و`contentEn` مع ترجمة كاملة لنموذج التواصل ومحتوى احتياطي إنجليزي لصفحات من نحن والخصوصية والشروط وجدول الأشهر.
- إضافة محرر عربي وإنجليزي مستقلين لمحتوى كل صفحة داخل `/admin/tools`، مع تنظيف HTML في الحقلين قبل الحفظ.
- إظهار بانر تاريخ اليوم وقسم الأسئلة الشائعة في صفحة التاريخ الإنجليزية بدل إخفائهما.
- ترجمة نافذة اختيار المواعيد للمشاركة بالكامل، بما فيها أزرار التحديد والإلغاء ونص الأيام المتبقية.
- توسيع المحتوى السفلي الإنجليزي لصفحات التاريخ والساعة والطقس ليطابق تغطية المحتوى العربي من حيث الدليل، حالات الاستخدام، الدقة والخصوصية ومصدر البيانات.
- إضافة نص إنجليزي مستقل للبانر التسويقي في مواضع الإعلانات عبر `houseAdTextEn` مع بديل إنجليزي آمن، وإضافة حقله إلى إدارة الإعلانات.

الأخطاء المكتشفة:

1. **النتيجة تبقى باللغة السابقة بعد تبديل اللغة**
   - الأعراض: تظهر نتيجة الحساب أو تحويل الساعة بالعربية بعد الانتقال إلى الإنجليزية أو العكس.
   - السبب: كانت النتيجة تحفظ كنص مترجم نهائي وقت الضغط على زر الحساب.
   - الحل: حفظ هوية آخر عملية ناجحة وإعادة توليد نص النتيجة وقالب المشاركة عند تغير اللغة دون تكرار التتبع.
   - الحالة: منجز ومغطى باختبارات.

2. **نقص المحتوى السفلي والأسئلة في صفحة التاريخ الإنجليزية**
   - الأعراض: المحتوى الإنجليزي أقصر من العربي، وأسئلة التاريخ وبانر اليوم لا يظهران في `/en`.
   - السبب: وجود شروط قديمة تخفي `TodayBanner` و`SeoSections` عند أي لغة غير العربية، مع نسخة إنجليزية مختصرة من محتوى SEO.
   - الحل: إظهار بانر اليوم بالترجمة الحالية، وإظهار أسئلة التاريخ الإنجليزية فقط دون تكرار دليل الصفحة، واستكمال ترجمة جميع أقسام المحتوى السفلي.
   - الحالة: منجز ومختبر محليًا.

3. **نافذة مشاركة المواعيد والنص التسويقي يظهران بالعربية في الإنجليزية**
   - الأعراض: عناصر فرعية عربية داخل مسارات `/en` رغم ترجمة الواجهة الأساسية.
   - السبب: نصوص نافذة المشاركة واسم حقل الإعلان الاحتياطي كانت ثابتة بالعربية.
   - الحل: ربط نافذة المشاركة باللغة الحالية، وإضافة `houseAdTextEn` مع قيمة إنجليزية احتياطية.
   - الحالة: منجز ومختبر محليًا.
...
Invoke-WebRequest http://127.0.0.1:3000/en/clock
Invoke-WebRequest http://127.0.0.1:3000/en/weather
```

الحالة:

- نجح ESLint وفحص الفروقات وبناء Next.js الكامل وتوليد 34 صفحة.
- نجحت 80 حالة اختبار في 20 ملفًا.
- أعادت `/en` و`/en/clock` و`/en/weather` محليًا حالة `200`.
- احتوى HTML الفعلي للمسارات الإنجليزية على الأقسام السفلية الكاملة وعنوان الأسئلة الشائعة، ولم يحتو على عنوان نافذة مشاركة المواعيد العربي.
- اكتمل بناء OpenNext محليًا، ثم دُفع الالتزام `d740281` إلى `master` لتشغيل Cloudflare Build بالمفتاح المحدود المرتبط بدل إنشاء اعتماد CLI واسع.
- ظهر الإصدار المنشور على Cloudflare، وأعادت `/` و`/en` و`/clock` و`/en/clock` و`/weather` و`/en/weather` حالة `200`.
- أكد فحص الإنتاج ظهور المحتوى السفلي والأسئلة الإنجليزية الكاملة، وأعادت `/api/app-version` الإصدار `0.3.51` بتاريخ `2026-09-04`.
- أُوقفت المعاينة المحلية على المنفذ `3000` لتحرير مخرجات `.open-next` قبل البناء، والحالة الحالية منشورة في الإنتاج.

### إغلاق نتائج تقارير الأمان الحالية - الإصدار 0.3.52

تم إنجازه:

- مراجعة سجلات Cloudflare Worker لآخر 7 أيام؛ أظهرت 9,880 عملية ناجحة و3,280 عملية مصنفة أخطاء مع تنبيه أن النتائج عيّنة غير مكتملة بسبب حجم البيانات.
- تحليل تقارير CSP الفعلية وتحديد أربعة مصادر شرعية محجوبة: Cloudflare Web Analytics وGoogle APIs وإطار Firebase Auth واتصال Google Tag Manager.
- إضافة المصادر الأربعة بصورة محددة إلى توجيهات CSP المناسبة، مع إضافة وجهة Cloudflare Insights اللازمة للـbeacon، وإبقاء السياسة بصيغة Report-Only للمراقبة بعد النشر.
- ربط قراءات `settings/public` الخادمية برمز OAuth لحساب الخدمة عند توفر أسرار الإنتاج، مع رجوع مؤقت للقراءة العامة الحالية عند غياب الأسرار أو تعذر إصدار الرمز.
- إضافة رفض مبكر لمسارات فحص PHP وWordPress و`.env` و`.git` و`_ignition` قبل تشغيل صفحات Next.js، لتقليل استهلاك CPU من طلبات الفحص الآلية الظاهرة في Cloudflare.
- مراجعة Firebase App Check لآخر 7 أيام: 8% موثق (917 من 12 ألف)، و1% عميل قديم (75)، و92% مصدر غير معروف (11 ألف)، وصفر طلبات غير صالحة.
- إبقاء Firestore App Check في وضع Monitoring؛ النسبة الحالية لا تسمح بالفرض دون خطر تعطيل طلبات شرعية وخادمية.
- مراجعة Search Console: خريطة الموقع ناجحة وبها 29 صفحة مكتشفة وآخر قراءة في 2 سبتمبر 2026، ولا توجد إجراءات يدوية أو مشاكل أمان.
- تأكيد أن تقرير فهرسة الصفحات قديم بآخر تعديل 28 أغسطس 2026 ويعرض 3 صفحات مفهرسة و19 غير مفهرسة، ولذلك لا يُعامل كقياس لآخر نشر قبل إعادة الزحف.
- إضافة اختبارات حارسة لمصادر CSP ورفض مسارات الفحص ومصادقة القراءة الخادمية، وتحديث إصدار PWA إلى `0.3.52`.

الأخطاء المكتشفة:

1. **سياسة CSP كانت ستمنع خدمات مستخدمة فعليًا عند فرضها**
   - الأعراض: أربعة تقارير `csp_report_only` على الإدارة وبوابة العميل.
   - السبب: غياب مصادر Cloudflare Insights وGoogle APIs وFirebase Auth وGoogle Tag Manager من التوجيهات الدقيقة.
   - الحل: إضافة كل مصدر إلى `script-src` أو `frame-src` أو `connect-src` بحسب التقرير، مع استمرار المراقبة قبل الفرض.
   - الحالة: محلول برمجيًا؛ ينتظر قياس التقارير بعد انتشار الإصدار.

2. **طلبات الفحص الآلية تستهلك Worker وتظهر بجوار تجاوزات CPU**
   - الأعراض: طلبات متكررة لمسارات PHP وWordPress وملفات `.env` و`.git` مع سجلات `Worker exceeded CPU time limit`.
   - السبب: وصول المسارات العشوائية إلى طبقة Next.js الديناميكية قبل إرجاع 404.
   - الحل: إرجاع 404 و`noindex` من middleware مباشرة لهذه الأنماط مع كاش حافة قصير.
   - الحالة: محلول ومختبر محليًا.

3. **غالبية طلبات Firestore لا تحمل App Check**
   - الأعراض: 92% من 12 ألف طلب مصنفة Unknown origin مقابل 8% فقط Verified.
   - السبب: القراءات الخادمية تستخدم Firestore REST ولا تأتي من Firebase Client SDK، إضافة إلى حركة النسخ الأقدم خلال نافذة القياس.
   - الحل: مصادقة القراءة الخادمية بحساب الخدمة، ثم إعادة القياس بعد انتشار الإصدار قبل أي Enforcement.
   - الحالة: عولج مسار الخادم برمجيًا؛ فرض App Check ما زال مؤجلًا عمدًا.

4. **تعذر إكمال npm audit من سجل npm**
   - الأعراض: فشل endpoint الخاص بالتدقيق ثم أعاد `ECONNRESET` بعد السماح بالاتصال الخارجي.
   - السبب: انقطاع اتصال سجل npm أثناء طلب advisories، وليس خطأ بناء أو تبعيات محلية.
   - الحل: إعادة المحاولة لاحقًا دون تشغيل `npm audit fix --force`، مع استمرار الاعتماد على الاختبارات والبناء وتنبيهات المستودع عند توفرها.
   - الحالة: مفتوح خارجيًا بسبب اتصال سجل npm.

الملفات المتأثرة:

- `middleware.js`
- `app/firestorePublicConfig.js`
- `app/serverFirestoreAuth.js`
- `app/version.js`
- `package.json`
- `package-lock.json`
- `tests/securityBoundaries.test.js`
...
npm start -- --hostname 127.0.0.1 --port 3000
Invoke-WebRequest http://127.0.0.1:3000/
Invoke-WebRequest http://127.0.0.1:3000/probe.php
...
- نجح ESLint وفحص الفروقات وبناء Next.js الكامل وتوليد 34 صفحة.
- نجح بناء OpenNext الكامل وإنشاء `.open-next/worker.js` على Windows بعد تشغيله خارج قيود القراءة المحلية.
...
- تأكد وجود المسارات الإنجليزية الثلاثة في `sitemap.xml` المنشور، ثم أُرسلت طلبات الفهرسة يدويًا إلى Google للمسارات `/en` و`/en/clock` و`/en/weather` وظهر تأكيد «تم طلب الفهرسة» لكل مسار.
...
### استكمال حماية مستودع GitHub - دون تغيير إصدار

تم إنجازه:

- فحص صفحة Security and quality للمستودع؛ Secret Scanning مفعّل، بينما Dependabot alerts معطّلة وCode scanning يحتاج إعدادًا وسياسة الإبلاغ الأمني غير موجودة.
- مراجعة تنبيه Secret Scanning الوحيد دون نسخ قيمة المفتاح إلى المذكرة؛ التنبيه يخص Firebase Web API Key العام الموجود في إعداد عميل Firebase ومواضع تاريخية أخرى.
- مطابقة التنبيه مع مراجعة Google Cloud السابقة التي أثبتت تقييد المفتاح بنطاقات HTTP المعتمدة وثماني واجهات Firebase لازمة فقط، مع نجاح اختبارات Authentication وFirestore بعد حفظ القيود.
- إضافة `SECURITY.md` لتوجيه البلاغات إلى قناة GitHub الخاصة ومنع نشر بيانات المستخدمين أو المفاتيح أو تفاصيل الثغرات في Issues عامة.
- إضافة سير عمل CodeQL متقدم لفحص JavaScript/TypeScript وGitHub Actions عند الدفع إلى `master` وطلبات الدمج وأسبوعيًا، بصلاحيات دنيا واستعلامات `security-extended`.
- التحقق من صحة YAML وتشغيل ESLint والحزمة الكاملة للاختبارات محليًا.

الأخطاء المكتشفة:

1. **تنبيهات اعتماديات GitHub معطلة**
   - الأعراض: صفحة أمان المستودع تعرض Dependabot alerts بحالة Disabled، كما تعذر جلب نتيجة `npm audit` بسبب `ECONNRESET` للمرة الثانية.
   - السبب: إعداد Dependabot غير مفعّل واتصال endpoint الخاص بسجل npm غير مستقر في بيئة التنفيذ.
   - الحل: تفعيل Dependabot alerts من إعدادات المستودع بعد الموافقة الصريحة، والإبقاء على CodeQL والاختبارات كطبقات فحص إضافية.
   - الحالة: ينتظر موافقة المستخدم على تغيير إعداد GitHub؛ لا يوجد تقرير اعتماديات جديد يمكن الحكم عليه حاليًا.

2. **تنبيه Secret Scanning لمعرّف Firebase Web عام ومقيّد**
   - الأعراض: GitHub يعرض تنبيه Google API Key مفتوحًا في عدة مواضع تاريخية.
   - السبب: Secret Scanning لا يميّز تلقائيًا بين المفتاح السري ومفتاح Firebase Web العام المضمّن عمدًا في عميل المتصفح.
   - الحل: إبقاء قيود النطاقات والواجهات في Google Cloud، ثم إغلاق التنبيه كاستخدام مقصود ومقيّد بعد الموافقة الصريحة، دون إلغاء المفتاح العامل أو نقله إلى متغير بيئة يوهم بأنه أصبح سريًا.
   - الحالة: القيود محققة ومختبرة؛ تصنيف التنبيه وإغلاقه ينتظران موافقة المستخدم.

3. **Code scanning وسياسة الإبلاغ لم يكونا معدّين**
   - الأعراض: GitHub يعرض Code scanning بحالة Needs setup وSecurity policy بحالة Disabled.
   - السبب: عدم وجود Workflow لـCodeQL أو ملف `SECURITY.md` في المستودع.
   - الحل: إضافة الملفين محليًا وتشغيل التحقق، ثم دفعهما إلى `master` بعد الموافقة ليبدأ أول فحص وتظهر السياسة في صفحة Security.
   - الحالة: محلول محليًا ومختبر؛ ينتظر النشر إلى GitHub.
...
```powershell
npm audit --omit=dev
node -e "const fs=require('fs'); const YAML=require('yaml'); YAML.parse(fs.readFileSync('.github/workflows/codeql.yml','utf8'))"
npm run lint
npm test
git diff --check
```
...
- صيغة Workflow صحيحة، ونجح ESLint دون أخطاء، ونجحت 81 حالة اختبار في 20 ملفًا، ونجح `git diff --check`.
- لم تُنشر ملفات الحماية الجديدة ولم يُغلق تنبيه Secret Scanning ولم تتغير إعدادات GitHub بعد.
- لا حاجة لتدوير Firebase Web API Key بسبب التنبيه وحده ما دامت قيود النطاقات والواجهات محفوظة؛ أي تدوير غير ضروري سيكسر العملاء المثبتين والنسخ القديمة دون فائدة أمنية إضافية.

### مركز الأمان الإداري - الإصدار 0.3.53 / admin 0.1.52

تم إنجازه:

- إنشاء صفحة `/admin/security` داخل `AdminShell` الموحد وإضافتها إلى القائمة الجانبية والإجراءات السريعة بصلاحية مستقلة للمساعدين ومتاح تلقائيًا للمدير الكامل.
- إضافة فحص حي غير مؤثر للرؤوس الأمنية السبعة وتهيئة Turnstile وApp Check واستقبال تقارير CSP ورقم الإصدار، مع زر إعادة الفحص ووقت آخر نتيجة.
- إضافة روابط مباشرة لتقارير CodeQL وDependabot وSecret Scanning وGitHub Security Advisories وCloudflare وFirebase App Check دون تخزين رموز هذه الحسابات أو جلب سجلاتها الحساسة إلى المتصفح.
- إضافة خريطة موحدة لطبقات حماية الجلسات والصلاحيات وقواعد Firestore ومفتاح Firebase Web ورفع R2 وCSP وApp Check وWAF والنسخ الاحتياطي.
- نقل إعداد زر الخصوصية واختيار صفحاته بالعربية والإنجليزية من إعدادات الأداة إلى مركز الأمان مع حفظ جزئي لا يغيّر بقية إعدادات الموقع.
- نقل تنظيف Firebase المحمي للمدير الكامل وتذكير النسخ الاحتياطي والاستعادة إلى مركز الأمان، وإزالتهما من صفحة إعدادات الأداة.
- اختبار الواجهة محليًا عند عرض جوال 629 بكسل وعرض كمبيوتر 1280 بكسل؛ لم يظهر تمدد أفقي وكان عدد أشرطة الإدارة واحدًا.
- تحديث الإصدار إلى `0.3.53` ونسخة الإدارة إلى `0.1.52` دون تغيير نسخة بوابة العميل.

الأخطاء المكتشفة:

1. **خادم محلي قديم كان يعرض 404 للمسار الجديد**
   - الأعراض: `/admin/security` أعاد صفحة 404 بعد نجاح البناء.
   - السبب: عملية Next.js قديمة كانت ما تزال تستمع على المنفذ `3000` ببناء سابق.
   - الحل: إيقاف العملية المحددة وتشغيل البناء الحالي ثم إعادة فحص المسار.
   - الحالة: محلول؛ ظهرت الصفحة الجديدة وحمّلت إعدادات الخصوصية والفحص الحي.

2. **اختبارات إصدار PWA كانت مثبتة على 0.3.52**
   - الأعراض: فشل اختباران بعد رفع الإصدار إلى `0.3.53`.
   - السبب: توقعات رقم الإصدار وتاريخه لم تُحدّث بعد.
   - الحل: تحديث التوقعات إلى `0.3.53` وتاريخ `2026-09-05` وإعادة الحزمة كاملة.
   - الحالة: محلول؛ نجحت 85 حالة اختبار.
...
- `package-lock.json`
- `tests/adminNoReload.test.js`
- `tests/securitySettings.test.js`
- `tests/publicSiteConfig.test.js`
- `tests/pwaVersionCheck.test.js`
- `VERSION_LOG.md`
- `PROJECT_MEMO.md`
...
npm run build
npm start -- --hostname 127.0.0.1 --port 3000
Invoke-WebRequest http://127.0.0.1:3000/admin/security
git diff --check
```

الحالة:

- نجح ESLint وفحص الفروقات وبناء Next.js الكامل وتوليد 35 صفحة بينها `/admin/security`.
- نجحت 85 حالة اختبار في 21 ملفًا.
- في البيئة المحلية ظهرت Turnstile وApp Check كغير مهيأتين لأن متغيرات إنتاج Cloudflare ليست محمّلة محليًا؛ الفحص سيقرأ حالة بيئة الإنتاج بعد النشر.
- ينتظر الإغلاق الخارجي دفع الالتزام، اكتمال Cloudflare Build، تفعيل Dependabot وPrivate Vulnerability Reporting، إغلاق تنبيه Firebase Web العام المقيّد، ثم مراجعة أول تشغيل CodeQL.

### معالجة نتائج CodeQL وتفعيل خدمات GitHub الأمنية - الإصدار 0.3.53

تم إنجازه:

- دفع مركز الأمان وملفي `SECURITY.md` وCodeQL إلى `master` وتشغيل أول فحص CodeQL بنجاح.
- تفعيل Dependency graph وDependabot alerts وPrivate vulnerability reporting في إعدادات المستودع، مع إبقاء التحديثات الآلية للاعتماديات معطلة لتجنب تغييرات غير مراجعة.
- مراجعة تنبيهات CodeQL السبعة ومعالجة أسبابها في تعقيم HTML، وحساب طول المحتوى، وتنقية روابط تقارير CSP دون إخفاء التنبيهات يدويًا.
...
1. **سبعة تنبيهات CodeQL عالية في معالجة HTML وروابط CSP**
   - الأعراض: أول تشغيل CodeQL اكتشف سبعة تنبيهات من أنواع التعقيم غير الكامل، وفحص مخطط URL غير الكامل، وتعبير نمطي مكلف.
   - السبب: وجود fallback قديم يعتمد على استبدالات Regex لتعقيم HTML، وتعبير نمطي آخر لتنقية رابط تقرير CSP، وتعبير لحساب طول النص داخل محتوى HTML.
   - الحل: الاعتماد على محللات HTML المهيأة في الخادم والمتصفح، والفشل المغلق عند غياب محلل DOM، وإزالة Regex غير الضروري من حساب الطول، واستخدام `indexOf` في تنقية الرابط.
   - الحالة: محلول ومنشور؛ أغلقت نتيجة CodeQL الثانية التنبيهات السبعة تلقائيًا وأظهر الفحص الثالث نجاحًا و0 تنبيه مفتوح.

2. **تصدير مساعد من ملف Route غير مقبول في Next.js**
   - الأعراض: فشل البناء لأن `app/api/csp-report/route.js` صدّر دالة غير دوال HTTP المعتمدة.
   - السبب: قيود أنواع App Router تمنع صادرات إضافية من ملف Route.
...
3. **ثلاثة تنبيهات Dependabot متوسطة في sanitize-html وqs**
   ...
   - الحالة: محلول ومنشور؛ أعاد `npm audit --omit=dev` صفر ثغرات وأغلق Dependabot التنبيهات الثلاثة تلقائيًا.
...
- نجحت تشغيلات CodeQL الثلاثة؛ لا توجد تنبيهات Code Scanning مفتوحة، كما لا توجد تنبيهات Dependabot مفتوحة بعد إغلاق الثلاثة تلقائيًا.
- أُغلق تنبيه Secret Scanning الوحيد بقرار `false_positive` وتعليق يوضح أن Firebase Web API Key معرّف عميل عام مقيّد بنطاقات HTTP المعتمدة وواجهات Firebase اللازمة.
- اكتمل Cloudflare Build للالتزام `1deb4f4` بنجاح، وأعاد `/api/app-version` الإصدار `0.3.53`.
- فُتحت صفحة `/admin/security` المنشورة وتأكد ظهور مركز الأمان والفحص الحي: 7/7 رؤوس أساسية، Turnstile فعّال، App Check مهيأ في Monitoring، ونقطة تقارير CSP متاحة.
- حالة GitHub النهائية: Security policy وPrivate vulnerability reporting وDependabot وCode scanning وSecret scanning كلها مفعّلة.

### جاهزية وربط Google AdSense - الإصدار 0.3.54

تم إنجازه:

- مراجعة حساب AdSense الحالي والتأكد أن `date-tool.com` مضاف مسبقًا إلى حساب الناشر الصحيح، وأن الملاحظة الحالية هي `محتوى غير ذي قيمة` بتاريخ 22 أغسطس 2026 وليست مشكلة ربط أو ملكية.
- مطابقة رقم الناشر المحفوظ في إعدادات الموقع مع حساب AdSense الحالي ومع السطر المنشور في `ads.txt`.
- التأكد أن رسالة Google المعتمدة للائحة الأوروبية منشورة للموقع وتدعم الإنجليزية و31 لغة إضافية، لذلك لم تُنشأ رسالة CMP مكررة.
- فحص صفحات العائلات والأدوات الفردية؛ لكل أداة فردية رابط مستقل وCanonical وH1 وMetadata وSchema ومحتوى إرشادي فريد وروابط داخلية، وليست صفحة فارغة أو تحويلًا إلى Hash.
- إضافة علامة إثبات الملكية الرسمية `<meta name="google-adsense-account">` إلى جميع الصفحات من رقم الناشر المنظم المحفوظ، مع رفض أي قيمة لا تطابق صيغة `ca-pub-*`.
- إبقاء سكربت AdSense خارج Layout العام؛ لا يُحمّل الإعلان إلا لوحدة مكتملة وبعد موافقة التسويق، ولا تظهر حاليًا طلبات أو مساحات إعلانية ناقصة قبل قبول الحساب وإضافة أرقام الوحدات.
- رفع إصدار التطبيق إلى `0.3.54` دون تغيير نسخة الإدارة أو بوابة المعلنين.

الأخطاء المكتشفة:

1. **طريقة إثبات AdSense المختارة لم تكن موجودة في HTML العام**
   - الأعراض: AdSense يعرض مقتطف السكربت كطريقة الإثبات المختارة، بينما الصفحات لا تحمّل السكربت لعدم وجود وحدات إعلانية مكتملة.
   - السبب: حجب سكربت الإعلان عمدًا حتى موافقة التسويق وعدم الحاجة إلى تحميله عالميًا قبل وجود وحدات صالحة.
   - الحل: استخدام علامة الحساب الوصفية الرسمية التي يعرضها AdSense، فهي تثبت الملكية دون تشغيل JavaScript أو إنشاء ملفات تعريف ارتباط قبل الموافقة.
   - الحالة: محلول ومنشور؛ اختيرت العلامة الوصفية داخل AdSense وأُرسل طلب المراجعة بنجاح.

2. **ملاحظة AdSense القديمة: محتوى غير ذي قيمة**
   - الأعراض: حالة الموقع `يتطلب عناية` وآخر تعديل في AdSense بتاريخ 22 أغسطس 2026.
   - السبب: المراجعة سبقت تحسينات المحتوى وSEO والمسارات الفردية التي نُشرت من 23 أغسطس حتى 1 سبتمبر.
   - الحل: التحقق من بقاء المحتوى الإرشادي الفريد والمسارات القابلة للفهرسة والصفحات القانونية والتنقل الواضح، ثم إعادة طلب المراجعة بعد نشر علامة الملكية.
   - الحالة: جاهز لإعادة الطلب بعد تحقق الإنتاج؛ قرار القبول النهائي يعود إلى Google ولا يمكن ضمانه.
...
npm start -- --hostname 127.0.0.1 --port 3000
Invoke-WebRequest http://127.0.0.1:3000/
Invoke-WebRequest http://127.0.0.1:3000/age-calculator
Invoke-WebRequest http://127.0.0.1:3000/en/age-calculator
Invoke-WebRequest http://127.0.0.1:3000/privacy
git diff --check
```

الحالة:

- نجح ESLint وبناء Next.js الكامل وتوليد 35 صفحة.
- نجحت 91 حالة اختبار في 23 ملفًا، بينها اختبار جديد يمنع تحميل AdSense عالميًا قبل الموافقة.
- أعادت الصفحات الأربع المفحوصة محليًا `200` مع Canonical صحيح وعلامة الحساب الوصفية، ومن دون سكربت `adsbygoogle`.
- ملف `ads.txt` المنشور يطابق الحساب الحالي، ورسالة الموافقة الأوروبية في AdSense منشورة ونشطة.
- اكتمل Cloudflare Build للالتزام `c24e9c6`، وأعاد `/api/app-version` الإصدار `0.3.54`، وأكد HTML الإنتاج وجود علامة حساب AdSense وعدم وجود سكربت الإعلان قبل الموافقة.
- تم تأكيد إصلاح المشكلة وإرسال طلب المراجعة من حساب AdSense؛ تغيّرت حالة `date-tool.com` إلى `يتم تجهيزه` وظهر تأكيد `تمّ طلب المراجعة`.
- لا تُضاف أرقام وحدات إعلانية أو تُفعّل Auto ads قبل قبول الموقع؛ المتبقي مراقبة نتيجة Google وإكمال معلومات الدفع في الحساب عند طلبها.

### إنشاء التقرير المرجعي الفارغ لاكتمال الموقع - دون تغيير إصدار

تم إنجازه:

- إنشاء `COMPREHENSIVE_AUDIT_CHECKLIST.md` كقائمة تدقيق مرجعية فارغة؛ لا تفترض إنجاز أي بند قبل التحقق منه بدليل.
- جرد نطاقات الموقع الحالية داخل القالب: 29 عنوانًا عامًا عربيًا وإنجليزيًا، صفحات الإدارة، بوابة العميل، مسارات API والنظام، ومجموعات ملفات المصدر والاختبارات.
- إضافة متطلبات تفصيلية للفكرة والمنتج، الوظيفة، التصميم، الوصول، الأداء، SEO، AdSense، الخصوصية، الأمن، PWA والتحديث متعدد المنصات، التشغيل، المراقبة، والنسخ والاستعادة.
- إضافة قوائم مستقلة لحسابات Cloudflare وFirebase/Google Cloud وGitHub وSearch Console وAdSense والبريد والتحليلات والخدمات الخارجية.
- اعتماد حالات موحدة للاستخدام في التدقيق اللاحق، وجدول نتائج يربط كل حكم بالدليل والتاريخ والمسؤول.
- مراجعة المتطلبات المتغيرة مقابل المراجع الرسمية الحالية لـNext.js وGoogle Search وCore Web Vitals وCloudflare Workers وFirebase وAdSense.

الأخطاء المكتشفة:

- لا توجد أخطاء تطبيقية مكتشفة في هذه المهمة؛ المطلوب كان إنشاء قالب فارغ، ولذلك بقيت جميع بنود التحقق دون تقييم.
...
```powershell
rg --files app tests .github public
rg --files -g "!node_modules/**" -g "!.next/**" -g "!.open-next/**" -g "!coverage/**"
git status --short
git diff --check
```
...
- يتكون التقرير من 23 قسمًا و519 بند تحقق فارغًا موزعة بحسب الصفحة والمسار والملف والحساب والمنصة.
- نجح `git diff --check`، ولم تُشغّل اختبارات التطبيق أو البناء لأن التغيير توثيقي فقط ولا يغير الشيفرة أو سلوك الإنتاج.
- لم يُنشر شيء ولم تتغير أرقام الإصدارات؛ الخطوة اللاحقة هي تنفيذ التدقيق الفعلي على دفعات وتعبئة الحالة والدليل لكل بند.

### توسيع التقرير بمنظومة الإعلانات والحسابات والصلاحيات - دون تغيير إصدار

تم إنجازه:

- إضافة قسم مستقل ومفصل إلى `COMPREHENSIVE_AUDIT_CHECKLIST.md` لمنظومة الإعلانات المباشرة وAdSense وإعلانات الموقع الداخلية وترتيب الأولوية بينها.
- تعريف قاموس مقترح وثابت لمسميات حسابات المنصة والمعلنين، مع فصل اسم الدور عن الصلاحيات الفعلية وترجمة مسمى العرض دون استخدام النص العربي كمفتاح منطقي.
- إضافة مصفوفة صلاحيات دقيقة للحسابات والإعدادات والأمان والمعلنين والحملات والمواضع والتقارير والفوترة وسجل التدقيق.
- إضافة دورة حياة كاملة للحملة بمفاتيح تقنية تبدأ بالمسودة والمراجعة والاعتماد والجدولة والتشغيل وتنتهي بالإيقاف أو الانتهاء والأرشفة.
- تغطية نموذج منظمة المعلن وعضويات الفريق، المواد الإعلانية، المواضع التسعة الحالية، الحجز والتعارض، المراجعة، الأسعار، الفواتير، القياس، مكافحة الاحتيال، الإشعارات، وبنية البيانات المقترحة.
- إضافة قائمة صفحات مطلوبة لاستكمال منصة المعلن والإدارة، واختبارات قبول تمنع تجاوز الملكية أو الصلاحيات وتثبت أولوية مصادر الإعلان.

الأخطاء المكتشفة:

- لا توجد أخطاء تطبيقية جديدة في هذه المهمة؛ جرى توثيق القيم الحالية المتعددة للأدوار والحالات العربية كعناصر يجب تقييمها وترحيلها فقط إذا اعتمد نموذج الصلاحيات المقترح لاحقًا.
...
```powershell
Get-Content app\adminRoles.js
Get-Content app\api\_lib\adminPermissions.js
Get-Content app\admin\ad-settings\page.jsx
Get-Content app\client\create-campaign\page.jsx
Get-Content firestore.rules
rg -n "role|status|campaign|advert|budget|price|payment|owner|userId|clientId|approved|rejected|active" app firestore.rules
git diff --check
```

الحالة:

- يتكون التقرير الموسع من 24 قسمًا و766 بند تحقق فارغًا؛ أضيفت إليه ثلاثة أسطر مستقلة لنتائج منصة الإعلانات والحسابات والفوترة في جدول التدقيق اللاحق.
- بقيت كل البنود بلا تقييم كما طلب المستخدم، ولم تُغير هذه المهمة الأدوار أو قواعد Firestore أو واجهات الإعلانات الحالية.
- لم تُشغّل اختبارات التطبيق أو البناء لأن التغيير توثيقي فقط، ولم يُنشر شيء أو يتغير رقم الإصدار.

### توسيع التقرير ببنود ملف التصميم المرجعي - دون تغيير إصدار

تم إنجازه:

- إضافة قسم مستقل لملف التصميم المرجعي داخل `COMPREHENSIVE_AUDIT_CHECKLIST.md` مع توضيح أنه متطلب مفتوح لعدم وجود ملف Figma أو Sketch أو Penpot داخل المستودع حاليًا.
- تعريف هيكل مقترح لصفحات ملف التصميم يبدأ بالغلاف والأسس والمكونات، ويغطي الشاشات العامة والإدارة ومنصة المعلن والإعلانات وPWA والرحلات والتسليم والأرشيف.
- إضافة متطلبات ملكية الملف والإصدارات والصلاحيات، وDesign Tokens، والشبكات والاستجابة، وRTL/LTR، ومكتبة المكونات وكل حالات الواجهة.
- جرد الشاشات المطلوب تمثيلها في التصميم للصفحات العامة والإدارة ومنصة الإعلانات، مع متطلبات الأصول والأيقونات والنماذج التفاعلية والوصول.
- إضافة شروط تسليم التصميم للتطوير، وربط الإطارات والمكونات والمتغيرات بمسارات Next.js وReact وCSS، ومراجعة المطابقة مع الإنتاج وحوكمة الملف وصيانته.
- إضافة صف مستقل لملف التصميم ومطابقته مع الكود في جدول نتائج التدقيق اللاحق.

الأخطاء المكتشفة:

- لا يوجد ملف تصميم أصلي معروف داخل المستودع بالامتدادات أو التسميات المعتادة؛ لم يُعتبر ذلك خللًا مؤكدًا لأن الملف قد يكون محفوظًا في منصة تصميم خارجية لم تُربط بالمشروع بعد.
...
```powershell
rg --files -g "*.fig" -g "*.sketch" -g "*.xd" -g "*.penpot" -g "*.tokens.json" -g "*design*" -g "*style*"
git status --short
git diff --check
```
...
- أصبح التقرير يحتوي 24 قسمًا رئيسيًا و966 بند تحقق فارغًا، منها 200 بند جديد تخص ملف التصميم المرجعي.
- بقيت جميع بنود التصميم دون تقييم، ولا يعني عدم العثور على ملف داخل المستودع أنه غير موجود في حساب خارجي.
- التغيير توثيقي فقط؛ لم تتغير الواجهة أو الشيفرة التشغيلية أو أرقام الإصدارات ولم يُنشر شيء.

### تعبئة حالات التقرير المرجعي بالتدقيق الأولي - دون تغيير إصدار

تم إنجازه:

- تحويل `COMPREHENSIVE_AUDIT_CHECKLIST.md` من قالب فارغ إلى تدقيق أولي مؤرخ في 2026-09-06، وتصنيف جميع بنوده البالغ عددها 1052 بندًا دون إبقاء أي بند غير مفحوص.
- إضافة ملخص رقمي للحالات: 142 منجزًا مثبتًا، و350 منجزًا جزئيًا، و259 فجوة أو متطلبًا غير منفذ، و101 بندًا يحتاج إثبات منصة/جهاز خارجي، و200 بند غير منطبق.
- توضيح أن بنود ملف Figma/Penpot/Sketch غير منطبقة لأن ملفات CSS في المستودع هي مرجع التصميم المعتمد حاليًا، مع إبقاء بنود نظام CSS والاستجابة والوصول ضمن التقييم.
- تعبئة بوابات الأولوية P0-P3 وسجل نتائج التدقيق لكل نطاق: الصفحات، الإدارة، بوابة العميل، API، الأمن، PWA، التصميم، الأداء، SEO، AdSense، الإعلانات، الحسابات، Cloudflare، Firebase، GitHub، النسخ، والتشغيل.
- فحص عينة ممثلة من مسارات الإنتاج العربية والإنجليزية وملفات النظام العامة، والتأكد من حالات HTTP وCanonical وH1 وملفات Sitemap وrobots وads.txt وManifest ونسخة التطبيق.
- مراجعة تنفيذ PWA ورؤوس الأمان وCodeQL وSECURITY.md وملفات CSS والوصول، مع توثيق حدود الأدلة الخارجية بدل افتراض نجاحها.

الأخطاء المكتشفة:

1. **سياسة CSP ليست مفروضة بعد**
   - الأعراض: الإنتاج يرسل `Content-Security-Policy-Report-Only` دون رأس CSP تنفيذي.
   - السبب: بقيت السياسة في مرحلة جمع التقارير ولم تنتقل إلى Enforcement.
   - الحل: مراجعة تقارير الانتهاكات وإغلاق المصادر اللازمة ثم فرض سياسة تدريجية دون تعطيل التكاملات.
   - الحالة: متبقٍ ومصنف P1 في التقرير.

2. **لغة واتجاه HTML الأولي للمسارات الإنجليزية غير صحيحين**
   - الأعراض: HTML الخام يبدأ بـ`lang="ar" dir="rtl"` ثم يصححه JavaScript إلى `en/ltr` بعد التشغيل.
   - السبب: `app/layout.jsx` يثبت اللغة والاتجاه العربيين في جذر المستند المشترك.
   - الحل: جعل اللغة والاتجاه صحيحين من الخادم لكل شجرة مسارات مع الحفاظ على Metadata والترجمة الحالية.
   - الحالة: العرض النهائي صحيح، والإصلاح البنيوي متبقٍ ومصنف P1.

3. **دورة تحديث PWA والعمل دون اتصال غير مكتملة**
   - الأعراض: فحص النسخة وتعليمات التثبيت موجودان، لكن لا يوجد Service Worker مستقل أو استراتيجية Offline/Cache upgrade كاملة.
   - السبب: آلية التحديث الحالية تعتمد مقارنة `/api/app-version` وإعادة تحميل الصفحة.
   - الحل: إما توثيق أن التطبيق Online-only وإزالة ادعاءات Offline، أو إضافة Service Worker ودورة تفعيل وترقية كاش واختبارات أجهزة حقيقية.
   - الحالة: متبقٍ ومصنف P1.

4. **النسخ الاحتياطي والاستعادة غير مثبتين عمليًا**
   - الأعراض: لا يتوفر في الفحص الحالي دليل تشغيل Scheduled Backups/PITR أو تجربة استعادة Firestore وR2.
   - السبب: هذه إعدادات حسابات خارجية ومدفوعة، وتعذر التحقق منها من الشيفرة وحدها.
   - الحل: تفعيل السياسة المناسبة وتوثيق RPO/RTO وتنفيذ استعادة تجريبية إلى مورد جديد.
   - الحالة: خارجي `[E]` حتى يقدم دليل المنصة.

5. **الفحوص الخارجية والحقلية غير مكتملة حاليًا**
   - الأعراض: تعذر إعادة التحقق من Cloudflare وFirebase وGitHub وSearch Console وAdSense والأجهزة الحقيقية، كما لم يكتمل `npm audit` الحالي بسبب اتصال السجل المقيد.
   - السبب: انتهاء مصادقة Wrangler وعدم توفر `gh` وقيود الشبكة والحاجة إلى أجهزة/حسابات حقيقية.
   - الحل: إعادة المصادقة وتنفيذ البنود `[E]` من الحسابات والأجهزة وتسجيل رابط أو لقطة وتاريخ لكل نتيجة.
   - الحالة: متبقٍ خارجيًا؛ آخر سجل سابق في المذكرة يذكر صفر ثغرات npm وتنبيهات GitHub نظيفة، لكنه لا يعد إثباتًا جديدًا.
...
npm run build
npm audit --omit=dev --json
Invoke-WebRequest https://date-tool.com/
Invoke-WebRequest https://date-tool.com/en
Invoke-WebRequest https://date-tool.com/clock
Invoke-WebRequest https://date-tool.com/en/clock
Invoke-WebRequest https://date-tool.com/weather
Invoke-WebRequest https://date-tool.com/en/weather
Invoke-WebRequest https://date-tool.com/age-calculator
Invoke-WebRequest https://date-tool.com/en/age-calculator
Invoke-WebRequest https://date-tool.com/date-converter
Invoke-WebRequest https://date-tool.com/time-converter
Invoke-WebRequest https://date-tool.com/weather-search
Invoke-WebRequest https://date-tool.com/privacy
Invoke-WebRequest https://date-tool.com/terms
Invoke-WebRequest https://date-tool.com/contact
Invoke-WebRequest https://date-tool.com/about-us
Invoke-WebRequest https://date-tool.com/sitemap.xml
Invoke-WebRequest https://date-tool.com/robots.txt
Invoke-WebRequest https://date-tool.com/ads.txt
Invoke-WebRequest https://date-tool.com/manifest.webmanifest
Invoke-WebRequest https://date-tool.com/api/app-version
git diff --check
```
...
- نجح ESLint، ونجحت 91 حالة اختبار في 23 ملفًا، ونجح بناء Next.js الكامل مع توليد 35 صفحة.
- نجحت المسارات الممثلة في الإنتاج، وأعاد المسار غير الموجود `404` وأعاد `/index.html` تحويلًا دائمًا `308`، وكانت ملفات النظام العامة متاحة.
- لم يكتمل `npm audit --omit=dev` في هذه الجولة بسبب منع اتصال npm registry في بيئة التنفيذ؛ لم يُستخدم `npm audit fix --force`.
- التقرير الآن نتيجة تدقيق أولية لا ادعاء اكتمال نهائي؛ البنود `[!]` تشمل فجوات فعلية ومتطلبات مستقبلية، والبنود `[E]` تنتظر دليلًا مباشرًا من المنصة أو الجهاز.
- التغيير توثيقي فقط؛ لم تتغير الشيفرة التشغيلية أو أرقام الإصدارات ولم يُنشر شيء.

### استكمال البنود المحلية وبوابة المعلنين التجريبية - دون تغيير إصدار

تم إنجازه:

- إنشاء طبقة صلاحيات مستقلة للمعلنين بثلاثة أدوار: مالك المؤسسة، مدير الحملات، ومحلل التقارير، مع صلاحيات منفصلة للملف والفريق وإنشاء الحملات وتعديلها وقراءة التقارير.
- إضافة وضع تجريبي محلي لبوابة المعلنين يعمل فقط في بيئة التطوير على `localhost` أو `127.0.0.1` ويخزن الحسابات والجلسات والحملات في `localStorage` دون الاتصال بـFirebase أو كتابة بيانات في الإنتاج.
- إنشاء حساب معلن محلي جاهز للتجربة بالبريد `demo.advertiser@local.test` وكلمة المرور `LocalDemo2026!`، مع حملة نموذجية، وإتاحة تسجيل حسابات محلية مستقلة لكل مؤسسة.
- ربط صفحات تسجيل الدخول والتسجيل ولوحة المعلن وإنشاء الحملة بالوضع المحلي، مع إبقاء تدفق Firebase وTurnstile الحالي للإنتاج دون تغيير.
- تطبيق الصلاحيات على التنقل وإنشاء الحملات وتغيير حالتها، وإظهار الدور وحالة الوضع المحلي أو القراءة فقط بوضوح داخل بوابة المعلن.
- إضافة صفحة إدارة موحدة في `/admin/advertisers` لعرض حسابات المعلنين والبحث فيها وتعديل الدور والحالة، وإضافتها إلى تنقل الإدارة والاختصارات بحسب صلاحية إدارة العملاء.
- تقوية قواعد Firestore محليًا لدعم المؤسسة والدور والصلاحيات، ومنع انتقال الحملة بين المؤسسات أو تغيير مالكها، وتقييد تحديث حساب المعلن إداريًا على الدور والحالة فقط.
- نقل سياسة CSP من المراقبة وحدها إلى الإرسال التنفيذي أيضًا في النسخة المحلية، مع إبقاء رأس `Report-Only` لجمع التقارير قبل النشر.
- إضافة جذر محتوى إنجليزي خادمي لمسارات `/en` يحمل `lang="en" dir="ltr"` دون تحويل التطبيق كله إلى إخراج ديناميكي.
- تحديث تقرير التدقيق الشامل بحالة بوابة المعلنين والعزل المحلي ونتائج الاختبارات الحالية.

الأخطاء المكتشفة:

1. **بوابة المعلن المحلية كانت تعتمد سابقًا على خدمات الإنتاج**
   - الأعراض: تجربة التسجيل أو إدارة الحملات محليًا قد تتصل بمشروع Firebase الحقيقي بحسب متغيرات البيئة.
   - السبب: لم تكن هناك طبقة بيانات محلية مخصصة لبوابة المعلنين.
   - الحل: إضافة مخزن تجريبي محلي محكوم ببيئة التطوير واسم المضيف، مع إبقاء مسار الإنتاج منفصلًا.
   - الحالة: محلول لبوابة المعلنين؛ عزل جميع صفحات الإدارة والخدمات الأخرى ما زال بندًا أوسع مستقلًا.

2. **صلاحيات المعلنين كانت تعتمد ملكية المستخدم فقط**
   - الأعراض: لا يوجد تمثيل واضح لمالك المؤسسة ومدير الحملات ومحلل التقارير أو مشاركة منظمة للحملات.
   - السبب: ملف المعلن والحملة كانا يعتمدان `uid` و`advertiserId` دون نموذج أدوار ومؤسسة.
   - الحل: إضافة `organizationId` و`role` وخريطة صلاحيات متوافقة مع الحسابات القديمة، وتطبيقها في الواجهة وقواعد Firestore.
   - الحالة: محلول في الشيفرة والقواعد المحلية، ولم تُنشر القواعد بعد.

3. **CSP كانت في وضع التقارير فقط**
   - الأعراض: انتهاك السياسة يُسجل دون أن يمنع المورد المخالف.
   - السبب: إرسال `Content-Security-Policy-Report-Only` فقط.
   - الحل: إرسال `Content-Security-Policy` التنفيذي مع سياسة المراقبة نفسها واختباره محليًا.
   - الحالة: محلول محليًا وينتظر مراجعة تقارير الإنتاج قبل النشر.

4. **جذر HTML العام يبقى عربيًا لمسارات الإنجليزية**
   - الأعراض: وسم `<html>` الخام ما زال `lang="ar" dir="rtl"` رغم أن محتوى `/en` إنجليزي.
   - السبب: تخطيط Next.js الجذري مشترك وثابت لتجنب جعل جميع الصفحات ديناميكية.
   - الحل: إضافة جذر إنجليزي خادمي داخل شجرة `/en`؛ تغيير وسم `<html>` نفسه يحتاج قرارًا معماريًا أوسع واختبار SEO كاملًا.
   - الحالة: معالجة جزئية محسنة، والبند البنيوي ما زال مفتوحًا.

5. **تعذر الاختبار البصري الآلي داخل نافذة المتصفح الحالية**
   - الأعراض: أداة التحكم لم تستطع إثبات عنوان نافذة المتصفح بثقة.
   - السبب: حالة النافذة الخارجية لا تسمح بتحديد الهدف بأمان.
   - الحل: التحقق عبر اختبارات الوحدة والبناء وطلبات HTTP المحلية، وترك خادم المعاينة مفتوحًا للفحص اليدوي.
   - الحالة: التحقق البرمجي ناجح، والمراجعة البصرية اليدوية متاحة محليًا.

الملفات المتأثرة:

- `app/advertiserAccess.js`
- `app/client/localAdvertiserDemo.js`
- `app/client/page.jsx`
- `app/client/register/page.jsx`
- `app/client/ClientShell.jsx`
- `app/client/dashboard/page.jsx`
...
- `middleware.js`
- `firestore.rules`
- `tests/advertiserAccess.test.js`
- `tests/localAdvertiserDemo.test.js`
- `tests/securityPolicies.test.js`
- `tests/securityBoundaries.test.js`
- `COMPREHENSIVE_AUDIT_CHECKLIST.md`
- `PROJECT_MEMO.md`
...
Invoke-WebRequest http://127.0.0.1:3000/client
Invoke-WebRequest http://127.0.0.1:3000/client/register
Invoke-WebRequest http://127.0.0.1:3000/client/dashboard
Invoke-WebRequest http://127.0.0.1:3000/client/create-campaign
Invoke-WebRequest http://127.0.0.1:3000/admin/advertisers
Invoke-WebRequest http://127.0.0.1:3000/en
git diff --check
```
...
- نجح ESLint، ونجحت 103 حالات اختبار في 25 ملفًا، ونجح بناء Next.js الكامل مع توليد 36 صفحة.
- استجابت مسارات بوابة المعلنين وصفحة إدارة المعلنين محليًا بالحالة `200`، وتحمل المسارات الخاصة رأس `X-Robots-Tag` لمنع الفهرسة، وتأكد وجود رأس CSP التنفيذي.
- خادم المعاينة المحلي متاح على `http://127.0.0.1:3000`، والحساب التجريبي لا يمنح أي وصول إلى Firebase أو الإنتاج.
- لم يُنشر التطبيق، ولم تُنشر قواعد Firestore، ولم يُرفع إصدار جديد، ولم تُنشأ حسابات حقيقية أو تُعدل حسابات المنصات الخارجية.
- المرحلة التالية تخص إثباتات وإعدادات المنصات الخارجية، ونشر القواعد بعد مراجعتها، وحسابات الفريق الحقيقية والدعوات والفوترة وسجل التدقيق، وتبقى منفصلة حتى يطلب المستخدم البدء بها.

### نقل إعدادات الخصوصية إلى صفحة إعدادات الأداة - دون تغيير إصدار

تم إنجازه:

- نقل قسم «الخصوصية وموافقة الزائر» كاملًا من `/admin/security` إلى `/admin/tools` ليعود ضمن إعدادات الأداة المرتبطة بسلوك الواجهة العامة.
- إزالة اختيار لغة أسماء الصفحات من قسم الخصوصية؛ أسماء الإدارة عربية والمسارات وإعدادات الظهور مشتركة بين نسختي الموقع العربية والإنجليزية.
- استبدال شبكة الصفحات المفتوحة بقائمة منسدلة مضغوطة تدعم اختيار عدة صفحات، وتعرض عدد الصفحات المحددة وتبقي اسم الصفحة ومسارها ظاهرين داخل القائمة.
- إبقاء بنية الحفظ نفسها `privacySettingsButton` دون ترحيل بيانات أو تغيير عقد قاعدة البيانات، ودمج الحفظ مع تدفق «حفظ إعدادات الأداة» الحالي.
- استخدام مكوّن التشغيل والإيقاف الموحد `AdminEnableToggle` لزر ظهور إعدادات الخصوصية.
- إضافة اختصار «الخصوصية» إلى شبكة التنقل السريع في صفحة إعدادات الأداة.
- تحديث اختبار حدود صفحات الإدارة ليثبت أن إعداد الخصوصية موجود في صفحة الأداة وغير موجود في صفحة الأمن.

الأخطاء المكتشفة:

1. **إعداد واجهة عامة كان موضوعًا داخل صفحة الأمن**
   - الأعراض: التحكم في ظهور زر إعدادات الخصوصية والصفحات المختارة كان داخل مركز تقارير وحماية المنصة.
   - السبب: نُقل القسم سابقًا عند تجميع العناصر الأمنية رغم أن وظيفته إعداد سلوك واجهة الأداة.
   - الحل: إعادة القسم إلى إعدادات الأداة وترك صفحة الأمن للتقارير والفحوص والصيانة الأمنية.
   - الحالة: محلول محليًا.

2. **قائمة الصفحات كانت تستهلك مساحة عمودية كبيرة**
   - الأعراض: جميع الصفحات تظهر دائمًا في شبكة طويلة، خاصة على شاشة الجوال.
   - السبب: استخدام قائمة مربعات اختيار مفتوحة بلا حاوية قابلة للطي.
   - الحل: قائمة منسدلة متعددة الاختيار بارتفاع داخلي محدود وتمرير مستقل عند فتحها.
   - الحالة: محلول محليًا.

3. **خدمات الوسائط والحملات الخارجية لا تعمل بالكامل داخل Next dev المباشر**
   - الأعراض: ظهرت استجابات `404` لبعض صور R2 و`503` للحملات والإحصاءات أثناء المعاينة المحلية.
   - السبب: خادم Next المحلي لا يملك جميع روابط وBindings بيئة Cloudflare Worker المنشورة.
   - الحل: لا يخص نقل إعداد الخصوصية؛ يُختبر لاحقًا عبر OpenNext preview أو تُربط خدمات تطوير منفصلة عند الحاجة.
   - الحالة: متبقٍ كقيد معروف للمعاينة المحلية، ولم يؤثر في حفظ أو عرض قسم الخصوصية.
...
npm run build
npm run dev -- --hostname 127.0.0.1
Invoke-WebRequest http://127.0.0.1:3000/admin/tools
Invoke-WebRequest http://127.0.0.1:3000/admin/security
git diff --check
```
...
- نجح ESLint، ونجحت 103 حالات اختبار في 25 ملفًا، ونجح بناء Next.js الكامل مع توليد 36 صفحة.
- استجابت صفحتا `/admin/tools` و`/admin/security` محليًا بالحالة `200` مع CSP ومنع فهرسة صفحات الإدارة.
- خادم المعاينة أعيد تشغيله على `http://127.0.0.1:3000`، والقسم الجديد متاح مباشرة عبر `/admin/tools#privacy-settings-button`.
- لم يُنشر التطبيق أو القواعد ولم يتغير رقم الإصدار أو إعدادات الإنتاج.

### إضافة صفحة الحسابات لإدارة المعلنين - دون تغيير إصدار

تم إنجازه:

- إضافة الوجهة الإدارية الرسمية `/admin/accounts` باسم «الحسابات» لعرض حسابات المعلنين وإدارتها.
- تحديث القائمة الجانبية العامة والاختصار في لوحة الإدارة، وإضافة رابط الحسابات إلى القوائم الداخلية القديمة في صفحات إعدادات الأداة والحملات والربط الخارجي وPageSpeed.
- إبقاء المسار السابق `/admin/advertisers` متوافقًا كوجهة قديمة للصفحة نفسها، مع اعتباره ضمن صلاحية «الحسابات» حتى لا تنكسر الروابط المحفوظة.
- عرض ملخص عددي لكل الحسابات والنشطة والمنتظرة والمعلقة أو المغلقة، مع البحث باسم المتجر أو المسؤول أو البريد أو المعرّف.
- دعم تعديل دور المعلن بين مالك الحساب ومدير الحملات ومحلل التقارير، وتعديل حالته بين انتظار البريد والنشط والمعلق والمغلق.
- ربط الصفحة بالحسابات التجريبية المحلية عند العمل على `localhost` أو `127.0.0.1`، ومنعها من الاتصال بـFirebase في هذا الوضع.
- إضافة دوال محلية لعرض حسابات المعلنين وتحديث الدور والحالة فقط، مع التحقق من القيم المسموحة وعدم إعادة بيانات كلمات المرور.
- توحيد صلاحية `accounts` الجديدة مع مفاتيح الصلاحيات القديمة في واجهة الإدارة وقواعد Firestore المحلية.
- إضافة تنبيه واضح داخل الصفحة عندما يكون مصدر الحسابات محليًا، يوضح أن التعديلات محفوظة داخل المتصفح فقط.

الأخطاء المكتشفة:

1. **صفحة حسابات المعلنين لم تكن تحمل الاسم والمسار المطلوبين**
   - الأعراض: الصفحة موجودة باسم «حسابات المعلنين» على المسار التقني `/admin/advertisers` بدل صفحة «الحسابات» العامة.
   - السبب: أُنشئت أولًا كنطاق خاص بالمعلنين دون اعتماد اسم نهائي في قائمة الإدارة.
   - الحل: اعتماد `/admin/accounts` واسم «الحسابات» مع إبقاء المسار القديم متوافقًا.
   - الحالة: محلول محليًا.

2. **حسابات المعلنين التجريبية لا تظهر في الإدارة المحلية**
   - الأعراض: بوابة المعلن المحلية تستخدم `localStorage` بينما صفحة الإدارة كانت تقرأ مجموعة Firebase فقط.
   - السبب: لم تكن طبقة الحسابات المحلية مكشوفة لصفحة الإدارة.
   - الحل: إضافة قائمة وتحديث محليين محكومين ببيئة التطوير، واستخدام Firebase خارجها فقط.
   - الحالة: محلول محليًا.

3. **اسم صلاحية الصفحة الجديدة لم يكن موجودًا في قواعد Firestore**
   - الأعراض: قد يرى المدير المساعد رابط «الحسابات» بصلاحية `accounts` ثم يُرفض طلب البيانات.
   - السبب: قواعد Firestore كانت تقبل أسماء الصلاحيات القديمة فقط: `client` و`advertisers` و`clients`.
   - الحل: إضافة `accounts` إلى بدائل الصلاحية مع الحفاظ على التوافق مع جميع الأسماء القديمة.
   - الحالة: محلول في القواعد المحلية، ولم تُنشر القواعد بعد.

الملفات المتأثرة:

- `app/admin/accounts/page.jsx`
- `app/admin/advertisers/page.jsx`
- `app/admin/AdminShell.jsx`
- `app/admin/page.jsx`
- `app/admin/tools/page.jsx`
...
npm run build
npm run dev -- --hostname 127.0.0.1
Invoke-WebRequest http://127.0.0.1:3000/admin/accounts
curl.exe -I http://127.0.0.1:3000/admin/advertisers
git diff --check
```

الحالة:

- نجح ESLint، ونجحت 104 حالات اختبار في 25 ملفًا، ونجح بناء Next.js الكامل مع توليد 37 صفحة.
- صفحة الحسابات محمية بتخطيط الإدارة وصلاحياته وتحمل رؤوس CSP و`X-Robots-Tag: noindex, nofollow, noarchive`.
- خادم المعاينة المحلي متاح على `http://127.0.0.1:3000/admin/accounts`.
- لم يُنشر التطبيق أو قواعد Firestore، ولم يتغير رقم الإصدار أو أي إعداد إنتاجي.

### توحيد إجراءات جدول الحسابات - دون تغيير إصدار

تم إنجازه:

- تحويل صفوف جدول الحسابات إلى وضع عرض افتراضي بدل إظهار حقول الدور والحالة القابلة للتعديل طوال الوقت.
- إضافة زر تعديل بأيقونة القلم داخل عمود «الإجراء» لكل حساب.
- إظهار حقلي الدور والحالة فقط بعد بدء تعديل الصف المحدد.
- تجميع زري الحفظ والإلغاء داخل عمود «الإجراء» باستخدام حاوية وأحجام وألوان `tools-item-actions` الموحدة في جداول الإدارة.
- إعادة قيم الدور والحالة المحفوظة عند إلغاء التعديل، ومنع فتح صف جديد مع إبقاء مسودة غير محفوظة في الصف السابق.
- تعطيل زر الحفظ إلى أن تتغير قيمة فعلية، وتعطيل الإلغاء أثناء تنفيذ الحفظ لمنع تضارب الحالة.
- تحديث اختبار حدود الإدارة ليثبت وجود أزرار التعديل والحفظ والإلغاء داخل حاوية الإجراءات الموحدة.

الأخطاء المكتشفة:

1. **حقول تعديل الحساب كانت ظاهرة دائمًا خارج نمط إجراءات الجداول**
   - الأعراض: عمودا الدور والحالة يعرضان قوائم تعديل مباشرة، بينما يحتوي عمود الإجراء على زر حفظ نصي مختلف عن بقية جداول المنصة.
   - السبب: نُفذت النسخة الأولى كنموذج تحرير مباشر لكل صف.
   - الحل: إضافة وضع عرض وتعديل، ونقل أوامر التعديل والحفظ والإلغاء إلى عمود الإجراء بأزرار الأيقونات الموحدة.
   - الحالة: محلول محليًا.
...
- نجح ESLint، ونجحت 104 حالات اختبار في 25 ملفًا، ونجح بناء Next.js الكامل مع توليد 37 صفحة.
- أعيد تشغيل المعاينة المحلية على `http://127.0.0.1:3000/admin/accounts`.
- لم يُنشر التطبيق أو القواعد ولم تتغير بيانات الإنتاج أو نسخة التطبيق.

### تحويل تعديل الحساب إلى نافذة منبثقة - دون تغيير إصدار

تم إنجازه:

- إبقاء صفوف جدول الحسابات في وضع العرض فقط دون فتح حقول تحرير داخل الجدول.
- جعل زر القلم في عمود «الإجراء» يفتح نافذة منبثقة موحدة لتعديل حساب المعلن.
- عرض اسم المتجر واسم المسؤول والبريد داخل النافذة لتأكيد الحساب الذي يجري تعديله.
- نقل حقلي دور الحساب وحالته إلى النافذة المنبثقة مع زري «حفظ التعديل» و«إلغاء».
- دعم إغلاق النافذة بزر الإغلاق أو مفتاح `Escape` أو النقر خارجها، مع منع الإغلاق أثناء تنفيذ الحفظ.
- إبقاء النافذة مفتوحة عند فشل الحفظ حتى لا يفقد المدير اختياراته ويتمكن من المحاولة مجددًا.
- جعل زر الحفظ معطلًا حتى يحدث تغيير فعلي في الدور أو الحالة.
- إضافة اختبار ثابت يثبت وجود نافذة التعديل وربطها الدلالي بعنوانها وزر الإجراء.

الأخطاء المكتشفة:

1. **تعديل الدور والحالة كان يغيّر بنية صف الجدول**
   - الأعراض: الضغط على زر التعديل يحول خلايا الدور والحالة إلى حقول، ما يغيّر عرض الأعمدة ويزاحم محتوى الجدول على الشاشات الصغيرة.
   - السبب: نموذج التحرير كان مضمّنًا داخل الصف نفسه.
   - الحل: نقل نموذج التحرير إلى نافذة منبثقة مستقلة والإبقاء على الجدول للعرض والإجراءات فقط.
   - الحالة: محلول محليًا.
...
- نجح ESLint، ونجحت 104 حالات اختبار في 25 ملفًا، وجُمعت صفحة الحسابات محليًا واستجابت بالحالة `200`.
- بقي خادم المعاينة متاحًا على `http://127.0.0.1:3000/admin/accounts`.
- لم يُنشر التطبيق أو القواعد، ولم تتغير بيانات الإنتاج أو رقم الإصدار.

### إضافة الحسابات وربط أعضاء المنظمات برقم موحد - دون تغيير إصدار

تم إنجازه:

- إضافة زر «إضافة حساب معلن» ونافذة منبثقة إلى `/admin/accounts`.
- دعم إنشاء منظمة جديدة بمالك ورقم منظمة موحد من 12 رقمًا، أو ربط عضو بمنظمة موجودة بواسطة رقمها.
- إبقاء ربط العضو بالمنظمة عملية إدارية؛ رقم المنظمة معرّف للربط وليس كلمة مرور أو صلاحية دخول.
- إضافة رقمي `organizationId` و`organizationNumber` إلى ملف المعلن مع توليد رقم ثابت للحسابات القديمة عند عرضه.
- عرض رقم المنظمة بصيغة مقروءة في جدول الحسابات ولوحة المعلن مع زر نسخه.
- توسيع الحسابات المحلية التجريبية لإنشاء المالك والأعضاء واختبار اشتراكهم في حملات المنظمة نفسها.
- تجهيز إنشاء حساب Firebase بواسطة تطبيق ثانوي حتى لا تستبدل جلسة المعلن الجديدة جلسة المدير، مع إرسال توثيق البريد ورابط تعيين كلمة المرور.
- تحديث قواعد Firestore المحلية للتحقق من رقم المنظمة والسماح للمدير المخول بإنشاء الحساب وربطه أو نقله بين المنظمات.

الأخطاء المكتشفة:

1. **الحسابات لم تكن تملك معرّف منظمة قابلًا للمشاركة**
   - الأعراض: الاعتماد على `organizationId` الداخلي الطويل عند الحاجة إلى ربط عضو بجهة قائمة.
   - السبب: نموذج البيانات الأولي أنشأ منظمة للحساب لكنه لم ينشئ رقمًا إداريًا مقروءًا.
   - الحل: إنشاء `organizationNumber` ثابت من 12 رقمًا، عرضه للمالك، واستخدامه في الربط الإداري فقط.
   - الحالة: محلول محليًا.

2. **إنشاء مستخدم من جلسة Firebase الأساسية يخرج المدير**
   - الأعراض: استخدام `createUserWithEmailAndPassword` على تطبيق Firebase الأساسي يستبدل المستخدم الحالي بالمستخدم الجديد.
   - السبب: Firebase Auth يحتفظ بجلسة واحدة لكل تطبيق Firebase في المتصفح.
   - الحل: إنشاء تطبيق Firebase ثانوي مؤقت للمستخدم الجديد ثم حذفه بعد حفظ ملف الحساب وإرسال رسائل البريد.
   - الحالة: محلول في الشيفرة المحلية، ولم يُختبر على Firebase المنشور أو تُنشر القواعد.
...
- نجحت اختبارات الرقم الموحد وإنشاء المالك وربط العضو، ونجح ESLint وبناء Next.js الكامل.
- التغييرات محلية ولم تُنشر قواعد Firestore أو التطبيق، ولم تتغير بيانات الإنتاج أو أرقام الإصدارات.

### عزل أنماط الموقع والإدارة ومنصة المعلنين - دون تغيير إصدار

تم إنجازه:

- نقل استيراد `AdminDashboard.css` إلى `app/admin/layout.jsx` مرة واحدة وحذف الاستيرادات المكررة من صفحات الإدارة.
- نقل استيراد `ClientPortal.css` إلى `app/client/layout.jsx` مرة واحدة وحذف الاستيرادات المكررة من صفحات المعلنين.
- نقل استيراد `AdminLogin.css` إلى `app/admin_login/layout.jsx` وحصر جميع محدداته داخل `.login-page-wrapper`.
- إبقاء ملف دخول الإدارة مستقلًا عن `AdminDashboard.css` لأن صفحة الدخول لا تستخدم غلاف الإدارة، ولمنع تحميل ملف الإدارة الكبير قبل المصادقة.
- تحويل `body` إلى قاعدة تخطيط محايدة بلا حشوة، ونقل حشوة وخلفية الموقع العام إلى الغلاف `.public-site-root`.
- منع `SiteShell` من تطبيق لغة وثيم وأيقونات الموقع العام على مسارات الإدارة والمعلنين والدعم.
- حذف `app/admin/AdminPage.css` بعد التأكد أنه غير مستورد من أي ملف تشغيل، وتحديث قائمة التدقيق لإزالة مرجعه.
- إضافة اختبار معماري يمنع عودة استيرادات CSS المتكررة أو ملف الإدارة القديم أو حشوة الموقع العام إلى `body`.

الأخطاء المكتشفة:

1. **قواعد تخطيط الموقع العام كانت تُطبق على جميع المسارات**
   - الأعراض: حشوة `body` وثيم الموقع العام يمكن أن يؤثرا في حواف الإدارة ومنصة المعلنين رغم امتلاكهما أغلفة مستقلة.
   - السبب: `globals.css` محمل من Root Layout وكان `body` يحمل تخطيط الصفحة العامة نفسه.
   - الحل: إبقاء `body` محايدًا، وإسناد تخطيط الموقع العام إلى `.public-site-root`، وإبقاء قواعد كل منصة داخل غلافها وتخطيط مسارها.
   - الحالة: محلول محليًا.

2. **ملف إدارة قديم بلا استخدام فعلي**
   - الأعراض: وجود `AdminPage.css` إلى جانب ملف الإدارة الفعلي يوحي بوجود مصدرين للتصميم.
   - السبب: بقي الملف من تصميم إداري سابق بعد الانتقال إلى `AdminDashboard.css`.
   - الحل: التأكد عبر البحث الشامل من عدم وجود import ثم حذف الملف وتحديث التوثيق الجاري.
   - الحالة: محلول محليًا، مع إبقاء ذكره التاريخي في سجل الإصدارات فقط.
...
- `app/admin/AdminShell.jsx`
- `app/admin/AdminDashboard.css`
- `app/admin/AdminPage.css` (محذوف)
- صفحات الإدارة التي أزيل منها استيراد CSS المكرر
- `app/admin_login/layout.jsx`
- `app/admin_login/page.jsx`
- `app/admin_login/AdminLogin.css`
- `app/client/layout.jsx`
- صفحات المعلنين التي أزيل منها استيراد CSS المكرر
- `tests/securityBoundaries.test.js`
- `COMPREHENSIVE_AUDIT_CHECKLIST.md`
- `PROJECT_MEMO.md`
...
```powershell
rg -n "AdminPage\.css|AdminLogin\.css|AdminDashboard\.css|ClientPortal\.css" app
npm run lint
npm test
npm run build
npm run dev -- --hostname 127.0.0.1
Invoke-WebRequest http://127.0.0.1:3000/
Invoke-WebRequest http://127.0.0.1:3000/admin_login
Invoke-WebRequest http://127.0.0.1:3000/admin/accounts
Invoke-WebRequest http://127.0.0.1:3000/client/register
```
...
- نجح ESLint، ونجحت 107 حالات اختبار في 25 ملفًا، ونجح بناء Next.js الكامل مع توليد 37 صفحة.
- أصبح لكل منصة استيراد أنماط مركزي في تخطيط مسارها، بينما بقي `globals.css` للقواعد الأساسية والموقع العام داخل غلافه المخصص.
- استجابت صفحات الموقع ودخول الإدارة والحسابات وتسجيل المعلنين محليًا بالحالة `200`، وتأكد بصريًا ظهور منصة المعلنين دون حشوة `body` العامة.
- أعيد تشغيل خادم المعاينة على `http://127.0.0.1:3000`.
- لم يُنشر شيء، ولم تتغير إعدادات الإنتاج أو أرقام الإصدارات.

### إعادة بناء غلاف بوابة المعلنين كتخطيط دائم - دون تغيير إصدار

تم إنجازه:

- إضافة `ClientLayoutShell` إلى تخطيط `/client` لتمييز صفحات المصادقة عن مساحة المعلن المحمية.
- نقل الشريط الجانبي والشريط العلوي والفوتر إلى غلاف واحد دائم يلتف حول صفحات المعلن المحمية، على نمط غلاف الإدارة.
- إبقاء صفحات تسجيل الدخول والتسجيل واستعادة كلمة المرور مستقلة بصريًا وأمنيًا عن الغلاف المحمي.
- نقل فحص جلسة المعلن وملفه وصلاحياته إلى `ClientShell` مرة واحدة بدل تكراره في كل صفحة.
- إضافة `ClientPortalContext` لتزويد صفحات التقارير وإنشاء الحملة بالمستخدم والملف والصلاحيات دون مستمع Firebase إضافي.
- تحويل صفحتي لوحة التقارير وإنشاء الحملة إلى محتوى داخلي فقط، بحيث يغيّر تنقل Next.js المحتوى ويبقي الغلاف المشترك ثابتًا.
- إضافة حالة تحميل وخطأ داخل مساحة المحتوى مع بقاء الشريطين ظاهرين، وإضافة فوتر موحد للمنصة.
- إضافة تصغير للشريط الجانبي على سطح المكتب ودرج جانبي مغلق افتراضيًا على الجوال، مع حفظ تفضيل التصغير والمظهر محليًا.
- تغيير هوية منصة المعلنين إلى درجات خضراء مائلة للتركواز مستقلة عن ألوان الإدارة والموقع العام.
- تقليل الاستدارات والمسافات، وتوحيد البطاقات والجداول والنماذج مع النمط التشغيلي لصفحة تقارير الإدارة.
- إضافة اختبار معماري يمنع عودة الأغلفة ومستمعات المصادقة المكررة إلى صفحات المعلن.
- اختبار الانتقال الداخلي بين `/client/dashboard` و`/client/create-campaign` بصريًا ووظيفيًا.

الأخطاء المكتشفة:

1. **كل صفحة معلن كانت تعيد إنشاء الغلاف وجلسة المصادقة**
   - الأعراض: الانتقال بين التقارير وطلب الإعلان يعيد بناء الشريط الجانبي والناف بار، وكل صفحة تسجل مستمع Firebase مستقلًا.
   - السبب: استدعاء `ClientShell` ومنطق `onAuthStateChanged` داخل كل صفحة محمية.
   - الحل: رفع الغلاف والمصادقة إلى تخطيط `/client` الدائم وتمرير الجلسة عبر سياق مشترك.
   - الحالة: محلول محليًا.

2. **حجز علوي زائد في محتوى الجوال**
   - الأعراض: فراغ كبير بين الشريط العلوي وأول سيكشن في صفحة إنشاء الحملة.
   - السبب: بقاء حشوة علوية قديمة كانت مخصصة لزر قائمة عائم قبل وجود الشريط العلوي الدائم.
   - الحل: جعل حشوة محتوى الجوال موحدة بعد الشريط العلوي وإبقاء زر القائمة داخله بصريًا.
   - الحالة: محلول محليًا.

الملفات المتأثرة:

- `app/client/layout.jsx`
- `app/client/ClientLayoutShell.jsx`
- `app/client/ClientShell.jsx`
- `app/client/ClientPortal.css`
- `app/client/dashboard/page.jsx`
- `app/client/create-campaign/page.jsx`
...
- نجح ESLint، ونجحت 108 حالات اختبار في 25 ملفًا، ونجح بناء Next.js الكامل مع توليد 37 صفحة.
- تحقق العرض المرئي على سطح المكتب بعرض `1265px` دون تجاوز أفقي، وعلى الجوال بعرض `655px` دون تجاوز أفقي.
- تأكد ظهور الفوتر والقائمة الجانبية المتجاوبة، وتأكد أن الانتقال الداخلي يبدل محتوى الصفحة مع بقاء عناصر الغلاف نفسها.
- المعاينة المحلية متاحة على `http://127.0.0.1:3000/client/dashboard`.
- لم يُنشر شيء، ولم تتغير إعدادات الإنتاج أو بياناته أو أرقام الإصدارات.

### مطابقة تصميم بوابة المعلنين مع نظام واجهة الإدارة - دون تغيير إصدار

تم إنجازه:

- اعتماد قياسات واجهة الإدارة نفسها داخل الغلاف المعزول `.client-workspace` مع إبقاء ألوان المعلنين الخضراء المائلة للتركواز.
- مطابقة عرض السايد بار `260px` والعرض المصغر `80px` وحجم شعار المنصة `44px` وأيقونات التنقل `34px`.
- إعادة بناء رأس السايد بار وزر التصغير الدائري وخط التفعيل الجانبي وزر إغلاق درج الجوال وفق نظام الإدارة.
- إعادة بناء الناف بار كبطاقة بحد علوي ملون وارتفاع `65px`، ووضع ملف المعلن على اليمين وأزرار المظهر والخروج على اليسار.
- نقل عنوان الصفحة ووصفها من الناف بار إلى Hero مستقل بارتفاع `178px` وبنية مطابقة لصفحة تقارير الإدارة.
- إضافة دور حساب المعلن ورقم المنظمة داخل Hero بنفس نظام شارات الإجراءات في تقرير الإدارة.
- تحويل لوحة التقارير إلى أربع بطاقات KPI بنفس شبكة الإدارة وأحجام الأيقونات والقيم والعناوين والملاحظات.
- إعادة بناء رؤوس سيكشن الحملات ونموذج إنشاء الحملة بأيقونة `40px` وعنوان ووصف وفاصل سفلي مثل سيكشنات الإدارة.
- توحيد أحجام حقول النماذج والأزرار ورؤوس وصفوف الجداول والفوتر مع متغيرات واجهة الإدارة.
- إبقاء السايد بار والناف بار والفوتر داخل التخطيط الدائم، مع تغيير المحتوى فقط عند التنقل بين صفحات المعلنين.
- إضافة اختبارات ثابتة لأبعاد السايد بار والناف بار والـHero والشبكة الرباعية حتى لا تتراجع المطابقة مستقبلًا.

الأخطاء المكتشفة:

1. **واجهة المعلنين استخدمت تخطيطًا مشابهًا للإدارة دون مطابقة نظامها البصري**
   - الأعراض: عنوان الصفحة داخل الناف بار، أيقونات وتنقلات مستديرة، Hero وبطاقات بأحجام مختلفة، وفوتر ذو خلفية مستقلة.
   - السبب: تصميم الغلاف الأول استخدم مكونات وأبعادًا خاصة بدل تطبيق بنية `AdminShell` وصفحة التقارير نفسها.
   - الحل: إعادة توزيع عناصر الغلاف والمحتوى واعتماد القيم والأبعاد نفسها داخل محددات منصة المعلنين المعزولة.
   - الحالة: محلول محليًا.
...
```powershell
rg -n "legacy-sidebar|legacy-top-nav|admin-overview" app/admin/AdminDashboard.css
npm run lint
npm test
npm run build
git diff --check
...
- نجح ESLint ونجحت جميع الاختبارات.
- تحقق محليًا أن ارتفاع الناف بار `65px` وارتفاع Hero `178px` وأن عرض المستند يساوي عرض الشاشة دون تجاوز أفقي.
- عُرضت لوحة التقارير والسايد بار وصفحة إنشاء الحملة محليًا وتأكد تجاوبها على عرض الجوال.
- لم يُنشر شيء، ولم تتغير بيانات الإنتاج أو إعداداته أو أرقام الإصدارات.

### نظام الأدوار والصلاحيات ومالك المنصة - دون تغيير إصدار

تم إنجازه:

- إنشاء نموذج مركزي موحد لأدوار الإدارة وصلاحيات الإجراءات بدل الاعتماد على أسماء الصفحات فقط.
- إضافة الأدوار: مالك المنصة، المدير العام، مدير التشغيل، مدير المحتوى وSEO، مدير الإعلانات، مدير حسابات المعلنين، مسؤول الأمن، موظف الدعم، محلل الأداء، المدقق، ومساعد الإدارة.
- حصر صلاحيات ملكية المنصة وحذف الحسابات الإدارية في `platform_owner` فقط، مع إبقاء الأدوار القديمة متوافقة كمدير عام لا كمالك.
- اعتماد الحقل المرحلي `platformRole` قبل `role` و`adminRole` حتى يمكن ترقية الحسابات دون تعطيل النسخة المنشورة.
- تحويل حساب الإدارة الحالي `date.tool.official@gmail.com` في Firestore إلى `platformRole: platform_owner`، مع الإبقاء على `role` و`adminRole` بقيمة `super_admin` للتوافق مع النسخة الحالية.
- إنشاء صفحة `/admin/team` لإدارة حسابات الإدارة، الأدوار، حالة الحساب، وصلاحيات المساعد الدقيقة.
- منع الحساب من تغيير دوره أو تعطيل نفسه من جلسته الحالية، ومنع غير المالك من منح دور المالك أو حذف حساب إداري.
- فصل صلاحية تعديل الاسم والدور والصلاحيات عن صلاحية تعليق الحساب على مستوى الواجهة وقواعد Firestore.
- إنشاء صفحة `/admin/audit` وسجل `audit_logs` غير قابل للتعديل أو الحذف من العميل لتتبع العمليات الإدارية الحساسة.
- إضافة تسجيل تدقيق لتغييرات الأدوار والحالات والحملات والمعلنين والتذاكر وإعدادات الموقع.
- تطبيق صلاحيات الإجراءات على مسارات API لرفع الوسائط، PageSpeed، إرسال الفهرسة، ودعم العملاء.
- توسيع أدوار المعلنين إلى: مالك المنظمة، مدير المنظمة، مدير الحملات، محرر الحملات، محلل التقارير، مسؤول الفوترة، والمشاهد.
- إنشاء صفحة `/client/team` لإدارة أعضاء المنظمة وربطهم بالرقم الموحد، مع عزل كل منظمة ومنع تعديل المالك أو الحساب الحالي.
- فصل حفظ مسودة الحملة عن إرسالها للمراجعة وإيقافها بحسب صلاحية المعلن.
- تحديث قواعد Firestore لتطبيق أقل صلاحية لازمة، عزل المنظمات، حماية المالك، ومنع الكتابة المباشرة في سجل العمليات.
- إضافة اختبارات لنموذج الصلاحيات وتوافق الأدوار القديمة وفصل صلاحيات تعديل المدير عن تعليقه.

الأخطاء المكتشفة:

1. **الأدوار الإدارية القديمة كانت واسعة ولا تميز مالك المنصة**
   - الأعراض: قيم مثل `owner` و`admin` و`manager` كانت تعامل كصلاحية إدارية كاملة بلا فصل واضح للملكية.
   - السبب: الاعتماد على مجموعة أسماء قديمة بدل نموذج أدوار وصلاحيات إجراءات موحد.
   - الحل: إضافة `platformRole` ونموذج صلاحيات مركزي، وتحويل القيم القديمة إلى `super_admin` فقط.
   - الحالة: محلول ومنشور، وتم تعيين حساب المالك الحالي في Firebase.

2. **صلاحية المساعد كانت على مستوى الصفحة لا العملية**
   - الأعراض: منح الوصول إلى قسم قد يسمح بأكثر من الإجراء المطلوب.
   - السبب: ربط الصلاحيات بمسارات وأسماء صفحات عامة.
   - الحل: تعريف مفاتيح مستقلة للقراءة والإنشاء والتعديل والمراجعة والتشغيل والحذف مع تضمين القراءة تلقائيًا عند منح الكتابة.
   - الحالة: محلول ومنشور.

3. **صلاحية تعليق المدير كان يمكن أن تتداخل مع تعديل دوره عبر طلب مباشر**
   - الأعراض: قاعدة التحديث قبل المراجعة النهائية كانت تقبل أحد الإذنين ثم تسمح بمجموعة الحقول الإدارية نفسها.
   - السبب: التحقق من الإذن على مستوى عملية التحديث كلها بدل مقارنة الحقول المتغيرة.
   - الحل: فحص `affectedKeys()`؛ الاسم والدور والصلاحيات تتطلب `admins.update_role`، والحالة تتطلب `admins.suspend`، مع تطبيق التعطيل نفسه في الواجهة.
   - الحالة: محلول ومنشور ومغطى باختبار ثابت.

4. **لم يكن هناك سجل مركزي غير قابل للتلاعب للعمليات الحساسة**
   - الأعراض: صعوبة معرفة من غيّر دورًا أو حالة أو حملة أو تذكرة.
   - السبب: عدم وجود مجموعة تدقيق ومسار كتابة خادمي موحد.
   - الحل: إضافة `audit_logs` وواجهة عرض ومسار كتابة موثق الهوية، ومنع التعديل والحذف من قواعد العميل.
   - الحالة: محلول ومنشور، ويجمع السجل العمليات الجديدة وفق الصلاحيات.

5. **فريق المعلن لم يكن مقيدًا بأدوار منظمة مفصلة**
   - الأعراض: حساب واحد يتحمل إدارة الحملات والتقارير والفوترة والأعضاء.
   - السبب: نموذج أدوار محدود وعدم وجود صفحة فريق للمنظمة.
   - الحل: إضافة أدوار وصلاحيات المعلنين وصفحة الفريق وربط الأعضاء بـ `organizationId` والرقم الموحد.
   - الحالة: محلول ومنشور.
...
- `app/api/_lib/adminPermissions.js`
- `app/api/_lib/writeAdminAudit.js`
...
```powershell
rg -n "platform_owner|platformRole|admins.delete|admin_assistant|organization_admin|audit_logs" app firestore.rules tests
npm run lint
npm test
npm run build
git diff --check
npm run dev -- --hostname 127.0.0.1 --port 3000
Invoke-WebRequest http://127.0.0.1:3000/admin/team
Invoke-WebRequest http://127.0.0.1:3000/admin/audit
Invoke-WebRequest http://127.0.0.1:3000/client/team
```

الحالة:

- نجح ESLint، ونجحت 118 حالة اختبار في 26 ملفًا، ونجح بناء Next.js الكامل مع توليد 40 مسارًا.
- استجابت صفحات `/admin/team` و`/admin/audit` و`/client/team` محليًا بالحالة `200` بعد إعادة تشغيل خادم التطوير عقب البناء.
- تم التحقق بصريًا من مستند حساب الإدارة في Firebase وظهور `platformRole: "platform_owner"` مع بقاء حقول التوافق القديمة.
- نُشر الكود وقواعد Firestore معًا، وأصبحت صفحات الفريق وسجل العمليات والصلاحيات الجديدة فعالة في المنصة الأساسية.
- دخلت الصلاحيات أولًا ضمن الإصدار `0.3.55`، ثم نُشر الإصدار `0.3.56` لإصلاح سكربت تهيئة الثيم وتحديث الاعتماديات الأمنية.

### تأمين الصلاحيات ونشر الإصدار 0.3.56

تم إنجازه:

- نشر نموذج أدوار الإدارة والمعلنين وقواعد Firestore المقيدة إلى مشروع الإنتاج `date-tool-official`.
- تفعيل دور `platform_owner` لحساب مالك المنصة مع إبقاء حقول التوافق القديمة.
- نشر صفحات `/admin/team` و`/admin/audit` و`/client/team` ومسارات الخادم المحمية بصلاحيات الإجراءات.
- إصلاح التعبير المنتظم داخل سكربت تهيئة الثيم المضمّن حتى يصل إلى المتصفح بصيغة JavaScript صحيحة.
- إضافة اختبار يقيّم النص النهائي لسكربت التهيئة ويمنع عودة خطأ التحليل في الإنتاج.
- تحديث `Next.js` إلى `15.5.25` و`Vitest` إلى `4.1.11` وتحديث الحزم العابرة المتأثرة بالتنبيهات الأمنية.
- تثبيت اعتماديات `sharp` المحمولة صراحة في ملف القفل حتى ينجح `npm ci` على Linux داخل بناء Cloudflare.
- نشر الإصدار العام `0.3.56` ونسخة الإدارة `0.1.53` ونسخة بوابة المعلنين `1.0.6`.
- التحقق من الإصدار المنشور عبر `/api/app-version` ومن استجابة الصفحات العامة وصفحات الفريق والتدقيق.
- التحقق من رؤوس الأمان المنشورة، ومنها `Strict-Transport-Security` و`Content-Security-Policy` و`X-Frame-Options: DENY` و`X-Content-Type-Options: nosniff`.

الأخطاء المكتشفة:

1. **سكربت تهيئة الثيم وصل إلى المتصفح بتعبير منتظم غير صالح**
   - الأعراض: توقف JavaScript في بعض صفحات الإدارة وظهور خطأ `Unexpected token '?'`.
   - السبب: فقدان تهريب الشرطات داخل النص القالبي المضمّن أثناء توليد HTML.
   - الحل: تهريب التعبير المنتظم داخل القالب وإضافة اختبار يترجم النص النهائي كما يصل إلى المتصفح.
   - الحالة: محلول ومنشور.

2. **بناء Cloudflare فشل بعد تحديث `sharp` رغم نجاح البناء على Windows**
   - الأعراض: توقف `npm ci` في Cloudflare بسبب فقدان حزمتَي `@emnapi/core` و`@emnapi/runtime` من قفل Linux.
   - السبب: ملف القفل المولّد على Windows لم يثبت الاعتماديتين الاختياريتين اللازمتين لمسار `sharp` المحمول في بيئة البناء.
   - الحل: إضافتهما كاعتماديات تطوير صريحة وإعادة توليد ملف القفل والتحقق بـ `npm ci` وبناء OpenNext.
   - الحالة: محلول ومنشور.
...
- `package-lock.json`
- `VERSION_LOG.md`
- `tests/layoutScripts.test.js`
- `tests/publicSiteConfig.test.js`
- `tests/pwaSettings.test.js`
- `firestore.rules`
- `PROJECT_MEMO.md`
...
npm run build
npx opennextjs-cloudflare build
npx firebase-tools@14.16.0 deploy --only firestore:rules --project date-tool-official
git push origin master
Invoke-WebRequest https://date-tool.com/?release=0.3.56-rbac
curl.exe https://date-tool.com/api/app-version?release=0.3.56-rbac
```

الحالة:

- نجحت 119 حالة اختبار في 26 ملفًا، ونجح ESLint وبناء Next.js وبناء OpenNext.
- أعاد `npm audit` النتيجة `0 vulnerabilities`.
- استجاب الموقع ومسارات `/admin/team` و`/admin/audit` و`/client/team` بالحالة `200` على الإنتاج.
- أعاد `/api/app-version` الإصدار `0.3.56` بتاريخ `2026-09-10`.
- التغييرات منشورة على Worker المسمى `datetools` وقواعد Firestore منشورة على مشروع الإنتاج.

### تدقيق Search Console وتصحيح خريطة الموقع - الإصدار 0.3.57

تم إنجازه:

- مراجعة تقرير فهرسة الصفحات في Search Console وخريطة الموقع المسجلة للدومين `date-tool.com`.
- توثيق وجود 11 صفحة مفهرسة و23 صفحة غير مفهرسة في التقرير المحدث بتاريخ 2026-09-04.
- التحقق من أن حالتي إعادة التوجيه والـCanonical البديل تخصان نسخ HTTP وwww المتوقعة، وليستا خطأ بنيويًا.
- التحقق من أن `/ad_request.html` مسار قديم يعيد `410` حاليًا وسيختفي بعد إعادة الزحف.
- اختبار `/en/date-converter` مباشرة عبر فحص URL الحي والتأكد من أنه متاح لـGoogle وقابل للفهرسة ويحمل Breadcrumb صالحًا.
- اكتشاف أن `/month-names` كانت معطلة في الإدارة وتعيد `404` لكنها بقيت داخل `sitemap.xml` بسبب فصل المحتوى عن حالة التشغيل في بيانات Firebase القديمة.
- توحيد تجميع صفحات sitemap مع أولوية الدمج نفسها المستخدمة في صفحة العرض، حتى تعتمد الخريطة حالة الصفحة الفعلية بعد دمج `customPages` و`internalPages`.
- جعل `llms.txt` يبني روابط الصفحات المدارة من الصفحات المفعلة بدل قائمة ثابتة قد تحتوي روابط معطلة.
- التحقق محليًا بعد بناء الإنتاج من أن الخريطة تحتوي 28 رابطًا، ولا تحتوي `/month-names` المعطلة، مع بقاء `/privacy` و`/terms` و`/contact` و`/about-us`.

الأخطاء المكتشفة:

1. **صفحة معطلة بقيت معلنة داخل خريطة الموقع**
   - الأعراض: `/month-names` موجودة ضمن sitemap المرسلة إلى Google لكنها تعيد `404`.
   - السبب: محتوى الصفحة محفوظ في `customPages` دون حقل تشغيل، وحالة التعطيل محفوظة في `internalPages`؛ وكانت الخريطة تفحص المجموعتين منفصلتين.
   - الحل: تجميع slugs أولًا ثم حل الصفحة الفعلية بنفس ترتيب دمج صفحة العرض قبل تقرير ظهورها في sitemap.
   - الحالة: محلول ومنشور ضمن الإصدار `0.3.57`.

2. **ملف llms.txt احتوى رابطًا ثابتًا لصفحة معطلة**
   - الأعراض: الملف يعلن عن `/month-names` رغم أن الصفحة تعيد `404`.
   - السبب: قائمة الصفحات التعريفية كانت مكتوبة داخل الكود ولا تقرأ حالة الإدارة.
   - الحل: تحميل الصفحات المدارة المفعلة فقط من إعدادات الموقع عند إنشاء الملف.
   - الحالة: محلول ومنشور ضمن الإصدار `0.3.57`.
...
npx opennextjs-cloudflare build
git diff --check
Invoke-WebRequest http://127.0.0.1:3201/sitemap.xml
Invoke-WebRequest http://127.0.0.1:3201/llms.txt
git push origin master
```

الحالة:

- نجحت 121 حالة اختبار في 26 ملفًا، ونجح ESLint وبناء Next.js الكامل وبناء OpenNext.
- التحقق الإنتاجي المحلي أثبت تطابق sitemap مع حالات تشغيل الصفحات.
- نُشر الإصدار `0.3.57` إلى المنصة الأساسية وتحقق مسار `/api/app-version` من ظهوره في الإنتاج.
...
### نشر الإصدار 0.3.58 والتحقق الإنتاجي
...
- دفع commit الإصدار `8b04338` إلى فرع `master` في GitHub.
- تسجيل Wrangler عبر OAuth بالحساب `date.tool.official@gmail.com` والتحقق من الحساب قبل النشر.
- نشر الحزمة إلى Cloudflare Worker الإنتاجي `datetools` بنجاح.
- تثبيت معرّف نسخة Cloudflare: `9f2d5c69-a45b-44bc-85ab-5caf453753df`.
- التحقق من أن `/api/app-version` يعيد `0.3.58` وتاريخ `2026-09-13` من النطاق الحقيقي.
- التحقق من أن الصفحة الرئيسية وبوابة المعلنين وإعدادات الإعلانات وصفحة دخول الإدارة تعيد الحالة `200`.
- التحقق من أن `/admin_login` يعيد `Cache-Control: no-store, must-revalidate, no-cache, max-age=0, private`.
- إعادة تحميل صفحة دخول الإدارة في متصفح جديد حتى اكتمل Turnstile وظهر زر الدخول دون أخطاء JavaScript أو `ChunkLoadError`.
...
- ظهرت مهلات اتصال غير قاتلة إلى Firebase أثناء توليد الصفحات الثابتة في بناء النشر؛ أكمل Next.js توليد الصفحات `39/39` ونجح رفع الحزمة، ثم نجحت اختبارات الإنتاج المباشرة.
...
npx wrangler whoami
npm run deploy
curl.exe -sS -D - "https://date-tool.com/api/app-version?release=0.3.58"
Invoke-WebRequest -Method Head https://date-tool.com/admin_login?release=0.3.58
git diff --check
```
...
- نُشر الإصدار `0.3.58` إلى المنصة الأساسية عبر Worker `datetools`، وتحققت واجهة الإصدار والصفحات الأساسية من النطاق الحقيقي.
- لا توجد قواعد Firestore أو تعديلات صلاحيات حسابات معلقة في هذا الإصدار؛ المفتاح القانوني لدور مالك المنصة بقي `platform_owner` كما هو في القواعد المنشورة سابقًا.

### معالجة P1 وتأسيس بوابة الاختبار والجودة - محلي فقط

تم إنجازه:

- حسم سياسة PWA إلى `online-only` صراحة: التطبيق قابل للفتح بواجهة مستقلة عند التثبيت لكنه يحتاج الإنترنت للأدوات وتحميل التحديثات، ولا يدعي دعم العمل دون اتصال.
- إضافة مصدر مركزي لسياسة PWA ورسالة اتصال عربية وإنجليزية، وإظهارها في تنبيهي التثبيت والتحديث.
- تثبيت عدم تسجيل Service Worker باختبار متصفح، مع إبقاء Manifest بوضع `standalone` ومسار البداية `/`.
- فصل الجذور الخادمية للموقع العام إلى مجموعتي مسارات Next.js دون تغيير عناوين URL: جذر عربي يخرج `lang="ar" dir="rtl"` وجذر إنجليزي يخرج `lang="en" dir="ltr"` من HTML الأولي.
- نقل التخطيط العام المشترك إلى `PublicRootLayout.jsx` وإضافة تخطيط داخلي مستقل للإدارة ودخول الإدارة وبوابة المعلنين والدعم.
- الحفاظ على التوليد الثابت للمسارات الأساسية العربية والإنجليزية؛ ولّد البناء 39 صفحة ثابتة وبقيت صفحات `/` و`/clock` و`/weather` ونظائرها الإنجليزية Static.
- إضافة Playwright وaxe وبوابة `npm run test:quality` التي تجمع اختبارات الوحدة وE2E.
- تغطية المسارات العامة الستة على Chrome لسطح المكتب ومحاكاة Pixel 5: HTML اللغة والاتجاه، غياب التمدد الأفقي، أخطاء الصفحة، ومخالفات WCAG الخطيرة والحرجة.
- تغطية إعادة تحميل صفحة دخول الإدارة وتسميات حقولها وخصائص `autocomplete`، وتغطية دخول حساب المعلن التجريبي المحلي حتى لوحة التحكم.
- ربط حقول البريد وكلمة المرور في دخولي الإدارة والمعلنين بتسميات صريحة وتحسين خصائص الإكمال التلقائي.
- تصحيح تباين الرابط النشط ورقم الإصدار وزر مشاركة المواعيد حتى اجتازت العينة فحص axe دون مخالفات خطيرة أو حرجة.
- إصلاح Hydration في صفحة الساعة؛ أصبح الرسم الخادمي والأول رسم للمتصفح ثابتين ثم يملأ الوقت الفعلي بعد التحميل.
- جعل Playwright يعمل بعامل واحد لأن تجميع مسارات Next في وضع التطوير بالتوازي كان يؤخر تهيئة الصفحات وTurnstile المحلي ويولد إخفاقات مهلة غير حقيقية.
- إضافة مخرجات Playwright إلى `.gitignore` حتى لا تدخل لقطات الفشل والتتبعات في المستودع.
- تحديث `COMPREHENSIVE_AUDIT_CHECKLIST.md`: P1 مكتمل محليًا، وP2 تحسن إلى جزئي، مع إبقاء البنود الخارجية واليدوية والمتطلبات الأخرى متبقية.

الأخطاء المكتشفة:

1. **HTML الإنجليزي الأولي كان يحمل لغة واتجاه العربية**
   - الأعراض: JavaScript يصحح الصفحة بعد التحميل، لكن المصدر الأولي غير دقيق لمحركات البحث وتقنيات المساعدة.
   - السبب: جميع المسارات كانت تشترك في Root Layout عربي واحد.
   - الحل: استخدام Route Groups بجذر عربي وآخر إنجليزي مع تخطيط عام مشترك، دون تغيير المسارات المنشورة.
   - الحالة: محلول ومختبر محليًا؛ غير منشور.

2. **سياسة PWA لم تكن تحسم توقع العمل دون اتصال**
   - الأعراض: وجود تثبيت وفحص نسخة دون Service Worker قد يوحي بدعم Offline غير موجود.
   - السبب: لم توجد سياسة تشغيل معلنة ومركزية.
   - الحل: اعتماد `online-only` وإظهار شرط الاتصال واختبار عدم تسجيل Service Worker.
   - الحالة: محلول محليًا؛ اختبار التثبيت الفعلي على الأنظمة ما زال متبقيًا.

3. **تباين ثلاثة عناصر أقل من WCAG AA**
   - الأعراض: كشف axe نسب تباين أقل من `4.5:1` للرابط النشط ورقم الإصدار وزر مشاركة المواعيد.
   - السبب: لون أزرق فاتح فوق خلفية زرقاء شفافة، وشفافية إضافية على نص الإصدار.
   - الحل: إضافة لون نص نشط مناسب للوضعين وإزالة شفافية الإصدار وربط زر المشاركة باللون الموحد.
   - الحالة: محلول؛ اجتازت الصفحات العامة الممثلة الفحص الآلي على العرضين.

4. **Hydration متغير في بانر الساعة الحالية**
   - الأعراض: قد يخرج الخادم دقيقة ويبدأ المتصفح في الدقيقة التالية، فيسجل React عدم تطابق.
   - السبب: تهيئة الحالة بـ`new Date()` أثناء SSR وأثناء Hydration.
   - الحل: قيمة أولية ثابتة ثم ضبط الوقت داخل `useEffect` وتحديثه عند بداية كل دقيقة.
   - الحالة: محلول ومغطى بفحص أخطاء الصفحة في E2E.

الملفات المتأثرة:

- `.gitignore`
- `app/(arabic)/layout.jsx`
- `app/(arabic)/page.jsx`
- `app/(arabic)/[slug]/page.jsx`
- `app/(arabic)/clock/page.jsx`
- `app/(arabic)/weather/page.jsx`
- `app/(english)/layout.jsx`
- `app/(english)/en/page.jsx`
- `app/(english)/en/[slug]/page.jsx`
- `app/(english)/en/clock/page.jsx`
- `app/(english)/en/weather/page.jsx`
- `app/PublicRootLayout.jsx`
- `app/InternalRootLayout.jsx`
- `app/admin/layout.jsx`
- `app/admin_login/layout.jsx`
- `app/admin_login/page.jsx`
- `app/client/layout.jsx`
...
- `app/components/PwaInstallPrompt.jsx`
- `app/components/PwaUpdatePrompt.jsx`
- `app/pwaRuntimePolicy.js`
- `app/globals.css`
- `e2e/login-quality.spec.js`
- `e2e/public-quality.spec.js`
- `playwright.config.mjs`
- `package.json`
- `package-lock.json`
- `tests/adsenseReadiness.test.js`
- `tests/pwaInstall.test.js`
- `tests/pwaRuntimePolicy.test.js`
- `tests/securityBoundaries.test.js`
- `tests/toolStandaloneRoutes.test.js`
- `COMPREHENSIVE_AUDIT_CHECKLIST.md`
- `PROJECT_MEMO.md`
...
npm test
npm run test:e2e
npm run test:e2e -- --grep "public page quality"
...
- نجح ESLint دون أخطاء.
- نجحت 135 حالة اختبار وحدة في 29 ملفًا.
- نجحت بوابة Playwright الكاملة: 30 اختبارًا على Chrome لسطح المكتب ومحاكاة Pixel 5.
- نجح فحص الصفحات العامة الست على العرضين دون مخالفات axe خطيرة أو حرجة، ودون تمدد أفقي أو أخطاء JavaScript.
- نجح بناء Next.js الكامل وتوليد 39 صفحة ثابتة مع بقاء المسارات الأساسية ثابتة.
- لم يُنشر أو يُدفع أي تغيير، ولم تتغير Firebase أو Cloudflare أو AdSense أو الحسابات أو الأسرار أو رقم الإصدار.

المتبقي المحفوظ:

- نشر هذه التغييرات لا يتم إلا بعد موافقة المستخدم الصريحة.
- اختبار قارئ شاشة فعلي ولوحة المفاتيح والتكبير 200% والوضعين على أجهزة حقيقية.
- اختبار تثبيت PWA وفتح Deep Links وتحديث النسخة على iOS وiPadOS وAndroid وSamsung Internet وWindows وmacOS؛ السياسة تبقى `online-only` ولا يتوقع عمل الأدوات دون اتصال.
- قياس Core Web Vitals ميدانيًا من CrUX/PageSpeed عند المئين 75، وتشغيل Trace محلي عند توافر Chrome DevTools MCP؛ نجاح البناء وPlaywright لا يعوض هذا القياس.
- استكمال E2E للتواصل والمشاركة وإنشاء الحملات والحفظ بحسابات اختبار معزولة، واختبارات Firestore Emulator والتكاملات الخارجية.
- تعريف RPO لكل Firestore وR2 والإعدادات، وتعريف RTO، وتفعيل النسخ المناسبة وتجربة استعادة فعلية إلى مورد منفصل.
- إنشاء Health checks متعددة للصفحة وواجهات API الحرجة، وربط تنبيهات التشغيل وخطة الحوادث وSLI/SLO وOn-call وRollback عملي.
- جميع بنود P2/P3 والبنود الخارجية الأخرى في `COMPREHENSIVE_AUDIT_CHECKLIST.md` تبقى كما هي ما لم يثبت إنجازها بدليل مستقل.

### إكمال بوابة الجودة والتشغيل وإحصاء تثبيت PWA - محلي فقط

تم إنجازه:

- إضافة إحصاء خصوصي لعدد الزوار الفريدين الموافقين على التحليلات، وعدد معرّفات المتصفحات التي ثبّتت PWA، ونسبة التثبيت بين الزوار الموافقين.
- عدم إنشاء معرّف القياس أو إرسال أي طلب إحصائي قبل موافقة الزائر على التحليلات.
- استخدام UUID عشوائي مجهول وتجزئته بـSHA-256 على الخادم، دون تخزين الاسم أو البريد أو UID الحساب أو بصمة الجهاز.
- منع قراءة وكتابة مجموعتي علامات الزائرين والتثبيت مباشرة من العملاء، واختبار ذلك داخل Firestore Emulator.
- إضافة بطاقات عدد الزوار الفريدين والمثبتين ونسبة التثبيت إلى لوحة الإدارة.
- إضافة واجهة `/api/health` صغيرة لا تكشف الأسرار، وأداة `npm run health:check` للتحقق من الصحة والنسخة والـManifest وrobots وsitemap.
- إضافة Runbook تشغيلي يشرح Health Check وSLI وSLO وRPO وRTO والحوادث والتراجع عن إصدار Worker.
- إضافة خطة قبول يدوية لقارئ الشاشة ولوحة المفاتيح والتكبير وPWA والروابط العميقة والمسارات العامة والإدارة والمعلنين.
- إضافة اختبارات Firestore Emulator مع مشروع وهمي ثابت `demo-date-tools-security` يمنع الوصول العرضي إلى الإنتاج.
- تغطية صلاحيات `platform_owner` والإداريين والمساعدين والمعلنين وأعضاء المؤسسات وعزل البيانات بين المؤسسات في 8 سيناريوهات قواعد.
- إضافة بوابة جودة محلية وCI تجمع ESLint وتدقيق حزم الإنتاج واختبارات الوحدة وقواعد Firestore وبناء Next.js واختبارات المتصفح.
- عزل E2E عن Firestore الإنتاجي والحملات الحقيقية، وتشغيله على منفذ محلي عشوائي مع إيقاف خادمه فقط عند الانتهاء.
- منع لوحة الإدارة المحلية من تنفيذ مزامنة تلقائية إلى إعدادات Firestore العامة عند فتحها على `localhost` أو `127.0.0.1` أو `::1`، مع إبقاء مزامنة النطاق المنشور كما هي.
- تغطية 50 حالة متصفح على سطح المكتب والجوال: اللغتان، الاتجاه، إمكانية الوصول، المقاسات، الموافقة على الإحصاء، الدعم، المشاركة، دخول المعلن، إنشاء حملة، وإضافة عضو للمؤسسة.
- نجاح بناء OpenNext المحلي وإنشاء `.open-next/worker.js`، ثم نجاح محاكاة Wrangler مع `--dry-run` وقراءة 186 ملف أصل دون رفع.
- فحص التغييرات بحثًا عن الأسرار؛ لم توجد مفاتيح خاصة أو ملفات `.env`. المطابقات الموجودة هي مثال وهمي، مفتاح Firebase Client العام، ونصوص معالجة المفاتيح على الخادم.

الأخطاء المكتشفة:

1. **بوابة الجودة لم تبدأ على Windows**
   - الأعراض: أعاد `spawnSync npm.cmd EINVAL` قبل ESLint.
   - السبب: تشغيل ملف Windows command مباشرة دون مشغّل محمول.
   - الحل: تشغيل npm عبر `process.execPath` و`npm_execpath` عند توفره، مع fallback مناسب فقط عند الحاجة.
   - الحالة: محلول؛ اكتملت البوابة كاملة.

2. **ESLint حلل حزمة Wrangler المولدة**
   - الأعراض: ظهرت أخطاء React داخل `.tools/wrangler-dry-run/worker.js`.
   - السبب: مسار أدوات الاختبار لم يكن ضمن الاستثناءات العامة لـESLint.
   - الحل: استثناء `.tools/**` مثل `.next/**` و`.open-next/**` لأنها مخرجات وأدوات محلية وليست مصدر المشروع.
   - الحالة: محلول؛ نجح ESLint.

3. **محاكي Firebase لم يجد Node/NPM في تشغيل الجودة الموسع**
   - الأعراض: فشل الأمر الداخلي برسالة أن `npm` أو `node` غير معروف على Windows.
   - السبب: بيئة العملية الموسعة لم تورّث مسار تثبيت Node إلى الأمر الذي ينفذه Emulator.
   - الحل: إضافة مجلد `process.execPath` صراحة إلى `PATH` واستخدام أمر npm بسيط داخل المحاكي.
   - الحالة: محلول؛ نجحت السيناريوهات الثمانية داخل المشروع الوهمي.

4. **نقرة إرسال الدعم قد تضيع أثناء Hydration البطيء**
   - الأعراض: قد يتفاعل المستخدم مع نموذج الدعم قبل اكتمال ربط React للأحداث.
   - السبب: زر الإرسال كان متاحًا قبل إعلان جاهزية المكوّن في المتصفح.
   - الحل: إبقاء الإرسال معطلًا حتى اكتمال `useEffect` ثم اختبار التدفق بطلب API معزول.
   - الحالة: محلول ومغطى في E2E.

5. **لوحة الإدارة المحلية حاولت مزامنة الإعدادات العامة مع Firestore**
   - الأعراض: ظهرت شارة خطأ تطوير ورسالة `Unable to synchronize the public site settings` عند فتح لوحة الإدارة محليًا.
   - السبب: مزامنة `settings/main` إلى `settings/public` كانت تعمل تلقائيًا بعد دخول المالك على جميع المضيفين، بما فيها loopback المحلي.
   - الحل: منع المزامنة التلقائية على مضيفات loopback الثلاثة، واختبار وجود الحاجز ثم إعادة تحميل الصفحة محليًا دون ظهور خطأ جديد.
   - الحالة: محلول محليًا؛ لم يُكتب أي إعداد من هذا الإصلاح إلى Firestore.
...
- `app/components/PwaInstallPrompt.jsx`
- `app/SiteShell.jsx`
- `app/statisticsClient.js`
- `app/statisticsPolicy.js`
- `app/firestorePublicConfig.js`
- `app/client/localAdvertiserDemo.js`
- `app/client/dashboard/page.jsx`
- `app/client/ClientPortal.css`
...
- `docs/STATISTICS_AND_PRIVACY.md`
- `e2e/login-quality.spec.js`
- `e2e/public-quality.spec.js`
- `e2e/workflows-quality.spec.js`
...
- `scripts/run-firestore-rules-tests.mjs`
- `scripts/run-quality-gate.mjs`
- `tests/healthRoute.test.js`
- `tests/localAdvertiserDemo.test.js`
- `tests/statisticsPolicy.test.js`
- `PROJECT_MEMO.md`
...
npm run build
npx opennextjs-cloudflare build
npx wrangler deploy --dry-run --outdir .tools/wrangler-dry-run
npm audit --omit=dev --audit-level=high
git diff --check
rg -n --hidden -g "!.git/**" -g "!node_modules/**" -g "!.next/**" -g "!.open-next/**" -g "!.tools/**" "<secret-patterns>" .
```

الحالة:

- نجحت بوابة الجودة المحلية كاملة دون نشر.
- نجح ESLint، وتدقيق حزم الإنتاج يعيد صفر ثغرات معروفة.
- نجحت 138 حالة اختبار وحدة وتكامل في 31 ملفًا.
- نجحت 8 حالات لقواعد Firestore داخل Emulator معزول؛ لم يُستخدم مشروع الإنتاج.
- نجح بناء Next.js وتوليد 39 صفحة، ونجح بناء OpenNext ومحاكاة Wrangler المحلية.
- نجح 49 اختبار متصفح، وتُرك اختبار واحد فقط عمدًا لأن فحص التكبير 200% ينفذ على سطح المكتب ولا يحتاج تكراره في محاكاة الجوال.
- لم يُنشر أو يُدفع شيء، ولم تُرفع قواعد Firestore، ولم تتغير Cloudflare أو Firebase أو AdSense أو الحسابات أو رقم الإصدار.
- ملف `firestore.rules` معدل محليًا، ويلزم نشره لاحقًا مع الكود فقط بعد موافقة صريحة واختبار النسخة المرشحة.

المتبقي المحفوظ:

- البنود المدفوعة أو الخارجية: نسخ Firestore المجدول أو PITR، نسخة R2 الخارجية أو Versioning المناسبة، وتنبيهات Cloudflare Health Checks حسب الخطة المتاحة.
- البنود التي تنتظر تقارير زمنية: Search Console وAdSense وCrUX/PageSpeed الميداني عند توفر بيانات كافية.
- البنود التي تحتاج أجهزة أو تفاعلًا بشريًا: قارئ شاشة فعلي، تثبيت وتحديث PWA والروابط العميقة على iOS وiPadOS وAndroid وSamsung Internet وWindows وmacOS، وتجربة استعادة فعلية إلى مورد منفصل.
- النشر نفسه متوقف حتى موافقة المستخدم الصريحة؛ لا توجد مشكلة برمجية محلية معروفة تمنع بدء اختبار القبول.

### تأسيس سجل الاختبارات البشرية والحسابات الخارجية - محلي فقط

تم إنجازه:

- إنشاء `docs/HUMAN_VERIFICATION_CHECKLIST.md` لفصل ما ينجز برمجيًا عما يحتاج حسابًا خارجيًا أو جهازًا أو خطة مدفوعة.
- اعتماد حالات ثابتة للبنود، وقاعدة تمنع اعتبار MFA أو صلاحية مكتملة اعتمادًا على الذاكرة دون دليل وتاريخ.
- تسجيل أن `PROJECT_MEMO.md` لم يكن يحتوي دليلًا يحدد الحسابات التي فُعل لها MFA سابقًا؛ لذلك بقيت حالتها غير موثقة بدل ادعاء اكتمالها.
- جرد الحسابات التي تحتاج MFA: Google الأساسي والاحتياطي، Cloudflare، GitHub، مسجل النطاق، بريد العمل والاسترداد، حساب مالك المنصة الداخلي، وأي دفع أو متجر أو خدمة حساسة مستقبلية.
- إضافة مصفوفة صلاحيات بشرية لـCloudflare وFirebase/Google Cloud وGitHub وAdSense وSearch Console وأدوار الإدارة الداخلية.
- شرح اختبار الملفات التنفيذية والامتدادات المزورة وHTML وSVG باستخدام Fixtures غير مؤذية لا تُشغّل.
- ترتيب التنفيذ المشترك ليبدأ بحساب Google الأساسي ثم Cloudflare وGitHub والنطاق والصلاحيات والوسائط والأجهزة والنسخ والتقارير.
- إنشاء هدف مستمر للمهمة حتى تظل بنود الجاهزية والأمان والجودة هي المرجع عند أي طلب لاحق متعارض، مع السماح فقط بإعادة ترتيبها بقرار صريح.

الأخطاء المكتشفة:

1. **حالة MFA السابقة غير قابلة للإثبات من المذكرة**
   - الأعراض: توجد إشارة عامة إلى أن MFA مطلوب، دون أسماء حسابات أو تاريخ أو وسيلة أو دليل فحص.
   - السبب: الجولات السابقة لم تسجل نتيجة MFA في قالب ثابت.
   - الحل: اعتبار كل حساب غير موثق، وإنشاء سجل اختبار لا يحتوي أسرارًا ويثبت الحالة والطريقة والتاريخ فقط.
   - الحالة: نظام التوثيق محلول؛ التحقق من الحسابات يبدأ مع المستخدم.
...
```powershell
rg -n -i "MFA|2FA|خطوتين|عاملين|التحقق الثنائي|المصادقة الثنائية" PROJECT_MEMO.md COMPREHENSIVE_AUDIT_CHECKLIST.md docs README.md
Select-String -Path PROJECT_MEMO.md -Pattern "MFA|2FA|التحقق بخطوتين|المصادقة الثنائية" -Context 3,5
```

الحالة:

- لم يُنشر أو يُدفع شيء، ولم تتغير الحسابات أو الصلاحيات الخارجية.
- H-MFA-01 لحساب Google الأساسي هو البند الجاري التالي.

المتبقي المحفوظ:

- تنفيذ البنود البرمجية المفتوحة في سجل الاختبارات البشرية.
- تنفيذ اختبارات الحسابات والأجهزة خطوة بخطوة مع المستخدم وتسجيل الدليل الآمن.
- البنود المدفوعة لا تفعل قبل قرار تكلفة وموافقة صريحة.

### اختبار رفض الملفات الخطرة بعينات غير مؤذية - محلي فقط

تم إنجازه:

- إضافة `validateImageUpload` كمصدر تحقق موحد تستخدمه واجهة رفع R2 والاختبارات.
- ترتيب التحقق بحيث يكتمل فحص الحجم والنوع وMagic Bytes قبل إنشاء مفتاح الوسيط وقبل `bucket.put`.
- إضافة Fixtures غير مؤذية لملف نصي باسم JPEG وHTML وSVG ذي حدث غير ضار وملف نصي بامتداد EXE.
- إثبات رفض الحالات الأربع دون تشغيلها، مع إبقاء صورة PNG ذات التوقيع الصحيح مقبولة.
- تحديث سجل الاختبارات البشرية لتمييز الإثبات البرمجي المكتمل عن اختبار واجهة الرفع اليدوي المتبقي.

الأخطاء المكتشفة:

1. **اختبارات الوسائط لم تغط الامتداد المزور والملفات النشطة تمثيليًا**
   - الأعراض: كان الاختبار يتحقق من SVG ومن توقيع PNG فقط دون عينات ملفات محفوظة.
   - السبب: لم توجد Fixtures أمنية موحدة أو دالة تجمع قرار الحجم والنوع والتوقيع.
   - الحل: إضافة عينات آمنة ودالة تحقق مشتركة واختبارات رفض لكل حالة قبل التخزين.
   - الحالة: محلول برمجيًا؛ اختبار الواجهة اليدوي محفوظ في السجل.
...
- `app/api/media/upload/route.js`
- `tests/mediaValidation.test.js`
- `tests/fixtures/upload-security/active-content.html`
- `tests/fixtures/upload-security/active-content.svg`
- `tests/fixtures/upload-security/fake-executable.exe`
- `tests/fixtures/upload-security/fake-image.jpg`
- `docs/HUMAN_VERIFICATION_CHECKLIST.md`
- `PROJECT_MEMO.md`
...
```powershell
npm test -- --run tests/mediaValidation.test.js
npm run lint
git diff --check
```
...
- نجحت 8 حالات اختبار في `tests/mediaValidation.test.js` ونجح ESLint.
- لم يُرفع أي ملف إلى R2، ولم يُشغل أي Fixture، ولم يُنشر شيء.

المتبقي المحفوظ:

- تجربة الرفض من واجهة الإدارة بعد توفير بيئة رفع اختبار لا تمس R2 الإنتاجي.
- بقية البنود البرمجية والبشرية في `docs/HUMAN_VERIFICATION_CHECKLIST.md`.

### إغلاق الحواجز البرمجية للأمان والمرونة وفحص ما بعد النشر - محلي فقط

تم إنجازه:

- إنشاء سياسة مركزية تمنع لوحة الإدارة وبوابة المعلنين المحليتين من تنفيذ عمليات الكتابة المعروفة إلى Firebase وR2 وIndexNow من `localhost` و`127.0.0.1` و`::1`، مع إبقاء القراءة متاحة للاختبار.
- إضافة حاجز مبكر في Middleware يرفض واجهات الكتابة الحساسة محليًا قبل وصولها إلى الخدمة الخارجية، وحماية Same-Origin لطلبات المتصفح لمنع CSRF من أصل أجنبي.
- تغطية SSRF في PageSpeed وIndexNow، وIDOR وعزل المنظمات عبر Firestore Emulator، والطلبات المتكررة في Turnstile والإحصائيات، والروابط والوسائط غير الآمنة.
- إضافة مهلات موحدة لطلبات الطقس ورفع الوسائط من الواجهات، مع معالجة HTTP وJSON غير الصالح والاتصال البطيء، والإبقاء على فشل Turnstile مغلقًا وإعادة محاولة Firebase Auth للشبكة مرة واحدة.
- إضافة سجل عمليات للمعلنين بهوية خادمية متحققة وحقول مقيدة، وربطه بالتسجيل والتفعيل والحملات والفريق والوسائط، واستكمال تغطية الإدارة للإعدادات والحسابات والدعم والتنظيف وIndexNow.
- منع العملاء من إنشاء أو تعديل `audit_logs` مباشرة، مع إبقاء الكتابة الخادمية فقط.
- إضافة فحص أسرار مانع إلى بوابة الجودة وCI دون طباعة القيم المكتشفة.
- إضافة فحص ما بعد النشر لا ينشر شيئًا، ويتحقق من رقم النسخة الدقيق و`/api/health` والمسارات العربية والإنجليزية وManifest وrobots وsitemap وads.txt، مع إعادة المحاولة أثناء انتشار الإصدار.
- تحديث التقرير الشامل وسجل الاختبارات البشرية بحيث لا يبقى ضمن نطاق البرمجة الحالي بند مفتوح بصياغة قديمة.

الأخطاء المكتشفة:

1. **إمكان وصول عمليات كتابة محلية إلى خدمات الإنتاج عبر مسارات متفرقة**
   - الأعراض: كان منع مزامنة إعداد عام واحد لا يغطي كل استدعاءات Firestore المباشرة أو واجهات R2 وIndexNow.
   - السبب: لم توجد سياسة واحدة مشتركة بين المتصفح وMiddleware لكل العمليات المتغيرة.
   - الحل: إضافة `localMutationSafety` وتطبيقها في جميع ملفات الكتابة المعروفة، مع رفض واجهات الكتابة الحساسة مبكرًا على loopback واختبار HTTP فعلي.
   - الحالة: محلول محليًا؛ لا يدعي ذلك إنشاء Staging قابل للكتابة أو منع مستخدم يكتب شيفرة خاصة خارج التطبيق.

2. **طلبات خارجية بلا مهلة موحدة في بعض الواجهات**
   - الأعراض: قد تبقى واجهة الطقس أو رفع الوسائط في انتظار طويل عند بطء المزود.
   - السبب: استخدام `fetch` مباشرة دون AbortSignal ووقت نهائي واضح.
   - الحل: إضافة `fetchWithTimeout` وربطه بالطقس ورفع R2 من الإدارة والمعلنين، مع اختبارات للمهلة وأخطاء الاستجابة.
   - الحالة: محلول في المسارات الحالية؛ انقطاع المزود الحقيقي يبقى اختبارًا خارجيًا.

3. **سجل العمليات لم يغط كل تغييرات بوابة المعلنين**
   - الأعراض: بعض التسجيل والتفعيل والحملات والفريق ورفع الوسائط لم تكن تنتج حدثًا إداريًا موحدًا.
   - السبب: كانت التغطية مركزة على عمليات الإدارة فقط.
   - الحل: إضافة `/api/client/audit` بقائمة إجراءات وحقول مسموحة وهوية خادمية متحققة وربطه بالتدفقات الحالية.
   - الحالة: محلول للنطاق الحالي؛ العملية الأساسية وحدث التدقيق طلبان منفصلان وليسا معاملة ذرية واحدة.

4. **فحص الإنتاج السابق لم يثبت وصول النسخة المطلوبة نفسها**
   - الأعراض: كان نجاح Health عام قد يقبل نسخة قديمة أثناء انتشار النشر.
   - السبب: عدم مقارنة `/api/app-version` بإصدار مرشح محدد مع إعادة محاولة.
   - الحل: إضافة `post-deploy-health.mjs` وGitHub Workflow اختياري يتحققان من النسخة والمسارات وملفات النظام دون تنفيذ النشر.
   - الحالة: محلول برمجيًا؛ تشغيله على إصدار جديد يحتاج نشرًا مصرحًا أولًا.

5. **بناء Next.js رفض تصدير دالة مساعدة من Route**
   - الأعراض: فشل فحص الأنواع لأن `app/api/admin/indexnow/route.js` صدّر `normalizeIndexNowUrl` بجانب HTTP handler.
   - السبب: ملفات Route في App Router تقبل صادرات HTTP وإعدادات معروفة فقط، بينما أضيف التصدير المباشر لتسهيل اختبار SSRF.
   - الحل: نقل سياسات تطبيع روابط IndexNow وPageSpeed إلى `app/api/_lib/urlPolicies.js` واستيرادها في المسارين والاختبار.
   - الحالة: محلول؛ نجح الاختبار المستهدف ثم البناء وبوابة الجودة كاملة.

الملفات المتأثرة:

- `.github/workflows/post-deploy-health.yml`
- `app/api/_lib/urlPolicies.js`
- `app/localMutationSafety.js`
- `app/fetchWithTimeout.js`
- `app/advertiserAudit.js`
- `app/api/client/audit/route.js`
- `app/api/admin/cleanup/route.js`
- `app/api/admin/indexnow/route.js`
- `app/api/media/upload/route.js`
- `app/api/pagespeed/route.js`
...
- `app/client/localAdvertiserDemo.js`
- `app/weather/WeatherPageClient.jsx`
- `scripts/post-deploy-health.mjs`
- `tests/apiAbuseProtection.test.js`
- `tests/auditCoverage.test.js`
- `tests/fetchWithTimeout.test.js`
- `tests/localMutationSafety.test.js`
- `tests/postDeployHealth.test.js`
- `tests/securityBoundaries.test.js`
- `docs/HUMAN_VERIFICATION_CHECKLIST.md`
- `docs/OPERATIONS_RUNBOOK.md`
- `COMPREHENSIVE_AUDIT_CHECKLIST.md`
...
```powershell
npm test -- --run tests/localMutationSafety.test.js tests/securityBoundaries.test.js tests/mediaValidation.test.js tests/secretScan.test.js
npm test -- --run tests/apiAbuseProtection.test.js tests/fetchWithTimeout.test.js tests/firebaseAuthRetry.test.js tests/turnstileServer.test.js
npm test -- --run tests/auditCoverage.test.js tests/postDeployHealth.test.js
npm run lint
npm run security:secrets
npm run quality:local
Invoke-WebRequest http://127.0.0.1:3000/api/health
Invoke-WebRequest -Method POST http://127.0.0.1:3000/api/media/upload
```

الحالة:

- نجحت الاختبارات المستهدفة للحواجز المحلية وإساءة استخدام API والمهلات وسجل العمليات وفحص ما بعد النشر.
- أعاد Health المحلي `200`، وأعادت محاولة الكتابة المحلية إلى رفع الوسائط `403` بالرمز `local_production_mutation_blocked` قبل الوصول إلى R2.
- نجح ESLint وفحص الأسرار دون اكتشاف سر غير معتمد.
- نجحت بوابة الجودة الكاملة: تدقيق حزم الإنتاج بصفر ثغرات معروفة، و188 اختبارًا في 37 ملفًا، و8 اختبارات قواعد Firestore على Emulator وهمي، وبناء 39 صفحة، و49 اختبار متصفح مع اختبار واحد متروك عمدًا لتجنب تكرار فحص التكبير 200% على محاكاة الجوال.
- لم يُنشر أو يُدفع شيء، ولم تُرفع قواعد Firestore، ولم تتغير Cloudflare أو Firebase أو AdSense أو الحسابات أو رقم الإصدار.
- لا توجد مشكلة برمجية محلية معروفة تمنع بدء التنفيذ البشري من H-MFA-01.

المتبقي المحفوظ:

- البنود الخارجية البشرية: MFA للحسابات الحساسة، مراجعة IAM في Cloudflare وFirebase وGitHub وAdSense وSearch Console، وفحص أدوار الحسابات الفعلية.
- إعداد Cloudflare WAF وRate Limiting الموزع وHealth Checks والتنبيهات وفق إمكانات الخطة؛ لا يستبدل ذلك بعداد داخل ذاكرة Worker.
- اختبار رفع الوسائط من واجهة متصلة ببيئة اختبار لا تستخدم R2 الإنتاجي.
- اختبارات قارئ الشاشة والأجهزة والتكبير واللغتين والوضعين، وتثبيت PWA وتحديثه والروابط العميقة على المنصات الحقيقية.
- البنود المدفوعة: Firestore Backups أو PITR ونسخ أو Versioning لوسائط R2 وتجربة الاستعادة إلى مورد منفصل.
- البنود المنتظرة زمنيًا: تقارير Search Console وAdSense وCrUX/PageSpeed، إضافة إلى المراجعة القانونية البشرية.
- النشر وتشغيل فحص ما بعد النشر على إصدار جديد لا يتمان إلا بعد موافقة المستخدم الصريحة.

### بدء التحقق البشري من حساب Google الأساسي - دون تغييرات خارجية

تم إنجازه:

- فتح صفحة أمان حساب Google الأساسي وقراءة حالة وسائل الدخول فقط دون فتح رموز الاستعادة أو نسخ أي سر.
- إثبات أن التحقق بخطوتين مفعّل، وأن الحساب يحتوي مفتاحي مرور وAuthenticator وGoogle Prompt، وأن رموزًا احتياطية أُنشئت سابقًا.
- تحديث H-MFA-01 إلى حالة جارٍ بدل اعتباره مكتملًا اعتمادًا على الذاكرة.

الأخطاء المكتشفة:

1. **حماية الدخول قوية لكن اكتمال الاسترداد غير مثبت**
   - الأعراض: صفحة الأمان تعرض وسائل تحقق متعددة، لكنها تعرض أن بريد الاسترداد غير مضاف، ولا تستطيع الشيفرة إثبات مكان حفظ رموز الاستعادة.
   - السبب: حفظ الرموز خارج الجهاز واختيار جهة الاسترداد قراران بشريان لا يظهر دليلهما كاملًا في الشيفرة.
   - الحل: يثبت المستخدم حفظ الرموز خارج الجهاز الحالي، ثم يضيف بريد استرداد موثوق أو يوثق وسيلة استرداد مستقلة بديلة قبل نقل H-MFA-01 إلى مكتمل.
   - الحالة: جارٍ؛ لم يجر أي تعديل على الحساب.
...
```txt
فحص قراءة فقط لصفحة https://myaccount.google.com/security
```

الحالة:

- لا توجد كلمات مرور أو أرقام هاتف أو رموز استعادة أو مفاتيح مسجلة في ملفات المشروع.
- لم يُغيّر أي إعداد حساب، ولم يُنشر أو يُدفع أي تغيير في المشروع.

المتبقي المحفوظ:

- اكتمل H-MFA-01 بتاريخ 2026-09-15 بعد تأكيد حفظ رموز الاستعادة خارج الجهاز وإضافة بريد استرداد موثوق.
- يبدأ H-MFA-03 الخاص بحساب Cloudflare.

### إكمال MFA لحساب Google وبدء فحص Cloudflare - قراءة فقط

تم إنجازه:

- اعتماد H-MFA-01 مكتملًا بعد تأكيد المستخدم حفظ رموز Google الاحتياطية خارج الجهاز وإضافة بريد استرداد موثوق.
- فحص صفحة مصادقة Cloudflare وإثبات أن TOTP مفعّل وأن تذكير رموز الاستعادة مفعّل.
- فحص قائمة جلسات Cloudflare دون إلغاء أي جلسة أو تسجيل عنوان IP.

الأخطاء المكتشفة:

1. **جلسة Cloudflare أقدم تحتاج تعرفًا بشريًا**
   - الأعراض: توجد الجلسة الحالية وجلسة Windows أخرى شوهدت قبل ثلاثة أيام.
   - السبب: لا يمكن إثبات ملكية الجهاز من اسم النظام والموقع العام وحدهما.
   - الحل: يؤكد المستخدم إن كانت الجلسة معروفة؛ إن لم تكن، تلغى بعد تأكيد صريح عند لحظة الإلغاء ثم تراجع كلمة المرور والنشاط.
   - الحالة: جارٍ؛ لم تُلغ أي جلسة.

2. **حفظ رموز Cloudflare الاحتياطية غير موثق**
   - الأعراض: الحساب يعرض وجود وظيفة رموز احتياطية وتذكيرها، لكن مكان الحفظ لا يظهر للمنصة.
   - السبب: التخزين خارج الجهاز إجراء بشري.
   - الحل: يؤكد المستخدم أن الرموز محفوظة خارج الجهاز الحالي دون فتحها أو إرسالها.
   - الحالة: جارٍ.
...
```txt
فحص قراءة فقط لصفحة مصادقة Cloudflare وصفحة الجلسات النشطة
```
...
- H-MFA-01 مكتمل.
- H-MFA-03 جارٍ.
- لم يُغيّر أي إعداد أو صلاحية أو جلسة، ولم يُنشر أو يُدفع شيء.

المتبقي المحفوظ:

- اكتمل H-MFA-03 بعد تأكيد الجلسة الأقدم وحفظ رموز Cloudflare خارج الجهاز، وإثبات تفعيل مفتاح أمان وTOTP والبريد.
- يبدأ H-MFA-04 الخاص بـGitHub.

### إكمال MFA لـCloudflare وبدء فحص GitHub - قراءة فقط

تم إنجازه:

- إثبات أن Cloudflare محمي بمفتاح أمان وAuthenticator والبريد، مع رموز استعادة محفوظة خارج الجهاز وفق تأكيد المستخدم.
- اعتماد جلسة Cloudflare الأقدم كجلسة معروفة مرتبطة على الأرجح بمتصفح العمل السابق، دون إلغائها.
- فحص صفحة أمان GitHub وإثبات تفعيل Authenticator ووجود رموز استعادة سبق عرضها.
- فحص جلسات GitHub وإثبات وجود جلسة ويب واحدة حالية وعدم وجود جلسات GitHub Mobile.

الأخطاء المكتشفة:

1. **حفظ رموز GitHub خارج الجهاز غير مثبت بعد**
   - الأعراض: GitHub يثبت أن رموز الاستعادة سبق عرضها، لكنه لا يثبت مكان تخزينها.
   - السبب: التخزين المنفصل إجراء بشري.
   - الحل: يؤكد المستخدم حفظ رموز GitHub خارج الجهاز الحالي دون فتحها أو إرسالها.
   - الحالة: جارٍ.

2. **GitHub لا يحتوي كلمة مرور مستقلة أو Passkey**
   - الأعراض: الدخول مرتبط بحساب Google، والـAuthenticator هو وسيلة 2FA المفضلة.
   - السبب: الحساب أُنشئ أو يُستخدم عبر تسجيل الدخول الموحد مع Google.
   - الحل: حماية Google مكتملة، وإضافة Passkey إلى GitHub مستحسنة لتقليل الاعتماد على مزود واحد، لكنها ليست شرطًا مانعًا مع Authenticator ورموز استعادة محفوظة.
   - الحالة: ملاحظة تحسين، لا عطل حالي.
...
```txt
فحص قراءة فقط لوسائل مصادقة Cloudflare وصفحة أمان GitHub وجلساته النشطة
```
...
- H-MFA-03 مكتمل.
- H-MFA-04 جارٍ.
- لم تُفتح رموز استعادة، ولم تُسجل عناوين اتصال أو مواقع جلسات، ولم يتغير أي حساب أو صلاحية.
- لم يُنشر أو يُدفع شيء.

المتبقي المحفوظ:

- تأكيد حفظ رموز GitHub الاحتياطية خارج الجهاز الحالي.
- بعد اكتمال H-MFA-04 يبدأ فحص مسجل النطاق والبريد والاسترداد H-MFA-05 وH-MFA-06.

### إكمال MFA لـGitHub وبدء فحص مسجل النطاق - قراءة فقط

تم إنجازه:

- إثبات إضافة Passkey إلى GitHub مع بقاء Authenticator مفعّلًا، وتأكيد المستخدم حفظ رموز GitHub خارج الجهاز الحالي.
- اعتماد جلسة GitHub الوحيدة بوصفها الجلسة الحالية، وعدم وجود جلسات GitHub Mobile.
- الاستعلام من RDAP العام وتحديد Name.com كمسجل `date-tool.com` بدل Cloudflare Registrar.
- إثبات تفعيل قفل النقل `client transfer prohibited`، وأن انتهاء التسجيل في 2027-03-15 وخوادم الأسماء على Cloudflare.
- فتح صفحة تسجيل دخول Name.com للمستخدم دون إدخال أو قراءة بيانات الدخول.

الأخطاء المكتشفة:

1. **حساب Name.com غير مسجل الدخول في متصفح الفحص**
   - الأعراض: لا يمكن رؤية MFA أو التجديد التلقائي أو بريد الاسترداد أو حالة الدفع من الصفحة العامة.
   - السبب: هذه إعدادات خاصة بحساب المسجل ولا تظهر في RDAP.
   - الحل: يسجل المستخدم الدخول بنفسه في التبويب المفتوح، ثم ينفذ Codex فحص قراءة فقط قبل اقتراح أي تغيير.
   - الحالة: ينتظر تسجيل دخول المستخدم؛ قفل النقل مثبت ومفعّل خارجيًا.
...
```powershell
Invoke-RestMethod https://rdap.verisign.com/com/v1/domain/date-tool.com
```

الحالة:

- H-MFA-04 مكتمل.
- H-MFA-05 جارٍ وينتظر تسجيل الدخول إلى Name.com.
- لم تُفتح رموز أو تُسجل بيانات اتصال أو بريد، ولم يتغير أي حساب أو نطاق أو صلاحية.
- لم يُنشر أو يُدفع شيء.

المتبقي المحفوظ:

- تسجيل المستخدم الدخول إلى Name.com ثم فحص MFA والتجديد التلقائي والاسترداد والدفع.
- فحص حماية بريد العمل وبريد الاسترداد H-MFA-06 دون كشف عنوانهما.

### فحص أمان وتجديد Name.com - قراءة فقط

تم إنجازه:

- الدخول إلى حساب Name.com بواسطة المستخدم ثم فحص إعدادات النطاق والحساب قراءة فقط.
- إثبات أن قفل نقل `date-tool.com` وWHOIS Privacy مفعّلان، وأن الاستحقاق في 2027-03-15.
- إثبات أن استرداد كلمة المرور عبر بريد الحساب مسموح.
- تحديد الحالة الفعلية للتجديد التلقائي ووسيلة الدفع دون عرض أو تسجيل أي تفاصيل مالية.

الأخطاء المكتشفة:

1. **MFA غير مفعّل في Name.com**
   - الأعراض: صفحة النطاق تعرض طلب إعداد Two-Step Verification وصفحة الحساب تعرض زر إضافة Authenticator.
   - السبب: لم يربط الحساب تطبيق مصادقة بعد.
   - الحل: يفعّل المستخدم Authenticator بنفسه، ويدخل رمز التحقق، ويحفظ رموز الاستعادة خارج الجهاز دون مشاركتها.
   - الحالة: مفتوح وينتظر إجراء المستخدم.

2. **التجديد التلقائي غير جاهز**
   - الأعراض: Automatic Renewal يعرض `Action Required` ولا توجد وسيلة دفع محفوظة أو رصيد حساب.
   - السبب: التجديد التلقائي يحتاج ملف دفع افتراضيًا صالحًا وإعداد عملة التجديد.
   - الحل: بعد MFA يضيف المستخدم بطاقة أو PayPal بنفسه، ثم نتحقق من زوال التنبيه وتفعيل التجديد دون تسجيل بيانات الدفع.
   - الحالة: مفتوح؛ أي إضافة وسيلة دفع خطوة مالية بشرية منفصلة.

3. **إشعارات دخول Name.com غير مفعّلة**
   - الأعراض: إشعار نجاح الدخول وإشعار فشل الدخول كلاهما غير محدد.
   - السبب: الإعدادات الافتراضية للحساب.
   - الحل: تفعيل الإشعارين بعد إنهاء MFA، بعد تأكيد المستخدم عند لحظة التغيير.
   - الحالة: تحسين أمني مفتوح.
...
```txt
فحص قراءة فقط لصفحات النطاق وTwo-Step Verification وSecurity Settings وBilling and Renewal Settings في Name.com
```

الحالة:

- لم يتغير النطاق أو DNS أو الحساب أو الفوترة.
- لم تُعرض أو تُسجل رموز أو بيانات دفع أو عناوين اتصال.
- لم يُنشر أو يُدفع شيء في المشروع.

المتبقي المحفوظ:

- تفعيل Authenticator في Name.com وحفظ رموز الاستعادة.
- إضافة وسيلة دفع صالحة وتأكيد التجديد التلقائي، بإدخال المستخدم للبيانات بنفسه.
- تفعيل إشعارات نجاح وفشل الدخول بعد موافقة المستخدم.
- استكمال H-MFA-06 بعد التحقق من حماية بريد الاسترداد نفسه.

### إعادة التحقق من MFA في Name.com - قراءة فقط

تم إنجازه:

- إعادة تحميل صفحة إدارة Two-Step Verification بعد تأكيد المستخدم إكمال الإعداد.
- فحص الحالة النصية والمرئية دون فتح رمز QR أو قراءة مفتاح أو رمز تحقق.
- إبقاء H-MFA-05 في حالة جارٍ لأن الدليل الظاهر لا يثبت تفعيل Authenticator بعد.

الأخطاء المكتشفة:

1. **تأكيد المستخدم لا يطابق الحالة الظاهرة في Name.com**
   - الأعراض: صفحة الإدارة بعد إعادة التحميل ما زالت تعرض `Setup Authenticator App` ضمن إعداد وسيلة جديدة، ولا تعرض Authenticator مفعّلًا.
   - السبب: قد تكون عملية الربط لم تصل إلى خطوة إدخال رمز التطبيق والحفظ النهائي، أو لم يكتمل الطلب لدى Name.com.
   - الحل: يعيد المستخدم فتح زر الإعداد، ويمسح الرمز من هاتفه، ويدخل رمز التحقق، ثم ينتظر رسالة النجاح؛ بعد ذلك يعاد فحص الصفحة قراءة فقط.
   - الحالة: مفتوح؛ لا يعتمد MFA حتى يظهر دليل نجاح صريح من الحساب.
...
```txt
إعادة تحميل وفحص قراءة فقط لصفحة Two-Step Verification في Name.com
```

الحالة:

- لم تُفتح أو تُسجل مفاتيح Authenticator أو رموز تحقق أو رموز استعادة.
- لم يتغير النطاق أو الحساب أو الفوترة، ولم يُنشر أو يُدفع شيء.

المتبقي المحفوظ:

- إكمال ربط Authenticator حتى تظهر حالة نجاح صريحة، ثم تأكيد حفظ رموز الاستعادة خارج الجهاز.
- بعد ذلك إضافة وسيلة دفع صالحة وتأكيد التجديد التلقائي بإجراء المستخدم.
- تفعيل إشعارات نجاح وفشل الدخول بعد موافقة المستخدم عند لحظة التغيير.
- استكمال H-MFA-06 وفحوص الصلاحيات والوسائط والأجهزة وPWA والبنود الخارجية الأخرى بالترتيب المثبت في قائمة التحقق.

### إكمال MFA في Name.com - تحقق نهائي قراءة فقط

تم إنجازه:

- إعادة تحميل صفحة Two-Step Verification والتحقق من ظهور Authenticator ضمن `Your Current Credentials` بتاريخ 2026-09-15.
- إثبات توفر رموز الاستعادة دون فتحها، واعتماد تأكيد المستخدم أنها محفوظة خارج الجهاز.
- إغلاق مشكلة عدم تطابق التأكيد السابق مع الحالة المرئية، واعتماد جزء MFA من H-MFA-05 مكتملًا.

الأخطاء المكتشفة:

1. **لا يزال التجديد التلقائي غير جاهز**
   - الأعراض: نتيجة الفحص السابق تعرض `Action Required` ولا توجد وسيلة دفع محفوظة.
   - السبب: يحتاج Name.com إلى وسيلة دفع صالحة حتى يمكن الاعتماد على التجديد التلقائي.
   - الحل: يضيف المستخدم وسيلة الدفع بنفسه، ثم يعاد فحص حالة التجديد قراءة فقط دون تسجيل أي بيانات مالية.
   - الحالة: مفتوح؛ MFA مكتمل ولا يعتمد H-MFA-05 كاملًا قبل إثبات التجديد.
...
- MFA في Name.com مفعّل ورموز الاستعادة محفوظة خارج الجهاز وفق تأكيد المستخدم.
- لم تُفتح أو تُسجل رموز الاستعادة أو بيانات الاتصال أو أي بيانات مالية.
- لم يُنشر أو يُدفع شيء في المشروع.

المتبقي المحفوظ:

- إضافة وسيلة دفع صالحة وتأكيد التجديد التلقائي بإجراء المستخدم.
- تفعيل إشعارات نجاح وفشل الدخول بعد موافقة المستخدم عند لحظة التغيير.
- استكمال H-MFA-06 ثم بقية الاختبارات البشرية بالترتيب المثبت في القائمة.

### تصحيح المدينة الافتراضية ودقة الموقع في الطقس والساعة - محلي فقط

تم إنجازه:

- إزالة الرياض ولندن والمنطقة الزمنية `Asia/Riyadh` من الحالة الابتدائية لصفحة الساعة؛ تعرض الساعة `--:--` حتى تتحدد المدينة.
- جعل حقول المدن والبحث عن الطقس فارغة عند التحميل، وإزالة أمثلة المدن الافتراضية من النصوص.
- طلب إحداثيات حديثة بدقة عالية، واستخدام الإحداثيات لاستخراج اسم المدينة والمنطقة الزمنية بدل استنتاجهما من إعدادات الجهاز.
- عند ضعف دقة الإحداثيات أو فشل التعرف على المدينة، يظهر وصف `موقعك التقريبي` أو `موقعك الحالي` بدل اسم مدينة قد لا يكون صحيحًا.
- منع إعادة استخدام بيانات الطقس القديمة إذا فشلت محاولة تحديد الموقع الجديدة.
- فتح معاينتَي `http://127.0.0.1:3000/weather` و`http://127.0.0.1:3000/clock` للمستخدم قبل النشر.

الأخطاء المكتشفة:

1. **ظهور الرياض قبل التعرف على الموقع**
   - الأعراض: تعرض صفحة الساعة الرياض وتوقيتها حتى قبل السماح للموقع، وقد يبقى اسم منطقة بدل المدينة الصحيحة.
   - السبب: قيم ثابتة في الحالة الابتدائية للساعة، مع استخدام المنطقة الزمنية للجهاز أو المنطقة الإدارية بوصفها مدينة احتياطية.
   - الحل: بدء الواجهة دون مدينة؛ الاعتماد على إحداثيات حديثة، وتسمية الموقع التقريبي بوضوح عند عدم كفاية الدقة.
   - الحالة: مصحح ومختبر محليًا؛ ينتظر تجربة المستخدم، ولم ينشر.
2. **بقاء طقس سابق بعد فشل المحاولة الجديدة**
   - الأعراض: قد يرى المستخدم طقس الموقع السابق بعد فشل إعادة تحديد الموقع.
   - السبب: الرجوع إلى `currentLocation` القديم عند فشل الطلب الجديد.
   - الحل: مسح النتيجة القديمة وعدم إعادة استخدامها في المحاولة القسرية.
   - الحالة: مصحح محليًا؛ لم ينشر.

الملفات المتأثرة:

- `app/SiteShell.jsx`
- `app/locationDisplay.js`
- `app/clock/ClockPageClient.jsx`
- `app/weather/WeatherPageClient.jsx`
- `tests/locationDisplay.test.js`
- `e2e/location-quality.spec.js`
- `PROJECT_MEMO.md`
...
npm run lint
npm run build
npm run test:e2e -- e2e/location-quality.spec.js
npm run dev
```

الحالة:

- نجح 204 اختبار وحدة، وفحص ESLint، والبناء، و4 اختبارات متصفح على سطح المكتب والجوال.
- معاينة محلية فقط على المنفذ 3000؛ لا نشر ولا دفع إلى GitHub.

المتبقي المحفوظ:

- تجربة المستخدم لتحديد الموقع على جهازه والتحقق من اسم مدينته الفعلي قبل أي نشر.
- بنود الجاهزية والاختبارات البشرية السابقة كما هي؛ لم تغير هذه المهمة حالتها.

### توحيد واجهات التاريخ والطقس والساعة وإصلاح اسم المدينة - محلي فقط

تم إنجازه:

- توحيد لون واجهات الأدوات الثلاث على أزرق الهوية والأسطح المحايدة، بما فيها شريط اليوم والساعة وأزرار الإجراءات ووضعَي التاريخ.
- إعادة تشكيل سكيلتون الغلاف بحسب الصفحة: الطقس يعرض البحث وبطاقة الطقس، والساعة تعرض شريط الوقت وأداة التحويل، والتاريخ يحتفظ ببنية التاريخ والأحداث. تطابق ألوان السكيلتون الأسطح الفعلية في الوضعين الفاتح والداكن.
- تحويل اسم المدينة من الإحداثيات عبر خدمة BigDataCloud من المتصفح، وتجربة نطاقها الرسمي البديل عند تعثر النطاق الأول، مع رفض تسمية المدينة المستنتجة من IP أو دقة موقع غير كافية.
- إزالة عبارة `موقعك الحالي` من نتيجة الساعة والطقس: يظهر اسم المدينة عند تأكيده، ولا يُعرض اسم مدينة مخمّن إذا فشلت خدمتا التسمية.
- إبقاء الصفحات والوظائف والمواضع الإعلانية كما هي، دون نشر.

الأخطاء المكتشفة:

1. **نتيجة الموقع تعرض وصفًا عامًا بدل اسم المدينة**
   - الأعراض: يظهر `موقعك الحالي` في الساعة والطقس رغم حصول الصفحة على إحداثيات الطقس.
   - السبب: انتهاء مهلة خدمة التسمية أو تعذر اتصالها، ثم استخدام نص احتياطي عام؛ بطاقة الطقس تضيف كذلك عبارة `حسب موقعك`.
   - الحل: زيادة مهلة الطلب، وتجربة نطاق رسمي بديل للخدمة نفسها، وعرض اسم المدينة وحده عندما يتأكد. عند استمرار الفشل لا تُخمن المدينة.
   - الحالة: مصحح ومختبر محليًا؛ ينتظر تجربة المستخدم.
2. **سيلكتون الصفحات وألوانه غير متطابقة مع المحتوى**
   - الأعراض: صفحات الطقس والساعة تستخدم هيكل انتظار شبيهًا بصفحة التاريخ وتظهر ألوان بنفسجية لا تطابق الهوية.
   - السبب: سكيلتون عام وألوان تدرج مختلفة في القواعد المشتركة.
   - الحل: هيكل انتظار خاص بكل صفحة وتوحيد متغيرات اللون في CSS.
   - الحالة: مصحح محليًا وفُحص بصريًا على مقاسي الهاتف وسطح المكتب.
...
- نجح 204 اختبار وحدة، وفحص ESLint، والبناء، و6 اختبارات متصفح على سطح المكتب والجوال.
- فُحصت لقطات المعاينة للصفحات الثلاث وهيكل تحميل الطقس محليًا. لم يُنشر أو يُدفع شيء.

المتبقي المحفوظ:

- تجربة المستخدم الفعلية لتسمية مدينته في صفحة الساعة والطقس قبل أي نشر؛ إذا لم تتوفر خدمة التسمية تبقى المدينة غير معروضة بدل إظهار اسم خاطئ.
- بقية بنود الجاهزية والاختبارات البشرية السابقة دون تغيير.

### استعادة واجهة الطقس المعتمدة بدقة - محلي فقط

تم إنجازه:

- التراجع الكامل عن محاولتي تعديل ألوان وهيكل سكيلتون الطقس الأخيرتين، وإعادة مكونات التحميل إلى شكلها السابق قبل تلك المحاولات.
- إعادة `app/globals.css` إلى مرجع ألوان وتصميم ما قبل الإصدار `0.3.60`، ثم إضافة تصحيح محدد لا يغيّر التصميم: تطبيق ألوان التحميل الداكن نفسها من أول لحظة عند وجود `data-site-theme='dark'` قبل اكتمال إضافة `dark-mode` إلى body.
- إعادة دوائر ومربعات وخطوط السكيلتون الأصلية ومقاساتها للجوال والكمبيوتر، وإزالة النصوص المخفية والأزرار المعطلة التي استُخدمت كعناصر تحميل.
- الإبقاء على إصلاحات تحديد اسم المدينة والخدمات الاحتياطية لأنها منطقية ولا تغيّر الهوية البصرية.

الأخطاء المكتشفة:

1. **تحويل السكيلتون إلى محتوى مخفي بدل عناصر تحميل**
   - الأعراض: ظهرت أشكال وأحجام غير منطقية وأزرار فعلية معطلة أثناء التحميل.
   - السبب: محاولة مطابقة الحجم باستعمال النصوص والأزرار النهائية نفسها مع إخفاء النص.
   - الحل: استعادة مكونات السكيلتون الأصلية حرفيًا وحذف القواعد المستحدثة.
   - الحالة: محلول محليًا.
2. **ومضة فاتحة داخل التحميل الداكن**
   - الأعراض: مؤشرات الطقس ولمعة السكيلتون تظهر فاتحة للحظات في الوضع الداكن ثم تتحول إلى اللون الصحيح.
   - السبب: قواعد التحميل الداكن اعتمدت على `body.dark-mode` فقط، بينما `html[data-site-theme='dark']` يصبح جاهزًا قبله.
   - الحل: توسيع المحددات الداكنة الموجودة لتعمل بالحالتين مع إبقاء قيم الألوان الأصلية دون تغيير.
   - الحالة: محلول ومختبر بصريًا.
...
```powershell
git diff c60d8ce^ -- app/globals.css
npm run lint
$env:PLAYWRIGHT_BASE_URL='http://127.0.0.1:3000'
node node_modules/@playwright/test/cli.js test e2e/weather-visual-restore.spec.js
node node_modules/@playwright/test/cli.js test e2e/location-quality.spec.js
npm test
git diff --check
```
...
- ملف CSS مطابق للمرجع القديم باستثناء أربعة محددات إضافية فقط تمنع ومضة التحميل الفاتحة في الوضع الداكن.
- نجحت أربعة فحوص بصرية مؤقتة: الجوال والكمبيوتر في الوضعين الفاتح والداكن. تطابقت خلفية البطاقة، نصف قطرها، عرضها وارتفاعها، وخلفيات وحدود وأحجام مؤشرات الطقس قبل وصول البيانات وبعده، ولم يوجد تجاوز لعرض الشاشة.
- نجح ESLint و209 اختبارات وحدة و8 اختبارات موقع ومدينة على الكمبيوتر والجوال.
- أزيل ملف الفحص البصري المؤقت بعد إتمام التحقق، ولم يُضف اختبار دائم أو يُنشر شيء.

المتبقي المحفوظ:

- مراجعة المستخدم محليًا. بقية بنود الجاهزية والخدمات الخارجية السابقة دون تغيير.
- القسمان التاليان يوثقان المحاولتين اللتين تم التراجع عنهما بالكامل في هذه المرحلة، وليستا الحالة الحالية للتصميم.

### نشر 0.3.61 وفحص الأمان وGoogle - مكتمل

تم إنجازه:

- رفع رقم النسخة محليًا إلى `0.3.61` وتسجيل استعادة واجهة الطقس المعتمدة وإصلاحات تسمية المدينة في `VERSION_LOG.md`، ثم دفع الكومت `771ea7f` إلى `origin/master`.
- نجاح Quality Gate وCodeQL للكومت `771ea7f` على GitHub.
- أغلق CodeQL التنبيهين المرتفعين السابقين بعد استبدال التعبيرين المنتظمين العامين، ثم كشف تنبيهًا ثالثًا عن فحص نطاق رابط Sitemap بمطابقة نصية غير مكتملة. عولج التنبيه الثالث بتحليل URL والتحقق من `origin` والمسار كاملين لمنع قبول نطاقات متشابهة خبيثة.
- نجاح ESLint وفحص الأسرار و210 اختبارات وحدة، ونجاح `npm audit --omit=dev --audit-level=high` بلا ثغرات معروفة.
- نجاح CodeQL رقم `21` وإغلاق التنبيهات الثلاثة؛ صفحة Code Scanning تعرض `0 Open` و`10 Closed`. نجحت أيضًا إعادة Quality Gate رقم `8` بعد تعثر محاولة البناء الأولى باتصال خط Google عارض.
- مراجعة Search Console بتاريخ 2026-09-24: خريطة `sitemap.xml` ناجحة، وآخر قراءة في اليوم نفسه، وتضم 28 عنوانًا؛ 10 صفحات مفهرسة و18 مكتشفة لم تُزحف بعد. لا توجد إجراءات يدوية أو مشاكل أمان.
- توثيق حالات Search Console غير المفهرسة: `/index.html` و`http://www.date-tool.com/` تحويلان متوقعان، و`http://date-tool.com/` بديل Canonical متوقع، و`/after-14-days.html` صفحة قديمة محذوفة، و`/ad_request.html` أثر قديم زُحف آخر مرة في 2026-03-31.
- مراجعة AdSense بتاريخ 2026-09-24: إثبات ملكية الموقع ناجح و`ads.txt` معتمد، ومركز السياسات لا يعرض مشكلة توقف عرض الإعلانات. سبب رفض الجاهزية هو `محتوى منخفض القيمة`، وإعادة المراجعة محظورة حتى 2026-09-25 بسبب بلوغ حد المحاولات.
- التحقق من أن الموقع المنشور يحمل وسم `google-adsense-account`، بينما سكربت وحدات AdSense غير محمل أثناء حالة المراجعة، كما تقضي حماية الإعلانات الحالية.

الأخطاء المكتشفة:

1. **ثلاثة تنبيهات مرتفعة من CodeQL في فحص ما بعد النشر**
   - الأعراض: تنبيهان من نوع `Missing regular expression anchor` في فحصي `robots.txt` و`sitemap.xml`، ثم تنبيه `Incomplete URL substring sanitization` في فحص رابط النسخة الإنجليزية داخل Sitemap.
   - السبب: استخدام تعبيرين منتظمين للبحث في استجابات نصية كاملة، ثم استخدام مطابقة نصية جزئية لنطاق URL.
   - الحل: استبدال المطابقات الثابتة بـ`includes`، وتحليل رابط Sitemap باستخدام `URL` مع مقارنة `origin` والمسار كاملين، وإضافة اختبار يرفض `https://date-tool.com.evil/en`.
   - الحالة: محلول ومرفوع؛ CodeQL رقم `21` ناجح ولا توجد تنبيهات مفتوحة.
2. **AdSense يرفض الموقع بسبب محتوى منخفض القيمة**
   - الأعراض: حالة الموقع `يتطلب عناية` وظهور سبب `محتوى منخفض القيمة`، مع منع طلب مراجعة جديدة حتى 2026-09-25.
   - السبب المعلن من Google: الحاجة إلى قيمة فريدة أكبر، وصيانة محتوى مستمرة، واهتمام مستخدمين حقيقي. يتوافق ذلك مع بقاء 18 من 28 صفحة في Search Console بحالة مكتشفة ولم تُزحف بعد.
   - الحل: عدم إعادة الطلب فور فتح المهلة؛ إضافة محتوى أصلي عميق لكل صفحة أداة يشمل أمثلة وطريقة الحساب وحدود الدقة والمصادر، وتحسين الإشارات الداخلية والاستخدام الفعلي، ثم إعادة المراجعة بعد انتشار التعديلات.
   - الحالة: مفتوح كمشروع محتوى؛ لا توجد مشكلة في إثبات الملكية أو `ads.txt` أو مركز السياسات.
3. **اضطرابات اتصال عارضة أثناء بوابة الجودة**
   - الأعراض: فشل `npm run quality:local` محليًا مرتين في خطوة audit برسالة `ECONNRESET`، وتعثر أول تشغيل لـQuality Gate على GitHub أثناء بناء `next/font` بخطأ في محمل خط Cairo.
   - السبب: انقطاعان مؤقتان في خدمتي سجل npm وخط Google، لا فشل في الاختبارات أو الشيفرة.
   - الحل: نُفذ تدقيق الاعتماديات مستقلًا ونجح بلا ثغرات، ثم أُعيد تشغيل Quality Gate نفسها دون تغيير الشيفرة فنجحت بكامل خطواتها.
   - الحالة: محلول ومثبت بإعادة ناجحة؛ لا يحتاج تغيير الخط أو التصميم.
...
npm run security:secrets
npm run build
npm test -- --run tests/postDeployHealth.test.js
npm test -- --run
npm run lint
npm audit --omit=dev --audit-level=high
npm run quality:local
git push origin master
Invoke-RestMethod https://date-tool.com/api/app-version
```

الحالة:

- نُشرت `0.3.61` تلقائيًا عبر تكامل GitHub مع Cloudflare، وتحقق فحص الصحة من الإصدار المنشور بنجاح من المحاولة الأولى.
- لم تُنشأ وصلة OAuth جديدة لـWrangler؛ لم تعد مطلوبة بعد نجاح النشر التلقائي، وبقيت صلاحيات Cloudflare دون توسيع.
- CodeQL رقم `21` ناجح مع صفر تنبيهات مفتوحة، وإعادة Quality Gate رقم `8` ناجحة.
- لم يُرسل طلب مراجعة جديد إلى AdSense، ولم يبدأ تحقق إصلاح جديد في Search Console.

المتبقي المحفوظ:

- تنفيذ خطة محتوى AdSense قبل إعادة المراجعة، مع إبقاء وحدات AdSense متوقفة حتى القبول.
- بقية بنود الجاهزية التي تحتاج خطة مدفوعة أو تقارير زمنية أو اختبار أجهزة فعلية تبقى كما هي.

### إبقاء تصميم بطاقات الطقس أثناء التحميل - محلي فقط

تم إنجازه:

- إلغاء تغيير خلفية بطاقات انتظار الطقس؛ تظهر بنفس خلفيات وتدرجات النتائج الفعلية، وكذلك خلفيات مؤشرات الرطوبة والرياح والمطر وUV.
- إبقاء العناوين والتسميات والأيقونات الثابتة ظاهرة؛ يقتصر السكيلتون على بيانات المدينة والحرارة والقيم والنصيحة والتوقعات المنتظرة.
- استخدام عناصر النصوص النهائية نفسها ومقاساتها الموروثة بدل ارتفاعات التحميل الثابتة، مع لون موحد وأطراف مناسبة للنصوص.
- حجز زر المشاركة بحجمه وتصميمه المعتاد عندما يكون مفعّلًا، وتعطيله أثناء التحميل.
- إزالة الحدود من مؤشرات الطقس في الوضع الداكن، سواء عُيّن الوضع على body أو html.

الأخطاء المكتشفة:

1. **اختلاف خلفية وحجم محتوى البطاقة أثناء التحميل**
   - الأعراض: خلفيات انتظار محايدة بدل التدرجات الأصلية، ومربعات صغيرة تستبدل نصوصًا كبيرة، وغياب مساحة المشاركة.
   - السبب: قواعد التحميل السابقة بدلت خلفيات الحاويات واستخدمت مقاسات ثابتة مستقلة عن العناصر النهائية.
   - الحل: إبقاء الحاويات الأصلية واستخدام h3 وp وstrong وغيرها نفسها مع سكيلتون نصي يرث مقاساتها، وإظهار زر المشاركة المعطل عند تفعيله.
   - الحالة: محلول محليًا ومختبر.

الملفات المتأثرة:

- `app/weather/WeatherPageClient.jsx`
- `app/globals.css`
- `e2e/weather-loading.spec.js`
- `PROJECT_MEMO.md`
...
```powershell
$env:PLAYWRIGHT_BASE_URL='http://127.0.0.1:3000'
node node_modules/@playwright/test/cli.js test e2e/weather-loading.spec.js
npm run lint
git diff --check
```
...
- نجحت اختبارات المتصفح الأربعة: الجوال والكمبيوتر بالوضعين الفاتح والداكن؛ جرى تأخير استجابة طقس وهمية للتحقق من حالة الانتظار ثم إطلاقها وفحص النتائج.
- تطابقت خلفيات بطاقة الطقس والمؤشرات وارتفاعات الحرارة والمؤشرات قبل ظهور البيانات وبعده؛ ألوان السكيلتون موحدة والحدود الداكنة معدومة، ولا تجاوز لعرض الشاشة.
- نجح ESLint وفحص الفرق، وفُحصت لقطة تحميل الجوال الداكن بصريًا.
- لا نشر ولا دفع إلى GitHub؛ لا تغيير لمنطق الموقع أو بيانات الحسابات.

المتبقي المحفوظ:

- مراجعة المستخدم محليًا؛ بقية البنود السابقة دون تغيير.

### توحيد ألوان سكيلتون الطقس - محلي فقط

تم إنجازه:

- توحيد ألوان عناصر التحميل بين غلاف الموقع وبطاقات انتظار الطقس باستخدام ألوان الخلفية والحدود المعتمدة للوضع الحالي.
- جعل خلفيات بطاقات انتظار الطقس ومؤشراتها هادئة؛ لا تغيير لألوان النتائج أو الأحجام أو ترتيب الصفحة أو منطق الموقع الجغرافي.
- الإبقاء على مرحلتي تحميل إعدادات الموقع وبيانات الطقس؛ التغيير لوني فقط حسب طلب المستخدم.

الأخطاء المكتشفة:

1. **اختلاف ألوان مرحلتي التحميل**
   - الأعراض: يظهر تحميل الطقس على مرحلتين بألوان مختلفة.
   - السبب: بعض عناصر سكيلتون الغلاف تحمل تدرجات خاصة، وبطاقة انتظار الطقس ترث خلفية بطاقة النتيجة الملونة.
   - الحل: قواعد ألوان مشتركة محصورة بعناصر التحميل وخلفيات خاصة ببطاقات الانتظار.
   - الحالة: محلول محليًا، دون نشر.
...
- فحص Playwright مؤقت عبر `node` يقارن الألوان المحسوبة لمرحلتي التحميل وخلفية النتيجة مع CSS النسخة السابقة.
- `git diff --check`

الحالة:

- نجحت الحالات الأربع: عرض 390 و1280 بكسل، بالوضعين الفاتح والداكن؛ ألوان السكيلتون متطابقة وخلفية النتيجة لم تتغير.
- التعديل محلي فقط، دون نشر أو دفع إلى GitHub.

المتبقي المحفوظ:

- مراجعة المستخدم محليًا؛ جميع البنود السابقة غير المتعلقة بهذا التعديل باقية كما هي.

### استعادة التصميم السابق وتصحيح اسم المدينة - محلي فقط

تم إنجازه:

- إعادة `app/globals.css` حرفيًا إلى حالته قبل `c60d8ce`، بما يشمل التدرجات والأخضر الهجري وألوان التحميل وزر صيغة الساعة؛ تحقق عدم وجود فرق مع الملف السابق.
- إعادة هيكل `PublicShellSkeleton` السابق، وإزالة إعادة التصميم التي لم يقصدها المستخدم.
- إزالة شرط حجب المدينة عندما تتجاوز دقة تحديد الموقع 20 كم؛ يظهر الاسم المستخرج من الإحداثيات مع توضيح أنه تقريبي عند ضعف الدقة.
- الإبقاء على عدم عرض أي مدينة افتراضية، ومنع استخدام اسم المنطقة الزمنية أو نتيجة IP كمدينة مؤكدة.
- إضافة Photon/OpenStreetMap كخدمة احتياطية مستقلة عند تعذر BigDataCloud، بعد الاطلاع على التوثيق الرسمي وتجربة إحداثيات عامة لمركز تبوك وإرجاع `تبوك` فعليًا.
- تقريب إحداثيات خدمة Photon إلى ثلاث منازل عشرية، وعدم إرسال بيانات اعتماد إليها أو حفظ إحداثيات المستخدم في قاعدة البيانات؛ إضافة إسناد مصدر البيانات في الفوتر عند استخدام الخدمة الاحتياطية فقط.
- منع ازدواج طلب الموقع بين غلاف الصفحة والطقس بعد الاستعلام المتزامن عن الإذن، وتحديث نتيجة الطقس عند تغير اسم المدينة حتى لو بقيت الإحداثيات نفسها.
- عرض رسالة تعذر التسمية عند فشل الخدمات جميعها بدل إخفاء اسم المدينة بصمت، مع بقاء إعادة تحديد الموقع والبحث اليدوي متاحين.

الأخطاء المكتشفة:

1. **تغيير التصميم خارج قصد المستخدم**
   - الأعراض: أزيلت التدرجات والأخضر الخاص بالتاريخ الهجري وتغيرت ألوان البطاقات والتحميل.
   - السبب: تفسير إعادة البناء كإعادة تصميم.
   - الحل: استعادة CSS وهيكل التحميل السابقين وتثبيت توجيه الحفاظ على التصميم في المذكرة.
   - الحالة: محلول محليًا، لم يُنشر.
2. **حجب اسم المدينة بسبب الدقة التقريبية**
   - الأعراض: يتوفر الطقس ولا يظهر اسم المدينة.
   - السبب: منع استدعاء التسمية وإرجاع اسم فارغ عند دقة تتجاوز 20 كم.
   - الحل: تسمية إحداثيات المتصفح مع توضيح الدقة التقريبية، واختبار حالة دقة 50 كم.
   - الحالة: محلول في الشيفرة والاختبارات.
3. **تعذر اتصال المتصفح المدمج بخدمات التسمية**
   - الأعراض: `Failed to fetch` في الطلبات و`ERR_BLOCKED_BY_CLIENT` عند فتح رابط الخدمة مباشرة.
   - السبب: حجب الاتصال في بيئة المتصفح المدمج؛ لا يوجد دليل يحدد أي إضافة أو إعداد بعينه. رؤوس CSP تسمح بالنطاقات، والاتصال من الطرفية نجح.
   - الحل: إضافة خدمة مستقلة واختبارها من الطرفية بإحداثيات عامة، ومحاكاة فشل المصدر الأول في اختبارات المتصفح. المتصفح المدمج حجب فتح نطاق Photon أيضًا؛ لم تُعطّل أي حماية أو إعداد متصفح.
   - الحالة: منطق الخدمات والواجهة مختبر؛ تأكيد اسم موقع المستخدم الفعلي يحتاج تجربته في متصفحه المعتاد.
...
npm test
$env:PLAYWRIGHT_BASE_URL='http://127.0.0.1:3000'
node node_modules/@playwright/test/cli.js test e2e/location-quality.spec.js e2e/public-quality.spec.js --grep 'clock|weather|public page quality'
git diff c60d8ce^ -- app/globals.css
git diff --check
```
...
- نجح ESLint و209 اختبارات وحدة في 40 ملفًا و30 اختبار متصفح على الكمبيوتر والجوال.
- CSS مطابق تمامًا للنسخة السابقة. فحص تباين الصفحة الإنجليزية نجح أيضًا بالألوان الأصلية.
- فُحصت ألوان صفحة التاريخ بصريًا، واختُبرت تسمية تبوك الحقيقية من Photon بإحداثيات عامة، وليس موقع المستخدم.
- لا نشر ولا دفع إلى GitHub؛ الإنتاج ما زال على `0.3.60` المنشور سابقًا.

المتبقي المحفوظ:

- تجربة المستخدم لاسم مدينته الفعلي في Chrome أو متصفحه المعتاد على `http://127.0.0.1:3000/weather` و`/clock`.
- Photon العام لا يضمن التوفر بلا انقطاع؛ يُستخدم كطلب احتياطي محدود فقط. العربية تستخدم الاسم المحلي لأن خدمة Photon العامة تدعم `default/en/de/fr` ولا تدعم `ar` مباشرة.
- التعديلات محلية للمراجعة؛ بنود الجاهزية والخدمات الخارجية السابقة كما هي.

### نشر تحسينات الموقع والهوية - الإصدار 0.3.60

تم إنجازه:

- رفع إصدار التطبيق إلى `0.3.60` بتاريخ 2026-09-24، مع إبقاء نسختَي الإدارة والمعلنين دون تغيير.
- نشر تحسينات صفحات التاريخ والساعة والطقس على Worker `datetools` بإصدار Cloudflare `76fce306-7610-4117-963b-bbd367a293cc`.
- إزالة اسم المدينة الافتراضية قبل محاولة تحديد الموقع، وإظهار اسم مدينة مؤكد من الإحداثيات في الساعة والطقس عند توفره دون التخمين عند فشل التسمية.
- توحيد ألوان الأدوات وهياكل التحميل مع الهوية، والحفاظ على مواضع الإعلانات والوظائف القائمة.
- التحقق حيًا من `https://date-tool.com` وظهور الإصدار `0.3.60` عبر فحص الصحة العام.

الأخطاء المكتشفة:

1. **تباين نص زر صيغة الساعة الإنجليزية غير كافٍ**
   - الأعراض: فشل فحص الوصولية في `/en/clock` على الكمبيوتر والجوال بنسبة تباين أقل من `4.5:1`.
   - السبب: خلفية شفافة ونص صغير شبه شفاف فوق شريط أزرق فاتح.
   - الحل: لون خلفية أغمق ونص أبيض كامل الوضوح؛ أعيدت بوابة الجودة ونجحت.
   - الحالة: محلول ومنشور.
...
```powershell
npm version 0.3.60 --no-git-tag-version
npm run quality:local
npx wrangler whoami
npm run deploy
...
- نجحت بوابة الجودة كاملة: تدقيق تبعيات الإنتاج بلا ثغرات معروفة، و204 اختبارات وحدة، و8 اختبارات قواعد Firestore، وبناء Next.js، و55 اختبار متصفح ناجحًا مع اختبار واحد متجاوز عمدًا.
- نُشر Worker `datetools`، واجتاز الموقع الأساسي فحص الصحة والإصدار `0.3.60`.
- لم تتغير قواعد Firestore أو إعدادات الإنتاج الخارجية في هذا الإصدار.

المتبقي المحفوظ:

- تحقق المستخدم من اسم مدينته الفعلي على جهازه بعد السماح بالموقع؛ عند تعذر تأكيد المدينة لا يظهر اسم مخمّن.
- بنود الجاهزية والاختبارات البشرية والخدمات الخارجية السابقة كما هي.

### نشر الطقس بالموقع الحالي ومراجعة رسائل Google - الإصدار 0.3.59

تم إنجازه:

- مراجعة رسالة Search Console بتاريخ 2026-09-16 وتحديد السببين الفعليين: صفحة تتضمن إعادة توجيه، وصفحة لم يتم العثور عليها.
- فحص أمثلة Search Console: `/index.html` هو مسار التحويل، و`/after-14-days.html` هو المسار القديم المحذوف.
- التحقق بعد النشر من أن `/index.html` يعيد `308` إلى `/` وأن `/after-14-days.html` يعيد `410 Gone` مع `X-Robots-Tag: noindex, nofollow, noarchive`.
- مراجعة AdSense وتحديد الحالة الفعلية `يتطلب عناية` بسبب `محتوى منخفض القيمة`، مع بقاء `ads.txt` معتمدًا وملكية الموقع مثبتة.
- إثبات أن AdSense يمنع طلب مراجعة جديدة حتى 2026-09-25 بسبب بلوغ الحد الأقصى لمحاولات المراجعة.
- إبقاء وحدات AdSense محجوبة برمجيًا حتى تصبح حالة الموقع معتمدة، مع الحفاظ على المواضع التسعة وإعداداتها دون تغيير.
- تعديل صفحة الطقس لتطلب الموقع الحالي عند التحميل بدل استخدام الرياض افتراضيًا.
- عند رفض إذن الموقع أو تعذر تحديده، لا تعرض الصفحة طقس مدينة بديلة غير حقيقية، وتبقي البحث اليدوي متاحًا مع رسالة واضحة.
- إضافة اختبار يثبت عدم عودة fallback الرياض وثبات البحث اليدوي.
- رفع إصدار التطبيق إلى `0.3.59` بتاريخ 2026-09-21 ونشره على Worker `datetools`.
- التحقق من `/api/app-version` على النطاق الأساسي ونطاق Worker، ومن استجابة `/weather` و`/en/weather` بحالة `200`.
- تشغيل اختبارات قواعد Firestore داخل مشروع المحاكاة المعزول ونجاح الاختبارات الثمانية.
- نشر `firestore.rules` إلى مشروع `date-tool-official` بعد نجاح التجميع، لتتطابق صلاحيات `platform_owner` وبقية الأدوار مع الكود المنشور.

الأخطاء المكتشفة:

1. **Search Console يحتفظ بأمثلة تاريخية لمسارين قديمين**
   - الأعراض: التقرير يعرض صفحة إعادة توجيه وصفحة 404 واحدة.
   - السبب: Google زحف إلى `/index.html` و`/after-14-days.html` قبل اعتماد الحالات النهائية للمسارين.
   - الحل: التحويل الدائم للمسار الأول وإرجاع `410 + noindex` للمسار المنتهي، مع إبقائهما خارج خريطة الموقع.
   - الحالة: محلول برمجيًا ومتحقق منه حيًا؛ تحديث التقرير ينتظر إعادة زحف Google.

2. **رفض AdSense بسبب محتوى منخفض القيمة**
   - الأعراض: الموقع يعرض `يتطلب عناية` و`محتوى منخفض القيمة` رغم اعتماد `ads.txt` وإثبات الملكية.
   - السبب: تقييم Google يرى أن القيمة الفريدة أو الصيانة المستمرة أو اهتمام المستخدمين لم تصل بعد إلى مستوى القبول.
   - الحل: الاستمرار في تحسين المحتوى الفريد وتجربة الأدوات والفهرسة والزيارات الحقيقية، ثم طلب مراجعة جديدة بعد فتحها في 2026-09-25.
   - الحالة: خارجي ومفتوح؛ لا يوجد خلل في مواضع الإعلانات، ولا يجوز تشغيل الوحدات أو إرسال مراجعة قبل الموعد.

3. **فحص الصحة المجمع من الطرفية تأثر بانقطاعات اتصال محلية**
   - الأعراض: `health:production` وبعض طلبات `curl` علقت أو أعادت connection reset.
   - السبب: اتصال الطرفية بالنطاق كان غير مستقر خلال الفحص، بينما النشر نفسه واختبارات لاحقة منفردة نجحت.
   - الحل: إيقاف العملية المعلقة والتحقق بطلبات قصيرة منفصلة من نطاق Worker والنطاق الأساسي.
   - الحالة: لا يؤثر في الموقع؛ الإصدار وصفحات الطقس ومسارات Search Console تحققت حيًا.

4. **تشغيل اختبار القواعد الفرعي مباشرة دون محاكي**
   - الأعراض: `test:rules:spec` رفض الاتصال على `127.0.0.1:8080` وتخطى الاختبارات.
   - السبب: الأمر الفرعي يفترض أن محاكي Firestore يعمل، ولا يبدأه بنفسه.
   - الحل: تشغيل `npm run test:rules` الذي يبدأ محاكيًا معزولًا ثم يستدعي مجموعة الاختبار.
   - الحالة: محلول؛ نجحت `8/8` اختبارات ثم نُشرت القواعد.

الملفات المتأثرة:

- `app/weather/WeatherPageClient.jsx`
- `tests/weatherDefaultLocation.test.js`
- `app/version.js`
- `package.json`
- `package-lock.json`
- `VERSION_LOG.md`
...
npm run build
npx opennextjs-cloudflare build
npx wrangler deploy --dry-run --outdir .tools\wrangler-0.3.59-dry-run
npx opennextjs-cloudflare deploy
npm run health:production
npm run test:rules:spec
npm run test:rules
node .tools\firebase-cli\node_modules\firebase-tools\lib\bin\firebase.js deploy --only firestore:rules --project date-tool-official
curl.exe --http1.1 https://datetools.date-tool-official.workers.dev/api/app-version
curl.exe --http1.1 https://date-tool.com/api/app-version
```

الحالة:

- نجح `202` اختبارًا في `39` ملفًا، ونجح ESLint وفحص الأسرار وبناء Next.js لـ`39` صفحة.
- نجح `8` اختبارات قواعد Firestore في المحاكي، ثم نُشرت القواعد إلى الإنتاج بنجاح.
- نجح OpenNext وWrangler dry-run، ثم نُشر Worker بمعرف النسخة `c72949a1-18cb-40e9-96b8-09b0cabdd1e0` ووقت بدء `20ms`.
- الإصدار المنشور هو `0.3.59`، وصفحة الطقس تستخدم الموقع الحالي بدل الرياض افتراضيًا.
- جُمعت حالة المصدر المنشورة ودُفعت إلى GitHub على فرع `master` في commit `c3404a2` بعد موافقة المستخدم الصريحة على النطاق الواسع واحتمال تشغيل نشر Cloudflare مرة أخرى.

المتبقي المحفوظ:

- انتظار تحديث Search Console بعد إعادة الزحف؛ لا حاجة لإعادة فهرسة الموقع كاملًا.
- لا يمكن طلب مراجعة AdSense جديدة قبل 2026-09-25، وبعدها يجب تأكيد استمرار المحتوى الفريد والصيانة واهتمام المستخدمين قبل الإرسال.
- اختبار الموقع الحالي يدويًا يتطلب سماح المستخدم بإذن الموقع في المتصفح؛ لم يُمنح الإذن أو تُقرأ إحداثيات المستخدم أثناء هذا العمل.
- متابعة فحوص GitHub Actions بعد دفع تصحيح بوابة الجودة المبين أدناه.

### تصحيح بوابة الجودة بعد دفع الإصدار 0.3.59

تم إنجازه:

- تأكيد تطابق `master` المحلي و`origin/master` عند الالتزام `c3404a2`؛ فحص CodeQL لهذا الالتزام نجح.
- إعادة إنتاج فشل GitHub في خطوة `npm ci` باستخدام npm 10 كما في بيئة Node 20: كان `esbuild@0.28.2` واعتماداته الاختيارية مفقودين من ملف القفل الذي قبله npm 11 محليًا.
- تحديث `package-lock.json` بواسطة npm 10 دون تغيير اعتمادات `package.json` المباشرة، ونجاح `npm ci` الجاف لشروط Linux والتثبيت الفعلي.
- إزالة توقعات رقم الإصدار القديم من اختباري PWA وربطها بثابتي الإصدار والتاريخ الحاليين.
- نجاح ESLint وفحص الأسرار وتدقيق الاعتمادات الإنتاجية دون ثغرات و`202` اختبار وحدة و`8` اختبارات قواعد Firestore وبناء `39` صفحة.
- نجاح إعادة اختبارات المتصفح: `49` ناجحة وواحد متجاوز وفق إعداد الاختبار.

الأخطاء المكتشفة:

1. **فشل تثبيت اعتمادات بوابة الجودة على GitHub**
   - الأعراض: فشل `Quality gate` للالتزام `c3404a2` بعد 17 ثانية عند `npm ci` وقبل الاختبارات.
   - السبب: npm 10 يرفض ملف قفل لا يتضمن `esbuild@0.28.2` المطلوب لاعتماد اختياري فرعي.
   - الحل: توليد القفل باستخدام npm 10 والتحقق من `npm ci` على شروط Linux.
   - الحالة: مغلق؛ نجح `npm ci` و`Quality gate` على GitHub في الالتزام `768178c`.
2. **توقعات PWA ثابتة لإصدار قديم**
   - الأعراض: اختباران يتوقعان `0.3.58` وتاريخ `2026-09-13` بدل `0.3.59`.
   - السبب: بيانات متوقعة ثابتة لم تُحدث مع إصدار التطبيق.
   - الحل: استخدام `APP_VERSION` و`APP_VERSION_DATE` في الاختبارات.
   - الحالة: مصحح؛ `202/202` اختبارًا ناجحًا.
3. **تعذر بدء Chromium مرة واحدة محليًا**
   - الأعراض: `spawn UNKNOWN` لأول اختبار متصفح على Windows فقط، ثم أكملت بقية الاختبارات.
   - السبب: عارض تشغيل محلي؛ لم يتكرر عند إعادة تشغيل مجموعة الاختبارات عبر مشغل المشروع.
   - الحل: إعادة المجموعة كاملة والتحقق من نجاحها، دون تغيير سلوك التطبيق.
   - الحالة: مغلق بعد نجاح `49` اختبار متصفح.
...
```txt
npm exec --package npm@10 -- npm install --package-lock-only --ignore-scripts --no-audit --no-fund
npm exec --package npm@10 -- npm ci --dry-run --ignore-scripts --os=linux --cpu=x64 --libc=glibc
npm exec --package npm@10 -- npm ci --ignore-scripts --no-audit --no-fund
npm test
npm run lint
npm run quality:local
npm run test:e2e
...
- السبب: خطوة التثبيت استخدمت مسار Playwright الافتراضي على Linux، لكن `scripts/run-browser-tests.mjs` يضع `XDG_CACHE_HOME` تحت `.tools/cache`، فتبدل مكان البحث أثناء الاختبار. عُيّن `PLAYWRIGHT_BROWSERS_PATH` على مستوى مهمة GitHub إلى `${{ github.workspace }}/.tools/playwright-browsers` ليستخدمه التثبيت والاختبار معًا. المحاولة الأولى باستخدام `runner.temp` رُفضت عند تحليل ملف العمل لأن سياق `runner` غير متاح في متغيرات مستوى المهمة، لذا استُبدل بـ`github.workspace`.
- دُفع تصحيح مسار Playwright في الالتزام `768178c`، ونجح `Quality gate` كاملًا على GitHub: `https://github.com/datetoolofficial-netizen/date_tools/actions/runs/35724565774`. ونجح `CodeQL` للالتزام نفسه. بقي تحديث هذه المذكرة محليًا لتجنب تشغيل فحوص ونشر إضافيين بسبب تغيير توثيقي فقط.

المتبقي المحفوظ:

- لا يوجد بند مفتوح متعلق بإصلاح بوابة الجودة.

### إكمال مراجعة مستخدمي AdSense H-IAM-04

تم إنجازه:

- اعتماد تأكيد المالك بتاريخ 2026-09-20 أن قائمة مستخدمي AdSense تحتوي الحساب الأساسي النشط فقط.
- مستوى الوصول للحساب الأساسي هو `Admin`، ولا توجد حسابات قديمة أو مستخدمون إضافيون بصلاحيات زائدة.
- اعتماد بقاء الإدارة لدى الحساب الأساسي فقط حاليًا، دون إجراء أي تغيير في الحساب أو ملف الدفعات.
- توثيق قاعدة الإضافة المستقبلية: `Standard` هو المستوى الافتراضي لمستخدم الإعلانات أو التقارير، و`Admin` يقتصر على من يحتاج إدارة المستخدمين والشروط وإغلاق الحساب.

الأخطاء المكتشفة:

- لا توجد صلاحيات AdSense زائدة حاليًا.
- الاعتماد على حساب واحد يبقى مخاطرة استمرارية مقبولة بقرار المالك، وتخففها حماية الحساب ووسائل استرداده الموثقة سابقًا.
...
```txt
توثيق نتيجة الفحص البشري الذي نفذه المالك لقائمة مستخدمي AdSense ومستوى الوصول
git diff --check للملفات المتأثرة
```

الحالة:

- H-IAM-04 مكتمل من ناحية المستخدمين والصلاحيات.
- لم يُفتح AdSense مجددًا، ولم تتغير صلاحيات أو بيانات دفع، ولم يُنشر أو يُدفع كود المشروع.

المتبقي المحفوظ:

- متابعة حالة مراجعة الموقع وسياسات AdSense كتقرير خارجي ينتظر Google، وليست فجوة صلاحيات.
- مراجعة H-IAM-06 لتوزيع أدوار المنصة الداخلية.

### إكمال مراجعة مستخدمي Search Console H-IAM-05

تم إنجازه:

- اعتماد تأكيد المالك بتاريخ 2026-09-20 أن Search Console يحتوي الحساب الأساسي فقط بصلاحية `Owner`.
- عدم وجود مالك سابق أو مستخدم إضافي أو صلاحية بشرية زائدة.
- توثيق قاعدة الإضافة المستقبلية: `Full` لمن يحتاج التقارير والإجراءات، و`Restricted` للقراءة المحدودة، ولا يمنح `Owner` إلا لاستمرارية الملكية عند الحاجة.

الأخطاء المكتشفة:

- لا توجد صلاحيات Search Console زائدة حاليًا.
- الاعتماد على مالك واحد يبقى مخاطرة استمرارية مقبولة بقرار المالك، وتخففها حماية حساب Google وإثبات ملكية النطاق الموثقان سابقًا.
...
```txt
توثيق نتيجة الفحص البشري الذي نفذه المالك لقائمة مستخدمي Search Console ومستوى الوصول
git diff --check للملفات المتأثرة
```

الحالة:

- H-IAM-05 مكتمل من ناحية المستخدمين والصلاحيات.
- لم يُفتح Search Console مجددًا، ولم تتغير الملكية أو طرق إثباتها، ولم يُنشر أو يُدفع كود المشروع.

المتبقي المحفوظ:

- مراجعة H-IAM-06 لتوزيع أدوار المنصة الداخلية.
- تبقى تقارير الفهرسة والأداء تقارير تشغيلية منفصلة عن صلاحيات الحساب.

### تأجيل مراجعة التوزيع الفعلي لأدوار المنصة H-IAM-06

تم إنجازه:

- اعتماد تأكيد المالك بتاريخ 2026-09-20 بعدم وجود سوى الحساب الأساسي في المنصة حاليًا.
- تجاوز مراجعة توزيع الحسابات الداخلية مؤقتًا لعدم وجود مدير أو مساعد أو معلن فعلي إضافي.
- تثبيت شرط إعادة الفحص قبل تفعيل أول حساب جديد: مراجعة الدور والنطاق وMFA وسجل الدعوة، وإبقاء `platform_owner` للحساب الأساسي فقط.

الأخطاء المكتشفة:

- لا توجد صلاحيات داخلية زائدة قابلة للمراجعة حاليًا لعدم وجود حسابات إضافية.
...
```txt
توثيق قرار المالك بتأجيل مراجعة توزيع الأدوار حتى إضافة أول حساب فعلي
git diff --check للملفات المتأثرة
```

الحالة:

- H-IAM-06 مؤجل بقرار واعٍ، وليس عطلًا أو مانع نشر حاليًا.
- لم تتغير حسابات أو صلاحيات، ولم يُنشر أو يُدفع كود المشروع.

المتبقي المحفوظ:

- إعادة H-IAM-06 قبل تفعيل أول حساب إداري أو مساعد أو معلن فعلي.
- استكمال اختبارات الوسائط والأجهزة وإمكانية الوصول وPWA والبنود الخارجية الأخرى.

### إكمال الاختبارات المعزولة لأمان رفع الوسائط - محلي فقط

تم إنجازه:

- فصل معالجة الرفع المصرح به إلى وحدة قابلة للاختبار دون الاتصال بـFirebase أوR2 الإنتاجيين، مع إبقاء التحقق من Firebase والهوية في مسار API كما هو.
- إضافة اختبار تكامل بBucket وهمي يثبت قبول PNG سليمة، إنشاء اسم ومسار آمنين، ضبط Content-Type وبيانات الوصف، وكتابة سجل التدقيق بعد التخزين الناجح فقط.
- إثبات رفض الامتداد المزور وHTML وSVG النشط وFixture تنفيذي آمن قبل استدعاء `bucket.put` أو سجل التدقيق.
- إثبات منع المعلن من غير أدوار تعديل الحملات، ومنع المعلن من رفع أي فئة غير `ads`، ومنع دور الإدارة للقراءة فقط من رفع الهوية.
- اكتشاف أن التنفيذ السابق لم يكن يقرأ أبعاد الصورة رغم أن قائمة الفحص اعتبرتها مغطاة، ثم إضافة قراءة آمنة لأبعاد PNG وJPEG وWebP وGIF وICO.
- إضافة حدود 8192 بكسل لكل ضلع و40 مليون بكسل للصورة، ورفض الأبعاد المفقودة أو الصفرية أو المفرطة قبل التخزين.
- نجاح 200 اختبار في 38 ملفًا، ونجاح ESLint وفحص الأسرار وبناء Next.js الذي ولّد 39 صفحة.
- نجاح بناء OpenNext وإنشاء `.open-next/worker.js`، ثم نجاح Wrangler `dry-run` وقراءة 191 ملف أصل دون رفع أو نشر.

الأخطاء المكتشفة:

1. **التحقق من أبعاد الصور كان موثقًا لكنه غير منفذ**
   - الأعراض: ملف يحمل توقيع صورة صحيحًا كان يجتاز التحقق دون فحص العرض والارتفاع.
   - السبب: `validateImageUpload` تحقق من الحجم بالبايت وMagic Bytes فقط.
   - الحل: استخراج الأبعاد من رؤوس جميع الصيغ المسموحة ورفض القيم المفقودة أو الصفرية أو التي تتجاوز الحدود.
   - الحالة: محلول ومغطى باختبارات وحدة وتكامل معزولة؛ غير منشور.

2. **اختبارات العقود القديمة افترضت وجود منطق الرفع داخل ملف route**
   - الأعراض: فشل اختباران بعد فصل المعالجة إلى وحدة قابلة للاختبار رغم نجاح السلوك الجديد.
   - السبب: الاختبارات كانت تبحث نصيًا داخل `route.js` فقط.
   - الحل: تحديثها لإثبات استدعاء الوحدة من المسار وفحص الحواجز وترتيب التحقق قبل التخزين داخل الوحدة.
   - الحالة: محلول؛ نجحت المجموعة كاملة.

الملفات المتأثرة:

- `app/api/_lib/mediaValidation.js`
- `app/api/_lib/mediaUploadHandler.js`
- `app/api/media/upload/route.js`
- `tests/mediaValidation.test.js`
- `tests/mediaUploadHandler.test.js`
- `tests/securityBoundaries.test.js`
- `tests/settingsSecurity.test.js`
- `docs/HUMAN_VERIFICATION_CHECKLIST.md`
- `PROJECT_MEMO.md`
...
```powershell
npx vitest run tests/mediaValidation.test.js tests/mediaUploadHandler.test.js tests/securityBoundaries.test.js
npm test
npm run lint
npm run security:secrets
npm run build
npx opennextjs-cloudflare build
npx wrangler deploy --dry-run --outdir .tools\wrangler-media-dry-run
git diff --check
```

الحالة:

- اختبارات أمان الرفع البرمجية والمعزولة مكتملة وناجحة.
- لم يتصل الاختبار بـFirebase أوR2 الإنتاجيين، ولم يُرفع ملف خارجي، ولم يُنشر أو يُدفع كود المشروع.

المتبقي المحفوظ:

- اختبار قبول واجهة الرفع بالحساب المعلن التجريبي وصورة سليمة ثم Fixtures الاختبار غير المؤذية، بعد اكتمال إنشاء الحساب.
- لا يستخدم الاختبار اليدوي ملفًا تنفيذيًا حقيقيًا ولا بيانات حساسة.

### مراجعة صلاحيات Cloudflare H-IAM-01 - قراءة فقط

تم إنجازه:

- فحص أعضاء حساب Cloudflare بتاريخ 2026-09-19؛ يوجد عضو بشري نشط واحد بدور `Super Administrator - All Privileges` والتحقق بخطوتين مفعّل.
- فحص الجلسات؛ ظهرت جلسة Chrome الحالية فقط ولم تظهر جلسات أخرى غير معروفة.
- فحص Account API Tokens وUser API Tokens وR2 API Tokens؛ لا توجد رموز مملوكة للحساب أو R2، ويوجد رمز مستخدم واحد مخصص للبناء والنشر.
- إثبات أن رمز البناء محصور في سبع صلاحيات لـWorkers Scripts وKV وR2 وWorkers Routes مع Account Settings Read وUser Details Read وMemberships Read، ودون الفوترة أو إدارة الأعضاء.
- إثبات عدم وجود سر Cloudflare أو أمر نشر في GitHub Actions الحالية؛ الجودة وفحص ما بعد النشر يعملان دون صلاحيات نشر.
- إثبات وجود الحاوية `datetools-media` وربطها فعليًا بالـWorker `datetools` عبر `MEDIA_BUCKET` كـR2 Binding مباشر، دون مفتاح S3 داخل التطبيق.
- فحص سجل التدقيق لآخر 7 أيام: 31 عملية ناجحة و0 فاشلة؛ الأحداث البشرية المرئية مرتبطة بالنشر عبر API Token أو OAuth، وبقية الأحداث الظاهرة تغييرات نظام للشهادات وDNS.
- فحص التطبيقات المتصلة؛ يوجد اتصال OAuth واحد لـWrangler بصلاحيات تفاعلية واسعة.

الأخطاء المكتشفة:

1. **لا يوجد مالك Cloudflare احتياطي**
   - الأعراض: عضو بشري واحد فقط يملك الحساب بالكامل.
   - السبب: لم يضف مالك احتياطي موثوق بعد.
   - الحل: إضافة مالك احتياطي موثوق مع MFA عند توفره، مع إبقاء الحساب اليومي الأقل صلاحية منفصلًا إذا أضيف فريق تشغيل.
   - الحالة: مفتوح؛ يتطلب قرارًا وهوية بشرية موثوقة من المالك.
2. **اتصال Wrangler أوسع من رمز البناء المحدود**
   - الأعراض: OAuth الخاص بـWrangler يملك 28 صلاحية تفاعلية تشمل منتجات لا يستخدمها النشر الحالي.
   - السبب: تفويض Wrangler التفاعلي القياسي أوسع من Token الإنتاج المحدود.
   - الحل: إبقاؤه فقط إذا كان مطلوبًا للإدارة المحلية التفاعلية، أو إلغاؤه بعد موافقة مستقلة والتأكد من عدم الحاجة؛ رمز البناء المحدود يبقى مسار النشر الآلي.
   - الحالة: قرار مفتوح؛ لم يُلغ أو يُعدّل أي وصول.
...
```txt
فحص قراءة فقط لأعضاء Cloudflare والأدوار والرموز والجلسات والتطبيقات المتصلة وسجل التدقيق وR2 وWorker Bindings
rg لمراجع أسرار Cloudflare وR2 في المستودع
```

الحالة:

- H-IAM-01 موثق بالحالة الجزئية `[~]`: الضوابط التقنية سليمة، ولا يوجد Global API Key مستخدم في CI.
- لم تُعرض أو تُنسخ قيم رموز أو مفاتيح، ولم تُسجل عناوين IP أو بيانات جلسات حساسة.
- لم يُلغ اتصال Wrangler، ولم تتغير الصلاحيات، ولم يُنشر أو يُدفع شيء.

المتبقي المحفوظ:

- قرار إضافة مالك Cloudflare احتياطي موثوق.
- قرار إبقاء اتصال Wrangler التفاعلي أو إلغائه.
- استكمال H-IAM-02 إلى H-IAM-06 ثم اختبارات الرفع والأجهزة وإمكانية الوصول وPWA والبنود الخارجية والمدفوعة.

### بدء مراجعة Firebase وGoogle Cloud H-IAM-02 - قراءة فقط

تم إنجازه:

- فحص IAM لمشروع `date-tool-official` بتاريخ 2026-09-19؛ يوجد مالك بشري واحد وحساب خدمة Firebase واحد.
- فحص أدوار حساب الخدمة: Firebase Admin SDK Administrator Service Agent وFirebase App Check Admin وFirebase Authentication Admin وService Account Token Creator.
- فحص المفاتيح: يوجد مفتاح مولد واحد نشط وطويل الأجل؛ لم تُفتح أو تُنسخ قيمته.
- مطابقة الاستخدام مع الكود: الخادم يوقع JWT بالمفتاح للوصول إلى OAuth ثم Firestore بنطاق Datastore، بينما التحقق من Firebase ID Token يستخدم JWKS العام ولا يحتاج Authentication Admin.
- فحص المستودع أكد عدم وجود قيمة مفتاح خاصة أو ملف اعتماد، مع بقاء ملفات البيئة وWrangler المحلية مستثناة من Git ووجود Secret Scan مانع.
- فحص Firebase Authentication Authorized domains: النطاقان الافتراضيان لـFirebase و`date-tool.com` وبيئتا التطوير المحليتان `localhost` و`127.0.0.1` فقط؛ لا توجد نطاقات قديمة أو غير معروفة.
- فحص Firebase App Check APIs: Cloud Firestore وAuthentication في وضع `Monitoring`، والطلبات المرصودة 0% موثقة و100% غير موثقة؛ لا يجوز تفعيل Enforcement قبل دمج App Check واختباره.
- فحص Firebase App Check Apps: تطبيق الويب `Registered` مع `reCAPTCHA Enterprise`. التهيئة البرمجية موجودة في `app/firebase.js` لكنها مشروطة بـ`NEXT_PUBLIC_FIREBASE_APP_CHECK_SITE_KEY`، والاسم لا يظهر إلا في `.dev.vars.example` ولا يظهر في `wrangler.jsonc`؛ يرجح غيابه وقت بناء Cloudflare.
- خطة المعالجة الآمنة: إضافة المفتاح العام إلى Cloudflare Build variables دون إدخاله في Git، بناء ونشر نسخة اختبارية عند الموافقة، مراقبة ارتفاع الطلبات الموثقة، ثم تفعيل Enforcement تدريجيًا فقط بعد نجاح التدفقات.
- فحص Firebase Authentication Usage: المشروع على خطة `Spark` المجانية، والاستخدام الحالي منخفض ولا توجد بيانات استخدام SMS أو مؤشرات ضغط ظاهرة على الحصة.
- فحص Firebase Usage and billing Overview: تكلفة المشروع `No-cost`؛ استهلاك Firestore أقل من 1% للقراءة والكتابة والحذف شبه صفر، ولا توجد إشارة إلى ضغط على الحصص اليومية.
- فحص Firebase Account & budgets: المشروع على `Spark No-cost` ولا يوجد حساب فوترة أو ميزانيات مرتبطة؛ لا توجد تكاليف أو تنبيهات مالية قابلة للضبط حاليًا.

الأخطاء المكتشفة:

1. **مالك Google Cloud واحد فقط**
   - الأعراض: لا توجد استمرارية وصول إذا فُقد الحساب الأساسي.
   - السبب: لم يضف مالك احتياطي موثوق بعد.
   - الحل: إضافة مالك احتياطي موثوق ومحمي بـMFA عند توفره، وعدم استخدام Owner للأعمال اليومية بعد إنشاء دور تشغيل أضيق.
   - الحالة: مفتوح ويتطلب قرار المالك وهوية موثوقة.
2. **حساب خدمة واسع الصلاحية ومفتاح طويل الأجل**
   - الأعراض: الحساب يحمل أدوار Auth وApp Check وToken Creator إلى جانب الدور الإداري الواسع، مع أن الاستخدام المرصود يحتاج Firestore فقط.
   - السبب: استخدام حساب Firebase Admin SDK الافتراضي للخادم بدل حساب خدمة مخصص بأقل صلاحية.
   - الحل: إنشاء حساب خدمة مستقل بصلاحيات Firestore الدنيا، اختبار جميع مسارات الخادم، تدوير أسرار Worker، ثم تعطيل المفتاح القديم بعد التحقق وخطة رجوع.
   - الحالة: مفتوح؛ تغيير الصلاحيات والمفتاح لم يُنفذ لأنه يحتاج موافقة مستقلة عند لحظة التنفيذ.
...
```txt
فحص قراءة فقط لـGoogle Cloud IAM وService Accounts وKeys
rg لاستخدام متغيرات حساب الخدمة ونطاق OAuth والتحقق من Firebase ID Tokens
```

الحالة:

- اكتمل الفحص الخارجي لـH-IAM-02 وبقيت حالته `[~]` لقرارات المعالجة المقصودة: مالك احتياطي، حساب خدمة أقل صلاحية، ومتغير بناء App Check واختباره قبل Enforcement.
- لم تتغير IAM أو الحسابات أو المفاتيح أو الأسرار، ولم يُنشر شيء.

المتبقي المحفوظ:

- لا يوجد فحص Firebase خارجي إضافي حاليًا؛ البنود المتبقية إجراءات تغيير حساسة أو دورة نشر اختبارية وتتطلب موافقة مستقلة.
- إضافة متغير App Check العام إلى إعدادات بناء Cloudflare واختباره عند الموافقة على دورة نشر اختبارية؛ لا يفعّل Enforcement قبل ظهور طلبات موثقة.
- قرار إضافة مالك احتياطي موثوق.
- تصميم وتنفيذ انتقال آمن إلى حساب خدمة مخصص بأقل صلاحية بعد موافقة مستقلة.

### مراجعة وتشديد GitHub H-IAM-03

تم إنجازه:

- فحص Repository Settings > Collaborators بتاريخ 2026-09-19.
- المستودع عام ولا يوجد أي متعاون مباشر؛ الحساب المالك وحده يستطيع المساهمة والإدارة، ولا توجد صلاحيات بشرية زائدة.
- MFA للحساب الأساسي موثق سابقًا مع Passkey ورموز استرداد محفوظة.
- فحص Deploy keys: لا توجد مفاتيح نشر مرتبطة بالمستودع، ولا يوجد وصول SSH خارجي مباشر.
- فحص GitHub Apps: التطبيق المثبت الوحيد هو `Cloudflare Workers and Pages`؛ يلزم استكمال صفحة Configure للتحقق من نطاق المستودعات والصلاحيات.
- فحص Configure لتطبيق Cloudflare: الوصول مقيد بمستودع `date_tools` فقط، مع قراءة metadata وقراءة/كتابة administration وchecks وcode وdeployments وpull requests. الصلاحيات واسعة لكنها لازمة للتكامل الحالي وتحتاج مراجعة دورية.
- فحص Actions secrets: لا توجد أسرار على مستوى المستودع أو البيئات؛ النشر لا يعتمد على مفاتيح محفوظة في GitHub Actions.
- فحص Actions variables: لا توجد متغيرات على مستوى المستودع أو البيئات.
- فحص Actions General: `GITHUB_TOKEN` للقراءة فقط، وخيار إنشاء/اعتماد Pull Requests معطل؛ workflows المحلية تضبط `contents: read` وCodeQL يمنح `security-events: write` لمهمته فقط.
- الفجوة: السماح بكل الإجراءات الخارجية وشرط SHA الكامل معطل. الإجراءات المستخدمة رسمية فقط لكنها مربوطة بعلامات `@v6` و`@v5` و`@v4`؛ يلزم تثبيتها على commit SHA كامل محليًا قبل تشديد إعداد GitHub.
- الموافقة على workflows من forks مطلوبة لأول مساهمة فقط؛ لا توجد أسرار وToken للقراءة، فتظل المخاطرة الحالية محدودة وتراجع مع حماية الفروع.
- ثُبتت جميع مراجع Actions الرسمية محليًا على commit SHA كامل مع تعليق رقم الإصدار في `quality.yml` و`codeql.yml` و`post-deploy-health.yml`.
- إنشاء Ruleset نشط باسم `Protect master` يستهدف الفرع الافتراضي `master` ويمنع حذف الفرع وforce-push، مع إبقاء الدفع المباشر وعمليات النشر العادية متاحة.
- تفعيل سياسة GitHub Actions التي تشترط تثبيت كل Action على commit SHA كامل؛ ظهرت رسالة `Actions policy updated` وبقي الخيار مفعّلًا بعد الحفظ.
- لم تُفرض Pull Requests أو status checks على `master` حتى لا تتعطل آلية الدفع والنشر الحالية.

الأخطاء المكتشفة:

- لا توجد مشكلة صلاحيات حالية في قائمة المتعاونين.
- توجد فجوة استمرارية: المستودع شخصي ولا يدعم مالكًا احتياطيًا للحساب؛ الحل المؤسسي مستقبلًا هو Organization بمالكين موثوقين وMFA إلزامي.

الأوامر المستخدمة:

```txt
فحص قراءة فقط لـGitHub Repository Settings > Collaborators
git ls-remote لمراجع الإصدارات الرسمية لـActions
rg للتحقق من عدم بقاء مراجع @vN أو main أو master داخل workflows
git diff --check للملفات المتأثرة
```
...
- H-IAM-03 موثق بالحالة `[~]`: الفحص والتشديد التقني مكتملان، وبقي قرار نقل المستودع إلى Organization لإتاحة مالك احتياطي موثوق.
- تغيرت إعدادات أمان المستودع فقط بعد موافقة المستخدم؛ لم تتغير عضوية الحسابات ولم يُنشر كود المشروع.

المتبقي المحفوظ:

- اتخاذ قرار الاستمرارية المؤسسية مستقبلًا: نقل المستودع إلى Organization وإضافة مالك احتياطي موثوق عند توفره.

### إكمال حماية وتجديد النطاق في Name.com

تم إنجازه:

- التحقق بتاريخ 2026-09-16 من تعيين وسيلة الدفع `Default Profile` دون عرض أو تسجيل تفاصيلها.
- إعادة فحص `date-tool.com` وإثبات اختفاء `Action Required` وظهور `Automatic Renewal` بحالة `ON`.
- اعتماد H-MFA-05 مكتملًا: Authenticator ورموز الاستعادة وقفل النقل وWHOIS Privacy ووسيلة الدفع وعملة التجديد والتجديد التلقائي جميعها جاهزة.

الأخطاء المكتشفة:

1. **لا توجد مشكلة مفتوحة في تجديد النطاق**
   - الأعراض السابقة: كان التجديد التلقائي يعرض `Action Required`.
   - السبب السابق: وسيلة الدفع لم تكن `Default Profile`.
   - الحل: عيّن المستخدم وسيلة الدفع الافتراضية، ثم أثبت الفحص أن `Automatic Renewal` أصبح `ON`.
   - الحالة: محلول ومتحقق منه قراءة فقط.
...
```txt
فحص قراءة فقط لخيار Default Profile وحالة Automatic Renewal في Name.com
```

الحالة:

- H-MFA-05 مكتمل.
- لم تُعرض أو تُسجل تفاصيل وسيلة الدفع أو رموز الاستعادة، ولم يجر خصم يدوي.
- لم يُنشر أو يُدفع شيء في المشروع.

المتبقي المحفوظ:

- تفعيل إشعارات نجاح وفشل دخول Name.com بعد موافقة المستخدم عند لحظة التغيير.
- استكمال H-MFA-06 الخاص ببريد العمل وبريد الاسترداد.
- استكمال بقية الاختبارات البشرية بالترتيب المثبت في القائمة.

### إكمال حماية بريد الاسترداد H-MFA-06

تم إنجازه:

- اعتماد تأكيد المستخدم بتاريخ 2026-09-19 بتفعيل التحقق بخطوتين للبريد الاحتياطي المخصص لاسترداد البريد الأساسي.
- اعتماد تأكيد المستخدم بحفظ رموز استرداد البريد الاحتياطي خارج الجهاز.
- إغلاق H-MFA-06 مع إبقاء مراجعة المالك البشري الاحتياطي ضمن اختبارات الصلاحيات الخارجية.

الأخطاء المكتشفة:

1. **لم يتوفر إثبات مرئي مستقل لحساب البريد الاحتياطي**
   - الأعراض: التبويب المتاح يعرض إعداد ربط بريد الاسترداد بالحساب الأساسي، لا صفحة أمان الحساب الاحتياطي نفسه.
   - السبب: صفحة أمان الحساب الاحتياطي لم تبق مفتوحة في جلسة الفحص.
   - الحل: اعتمد تأكيد المستخدم الصريح مع توثيق حد الدليل، ومنع فتح الرموز أو تسجيل عنوان البريد أو الأسرار.
   - الحالة: مقبول كبند تحقق بشري؛ لا توجد مطالبة بكشف معلومات إضافية.
...
```txt
فحص قراءة فقط لصفحة ربط بريد الاسترداد وتوثيق تأكيد المستخدم
```

الحالة:

- H-MFA-05 وH-MFA-06 مكتملان.
- لم تُفتح أو تُسجل رموز استرداد أو عناوين بريد أو بيانات دخول.
- لم يُنشر أو يُدفع شيء في المشروع.

المتبقي المحفوظ:

- H-MFA-07 لعامل الدخول الثاني داخل منصة الإدارة بحسب دعم Firebase والخطة.
- H-IAM-01 إلى H-IAM-06 لمراجعة الصلاحيات الخارجية والداخلية الفعلية.
- اختبارات الوسائط والأجهزة وإمكانية الوصول وPWA والبنود الخارجية والمدفوعة الأخرى.

### تفعيل تنبيهات أمان الدخول في Name.com

تم إنجازه:

- تفعيل إرسال بريد عند فشل محاولة تسجيل الدخول بعد موافقة المستخدم الصريحة.
- تفعيل إرسال بريد عند نجاح تسجيل الدخول بعد موافقة المستخدم الصريحة.
- التحقق من اكتمال الحفظ وظهور الخيارين بحالة مفعّلة.

الأخطاء المكتشفة:

1. **تنبيهات الدخول كانت معطلة**
   - الأعراض: لم يكن الحساب يرسل إشعارًا عند نجاح أو فشل تسجيل الدخول.
   - السبب: الخياران غير محددين في الإعدادات.
   - الحل: فُعّل الخياران بعد موافقة المستخدم وتحقق الحفظ.
   - الحالة: محلول.
...
```txt
تفعيل خياري Security Notification والتحقق من حفظهما في Name.com
```

الحالة:

- H-MFA-05 مكتمل بالكامل.
- لم تُسجل بيانات حساب أو عناوين اتصال في ملفات المشروع.
- لم يُنشر أو يُدفع شيء في المشروع.

المتبقي المحفوظ:

- استكمال H-MFA-06 للتحقق من حماية بريد العمل وبريد الاسترداد.
- استكمال بقية الاختبارات البشرية بالترتيب المثبت في القائمة.

### فحص التجديد بعد اختيار العملة في Name.com - قراءة فقط

تم إنجازه:

- إثبات حفظ عملة التجديد الافتراضية بتاريخ 2026-09-16.
- إعادة فحص صفحة النطاق وإثبات أن `Automatic Renewal` ما زال يعرض `Action Required`.
- فتح قائمة وسيلة الدفع قراءة فقط وتحديد المانع المتبقي دون تغييرها.

الأخطاء المكتشفة:

1. **وسيلة الدفع ليست Default Profile**
   - الأعراض: خيار `Default Profile` غير محدد، وتعرض الصفحة أن ملف دفع افتراضيًا مطلوب، ويستمر تنبيه `Action Required` للنطاق.
   - السبب: حفظ البطاقة واختيار العملة لا يكفيان؛ يجب تعيين وسيلة الدفع الافتراضية التي سيستخدمها التجديد التلقائي.
   - الحل: يفعّل المستخدم `Default Profile` بنفسه لأنه يجيز استخدام وسيلة الدفع في الخصومات المستقبلية، ثم يعاد التحقق قراءة فقط.
   - الحالة: مفتوح؛ ينتظر إجراء المستخدم المالي المباشر.
...
```txt
فحص قراءة فقط لعملة التجديد وحالة Automatic Renewal وقائمة Profile actions في Name.com
```

الحالة:

- لم تُعرض أو تُسجل تفاصيل وسيلة الدفع.
- لم يجر خصم أو تجديد أو نشر أو دفع تغييرات المشروع.

المتبقي المحفوظ:

- تعيين وسيلة الدفع `Default Profile` بواسطة المستخدم ثم التحقق من اختفاء `Action Required`.
- تفعيل إشعارات نجاح وفشل الدخول بعد موافقة المستخدم عند لحظة التغيير.
- استكمال H-MFA-06 ثم بقية الاختبارات البشرية بالترتيب المثبت في القائمة.

### التحقق من وسيلة الدفع وتجديد Name.com - قراءة فقط

تم إنجازه:

- التحقق بتاريخ 2026-09-16 من وجود وسيلة دفع محفوظة في Name.com دون عرض أو تسجيل تفاصيلها.
- فحص صفحة `date-tool.com` وإثبات بقاء قفل النقل وWHOIS Privacy مفعّلين.
- إثبات أن النطاق مسجل حتى 2027-03-15.

الأخطاء المكتشفة:

1. **عملة التجديد الافتراضية غير محددة**
   - الأعراض: صفحة الفوترة تعرض `Select a currency` وصفحة النطاق تعرض `Automatic Renewal: Action Required` رغم وجود وسيلة دفع محفوظة.
   - السبب: لا يستطيع Name.com اعتماد التجديد التلقائي قبل اختيار عملة الخصم الافتراضية.
   - الحل: يختار المستخدم العملة بنفسه، ثم يعاد فحص صفحة النطاق حتى يختفي `Action Required` وتظهر حالة التجديد التلقائي مفعّلة.
   - الحالة: مفتوح؛ وسيلة الدفع موجودة لكن التجديد التلقائي غير مكتمل.
...
```txt
فحص قراءة فقط لصفحة Billing and Renewal Settings وصفحة تفاصيل date-tool.com في Name.com
```

الحالة:

- لم تُعرض أو تُسجل بيانات البطاقة أو عناوين الحساب.
- لم يجر أي خصم أو تجديد أو نشر أو دفع تغييرات المشروع.

المتبقي المحفوظ:

- اختيار عملة التجديد الافتراضية بواسطة المستخدم والتحقق من تفعيل Automatic Renewal.
- تفعيل إشعارات نجاح وفشل الدخول بعد موافقة المستخدم عند لحظة التغيير.
- استكمال H-MFA-06 ثم بقية الاختبارات البشرية بالترتيب المثبت في القائمة.

### إصلاح دخول Firebase وتوحيد حقول الموقع العام - الإصدار 0.3.62

تم إنجازه:

- فحص خدمة Firebase Identity Toolkit مباشرة بالمفتاح العام ومرجع `https://date-tool.com/admin_login`؛ أعادت الخدمة `200` وأثبتت أن `date-tool.com` موجود ضمن `authorizedDomains`.
- فحص مسار تسجيل الدخول ببيانات وهمية غير حساسة؛ أعاد Firebase خطأ الاعتماد المتوقع، ما يثبت أن المشروع والمفتاح ومسار API يعملون من الدومين المنشور.
- منع تهيئة App Check الاختيارية من حجز إنشاء Firebase Auth؛ أصبحت تبدأ في الخلفية مع بقاء Turnstile وفحوص الصلاحيات دون تغيير.
- إضافة مهلة 10 ثوان لكل محاولة Firebase Auth، وتحويل الطلب المعلق إلى خطأ شبكة قابل لإعادة المحاولة بدل إبقاء زر الدخول عالقًا بلا نهاية.
- إضافة اختبار آلي لطلب Auth المعلق ونجاح 214 اختبارًا في 41 ملفًا.
- توحيد حقول التاريخ والساعة والمدينة والطقس عبر الصنف `public-tool-field` داخل `.public-site-root` فقط، بارتفاع وخط وخلفية موحدة ودون Border، مع Focus مرئي عبر الظل.
- إزالة سطر `أسماء المدن: © OpenStreetMap / Photon` من الفوتر وإلغاء تمرير `locationSource` غير المستخدم إليه.
- إزالة Border بطاقات الرطوبة والرياح وتوقع المطر وUV في الوضعين الفاتح والداكن.
- التحقق البصري محليًا عند عرض جوال من صفحات `/weather` و`/clock` و`/`، والتحقق من اختفاء سطر المصادر وحدود المؤشرات واتساق الحقول.
- نجاح ESLint وبناء Next.js الكامل وتوليد 39 صفحة.
- نجاح بوابة الجودة الكاملة: فحص الأسرار، و`npm audit` دون ثغرات، و214 اختبار وحدة، و8 اختبارات Firestore، وبناء الإنتاج.
- نجاح 57 اختبار متصفح على سطح المكتب والجوال، مع تجاوز اختبار واحد غير منطبق عمدًا ودون أي فشل.
- تحديث اختبار الموقع ليثبت غياب سطر مزود أسماء المدن بعد طلب إزالته.
- دفع commit الإصدار `15365da` إلى `master`، ونجاح مساري GitHub `Quality gate` و`CodeQL`.
- نشر الإصدار `0.3.62` والتحقق من ظهوره على `https://date-tool.com` عبر فحص الصحة الإنتاجي من أول محاولة.

الأخطاء المكتشفة:

1. **مسار تسجيل الدخول قد ينتظر طلبًا خارجيًا أو App Check مدة طويلة**
   - الأعراض: بقاء زر الدخول في حالة التحقق، أو ظهور `auth/network-request-failed` بعد انتظار طويل على أكثر من متصفح.
   - السبب: إعداد Firebase والمفتاح والدومين سليمة؛ نقطة الضعف البرمجية كانت انتظار App Check الاختياري قبل إنشاء Auth وعدم وجود مهلة لطلب تسجيل الدخول.
   - الحل: تشغيل App Check في الخلفية وإضافة مهلة وإعادة محاولة محدودة لطلب Firebase Auth.
   - الحالة: محلول محليًا ومغطى آليًا؛ يلزم اختبار حساب حقيقي بعد النشر للتأكيد الإنتاجي دون مشاركة كلمة المرور.
2. **أنماط حقول الأدوات متفرقة ومتعارضة عبر نقاط التوقف**
   - الأعراض: اختلاف الارتفاع والحجم والحدود بين حقول التاريخ والساعة والمدن والطقس، خصوصًا على الجوال.
   - السبب: لكل أداة قواعد CSS منفصلة مع قواعد جوال لاحقة.
   - الحل: صنف موحد محصور في غلاف الموقع العام ومضاف بعد قواعد نقاط التوقف لضمان نتيجة ثابتة دون التأثير على الإدارة أو منصة المعلنين.
   - الحالة: محلول محليًا ومتحقق بصريًا وآليًا.
...
- `app/globals.css`
- `tests/firebaseAuthRetry.test.js`
- `tests/publicToolFields.test.js`
- `e2e/location-quality.spec.js`
- `app/version.js`
- `package.json`
- `package-lock.json`
...
```powershell
npm test -- --run tests/firebaseAuthRetry.test.js tests/locationDisplay.test.js
npm run lint
npm test -- --run
npm run build
npm run dev
...
- الإصدار `0.3.62` منشور فعليًا على `https://date-tool.com` وبوابة الجودة والفحص الأمني وفحص الصحة الإنتاجي خضراء.
- لا توجد تغييرات وظيفية محلية غير منشورة ضمن هذا الإصدار.
...
- نجاح `Quality gate` و`CodeQL` للـ commit `91ea035`.
- نشر عامل Cloudflare `datetools` مباشرة بنجاح بالإصدار `0.3.63` ومعرف النسخة `88d43110-1bf0-4be0-9875-495e20d16cc6`.
- نجاح فحص الصحة الإنتاجي من أول محاولة بعد النشر.
- إعادة اختبار الدخول المنشور ببيانات وهمية: أعاد خطأ الاعتماد الصحيح خلال نحو 2.2 ثانية، ولم تظهر أي عناصر App Check/reCAPTCHA في الصفحة.
- أكد المستخدم بتاريخ 2026-09-24 نجاح تسجيل الدخول الحقيقي بحساب مالك المنصة على الموقع المنشور، وعدم عودة خطأ الشبكة بعد الإصلاح.
...
1. **وجود مفتاح App Check كان يفعّل المزود تلقائيًا قبل إثبات جاهزيته**
   ...
   - الحالة: محلول ومنشور ومثبت على الإنتاج باختبار متصفح مستقل.
...
- الإصدار `0.3.63` منشور على `https://date-tool.com` بعد نجاح جميع الفحوص المحلية والسحابية والإنتاجية.
- لا يتطلب الحل تغيير حساب المالك أو دوره أو قواعد Firebase.
- اكتمل التحقق الإنتاجي الآلي والبشري لمسار تسجيل الدخول، ولا يوجد عطل متبقٍ في هذا المسار.
...
### مراجعة الهدف المثبت وبنود الأمان - 2026-09-24

تم إنجازه:

- مطابقة الهدف المثبت مع `docs/HUMAN_VERIFICATION_CHECKLIST.md` وسجل المشروع بدل اعتبار البنود مكتملة بالذاكرة.
- إعادة تشغيل بوابة الجودة المحلية كاملة بعد نجاح الدخول الحقيقي للإصدار `0.3.63`.
- نجاح ESLint وفحص الأسرار دون العثور على بيانات اعتماد غير معتمدة.
- نجاح تدقيق حزم الإنتاج مع `0 vulnerabilities`.
- نجاح 214 اختبار وحدة وتكامل و8 اختبارات لقواعد Firestore ضد مشروع Emulator تجريبي دون لمس الإنتاج.
- نجاح بناء Next.js وتوليد 39 صفحة.
- نجاح 57 اختبار متصفح لسطح المكتب والجوال، مع تجاوز واحد مقصود ودون فشل.
- تأكيد أن اختبارات قواعد `platform_owner` وعزل منظمات المعلنين وحماية الرفع وCSRF وSSRF وIDOR وسجل التدقيق وفشل الخدمات مغطاة آليًا.

الأخطاء المكتشفة:

1. **الهدف المثبت أوسع من البنود البرمجية المكتملة**
   - الأعراض: يمكن أن تبدو بوابة الجودة الخضراء وكأنها تعني اكتمال جميع الجاهزية والأمان.
   - السبب: الهدف يشمل أيضًا MFA داخل المنصة، والصلاحيات الخارجية، والنسخ والاستعادة، والتنبيهات، واختبارات أجهزة وحسابات حقيقية لا تثبتها الاختبارات المحلية.
   - الحل: إبقاء الهدف غير مكتمل، وفرز المتبقي بوضوح في قائمة الاختبارات البشرية وعدم نقل أي بند إلى `[x]` بلا دليل وتاريخ.
   - الحالة: موثق؛ لا يوجد فشل برمجي جديد، لكن البنود الخارجية أدناه ما زالت مفتوحة أو مؤجلة بقرار المالك.
...
- الأمان البرمجي ضمن النطاق المختبر سليم، ولم يُنشر شيء في هذه المراجعة.
- الهدف المثبت غير مكتمل بعد، لأن اكتماله يتطلب إغلاق البنود البشرية والخارجية أو توثيق تعذرها بدليل واضح.

المتبقي المحفوظ:

- MFA داخل منصة الإدارة للحسابات الحساسة، واختبار App Check قبل أي إعادة تفعيل.
- قرار أقل صلاحية لحساب خدمة Firebase وتدوير مفتاحه بخطة رجوع.
- قرار المالك الاحتياطي واستمرارية الملكية في Cloudflare وFirebase وGitHub، مع بقاء تأجيله الحالي موثقًا بقرار المالك.
- اختبار واجهة رفع الوسائط بحساب معلن تجريبي للقبول والرفض، بعد اكتمال الاختبارات المعزولة.
- حماية موزعة عبر Cloudflare WAF/Rate Limiting لمسارات الدخول والدعم والإحصاء والرفع.
...
### إصلاح بوابة GitHub بعد نشر App Check - 2026-09-25

تم إنجازه:

- تحديث بيئة Node في بوابتي الجودة وفحص ما بعد النشر من الإصدار 20 إلى الإصدار 24 المتوافق مع الحزم الحالية.
- تثبيت الحد الأدنى المدعوم للمشروع عند Node `22.12.0` داخل `package.json` حتى يظهر الخطأ مبكرًا في أي بيئة قديمة.
- تثبيت `esbuild@0.28.2` صراحةً كاعتماد تطوير لتلبية اعتماد Vite الاختياري ومنع اختلاف تفسير ملف القفل بين npm 10 وnpm 11.
- إعادة توليد `package-lock.json` ثم نجاح `npm ci` من بيئة نظيفة مع 0 ثغرات.

الأخطاء المكتشفة:

1. **فشل GitHub Actions عند تثبيت الاعتمادات بعد نشر 0.3.64**
   - الأعراض: توقفت بوابة الجودة في `npm ci` برسالة `Missing: esbuild@0.28.2 from lock file` وظهرت تحذيرات أن Wrangler وMiniflare وsanitize-html تتطلب Node أحدث.
   - السبب: سير العمل كان مثبتًا على Node 20/npm 10، بينما شجرة الاعتمادات الحالية تتطلب Node `22.12.0` أو أحدث، وكان اعتماد Vite الاختياري على esbuild غير مثبت مباشرةً.
   - الحل: نقل CI إلى Node 24، إعلان الحد الأدنى المدعوم، وتثبيت نسخة esbuild المتوافقة مباشرةً ثم التحقق بواسطة `npm ci`.
   - الحالة: محلول ومثبت محليًا وعلى GitHub؛ نجح كل من Quality gate وCodeQL للالتزام `0a31d32`.
...
- إصلاح App Check المنشور لم يتأثر؛ هذا التعديل يخص بيئة الفحص الآلي في GitHub فقط ولا يحتاج نشر Worker جديدًا.
- ملف الاعتمادات صار قابلاً للتثبيت من الصفر على Node 24 دون فقدان esbuild أو ظهور ثغرات npm.
- نجحت بوابة الجودة الكاملة: 225 اختبارًا، و9 اختبارات قواعد Firestore، وبناء 39 صفحة، و57 اختبار متصفح مع تجاوز واحد مقصود.
- نجح فحصا GitHub `Quality gate` و`CodeQL` بعد دفع الالتزام `0a31d32`.

المتبقي المحفوظ:

- البنود التشغيلية والأمنية المتبقية محفوظة في القسم السابق دون تغيير.

### تجهيز توثيق بريد مالك المنصة قبل تسجيل MFA - الإصدار 0.3.65

تم إنجازه:

- اكتشاف أن حساب `platform_owner` لا يحمل حالة بريد موثّق في Firebase، ولذلك رفضت طبقة MFA إنشاء سر TOTP كما هو مطلوب أمنيًا.
- إضافة زر يرسل رسالة توثيق البريد بواسطة Firebase Auth من جلسة المالك الحالية دون عرض عنوان البريد أو رابط التوثيق أو أي رمز داخل لوحة الإدارة.
- إضافة زر يعيد تحميل حالة المستخدم من Firebase بعد فتح رابط البريد، ولا يسمح بإنشاء QR حتى تصبح `emailVerified=true` فعليًا.
- إضافة معالجة واضحة لحد الطلبات `auth/too-many-requests` مع إبقاء الفشل مغلقًا دون إنشاء عامل ثانٍ جزئي.
- نجاح بوابة الجودة الكاملة: فحص الأسرار، 0 ثغرات، 225 اختبارًا، 9 اختبارات قواعد Firestore، بناء 39 صفحة، و57 اختبار متصفح مع تجاوز واحد مقصود.
- نشر الإصدار `0.3.65` إلى Worker `datetools` بمعرف `7bbc90d8-3084-46ad-9951-ae23a8948c34`، ونجاح فحص الصحة الإنتاجي من أول محاولة.
- إرسال رسالة توثيق البريد إنتاجيًا من صفحة أمان حساب `platform_owner` وظهور تأكيد الإرسال في الواجهة.

الأخطاء المكتشفة:

1. **لا يمكن تسجيل TOTP قبل توثيق بريد حساب المالك**
   - الأعراض: عرضت الصفحة رسالة `يجب توثيق بريد الحساب أولًا` بعد الضغط على بدء إعداد Authenticator.
   - السبب: حساب الإدارة أُنشئ سابقًا دون اكتمال علامة `emailVerified` في Firebase، والواجهة لم تكن توفر إرسال رسالة التوثيق.
   - الحل: إضافة تدفق إرسال الرسالة وإعادة التحقق من الحالة قبل إنشاء السر، بدل تجاوز إثبات ملكية البريد من Firebase Console.
   - الحالة: مسار الحل منشور والرسالة أُرسلت؛ ينتظر فتح رابط التوثيق من المالك.
...
```powershell
npm test -- --run tests/mfaEnforcement.test.js tests/firebaseMfa.test.js
npm run lint
npm run quality:local
npm run deploy
$env:EXPECTED_VERSION='0.3.65'; npm run health:production
```

الحالة:

- النسخة المرحلية منشورة وفرض MFA العام ما زال معطلًا، لذلك لا يوجد خطر قفل حساب المالك أثناء التسجيل.
- رسالة توثيق البريد أُرسلت إلى بريد الحساب المسجل دون عرض عنوانه أو أي بيانات سرية في السجل.

المتبقي المحفوظ:

- يفتح المالك رابط توثيق Firebase من بريده ثم يعود إلى `/admin/security` ويضغط `تحققت من البريد`.
- بعد نجاح التوثيق: إنشاء QR، مسحه في Authenticator، وإدخال الرمز داخل الصفحة بنفسه دون مشاركته في المحادثة.
- تسجيل الخروج واختبار دخول إنتاجي كامل بكلمة المرور ثم TOTP قبل تفعيل `mfaRequired` أو الفرض العام.

### إصلاح رابط إجراءات Firebase البريدية - الإصدار 0.3.66

تم إنجازه:

- تتبع رابط رسالة توثيق البريد التي وصلت إلى مجلد الرسائل غير المرغوب فيها، وتحديد أن عنوان الإجراء القديم كان يستخدم النطاق الخاطئ `deta-tool.com` ومسارًا غير موجود، ولذلك انتهى بخطأ `DNS_PROBE_FINISHED_NXDOMAIN`.
- إضافة معالج داخلي آمن لمسارات إجراءات Firebase في `/auth/action` يدعم توثيق البريد واستعادة كلمة المرور واسترجاع البريد، مع منع إعادة التوجيه إلى نطاق خارجي غير موثوق.
- إضافة اختبارات وحدات لمعالجة أوضاع Firebase ورمز الإجراء ورابط المتابعة الآمن.
- محاولة ضبط رابط الإجراء المخصص من Firebase Console؛ واجهة Firebase رفضت حفظ العنوان المخصص، لذلك أُلغي إعداد نطاق الإرسال غير المكتمل وأعيد القالب إلى معالج Firebase الرسمي العامل بدل إبقاء رابط معطوب.
- التحقق من أن قالب Firebase يعرض المعالج الرسمي `date-tool-official.firebaseapp.com/__/auth/action`، وأن `date-tool.com` موجود ضمن النطاقات المصرح بها، وأن رابط المتابعة يرجع إلى `/admin/security`.
- نشر الإصدار `0.3.66` إلى Worker `datetools` بمعرف `de132a03-f2e8-4a7b-8900-12644119e500`، والتحقق من استجابة `/auth/action` بالحالة `200`.
- نجاح بوابة الجودة الكاملة: فحص الأسرار، 0 ثغرات npm، 227 اختبارًا في 44 ملفًا، 9 اختبارات قواعد Firestore، بناء 40 صفحة، و57 اختبار متصفح مع تجاوز واحد مقصود.
- إرسال رسالة توثيق جديدة إنتاجيًا من صفحة `/admin/security` بتاريخ 2026-09-25، وظهور رسالة نجاح تؤكد الإرسال.

الأخطاء المكتشفة:

1. **رابط توثيق البريد يشير إلى نطاق خاطئ**
   - الأعراض: فتح الرسالة يؤدي إلى صفحة متصفح لا يمكن الوصول إليها مع `DNS_PROBE_FINISHED_NXDOMAIN`.
   - السبب: رابط الإجراء المحفوظ في قالب Firebase استخدم `deta-tool.com` بدل `date-tool.com`، وكان يشير كذلك إلى مسار قديم غير موجود.
   - الحل: إعادة قالب Firebase إلى المعالج الرسمي المستضاف من Firebase مع رابط متابعة آمن للموقع، وإبقاء المعالج الداخلي جاهزًا عند نجاح ضبط النطاق المخصص مستقبلًا.
   - الحالة: مصحح إنتاجيًا، ورسالة جديدة أُرسلت بعد التصحيح؛ ينتظر أن يفتحها المالك.

2. **رسالة Firebase وصلت إلى مجلد الرسائل غير المرغوب فيها**
   - الأعراض: صنف Gmail الرسالة القديمة على أنها غير مرغوب فيها.
   - السبب: الرسالة مرسلة من نطاق Firebase الافتراضي، كما احتوت النسخة القديمة رابطًا معطوبًا، وهما عاملان قد يضعفان الثقة بالرسالة.
   - الحل: استخدام رابط Firebase الرسمي العامل حاليًا، ووضع أحدث رسالة يدويًا كـ«ليست غير مرغوب فيها». تخصيص نطاق الإرسال يؤجل حتى يمكن ضبطه والتحقق منه دون تعطيل التدفق.
   - الحالة: الإرسال يعمل؛ تصنيف أحدث رسالة يحتاج تحقق المالك داخل بريده.

الملفات المتأثرة:

- `app/(arabic)/auth/action/page.jsx`
- `app/(arabic)/auth/action/ActionHandler.module.css`
- `app/firebaseEmailActions.js`
- `tests/firebaseEmailActions.test.js`
- `app/version.js`
- `package.json`
- `package-lock.json`
- `VERSION_LOG.md`
...
npm run quality:local
npm run deploy
$env:EXPECTED_VERSION='0.3.66'; npm run health:production
curl.exe -4 -I https://date-tool.com/auth/action
```

الحالة:

- معالج Firebase الرسمي هو المستخدم فعليًا في الرسائل الجديدة، ولا يعتمد نجاح التوثيق على النطاق المكتوب خطأ أو على صفحة مفقودة.
- فرض MFA العام ما زال معطلًا، ولا يوجد خطر قفل الحساب قبل إنهاء التسجيل والاختبار.

المتبقي المحفوظ:

- يفتح المالك أحدث رسالة توثيق، يختار «ليست رسالة غير مرغوب فيها» إن ظهرت داخل Spam، ثم يضغط الرابط دون نسخ رمز الإجراء إلى المحادثة.
- يعود المالك إلى `/admin/security` ويضغط `تحققت من البريد`، وبعدها يبدأ تسجيل Authenticator واختبار الدخول الإنتاجي قبل فرض MFA.

### إعدادات الحساب ونقل إدارة MFA - الإصدار المحلي 0.3.67

تم إنجازه:

- توثيق بريد حساب `platform_owner` بنجاح وتسجيل عامل TOTP في تطبيق Authenticator؛ وصلت رسالة Firebase التي تؤكد إضافة التحقق بخطوتين.
- إضافة صفحة `/admin/account` داخل هيكل الإدارة نفسه باسم «إعدادات الحساب»، وإتاحتها لكل حساب إداري نشط دون منح صلاحية لإدارة حسابات الآخرين.
- تثبيت رابط إعدادات الحساب في أسفل الشريط الجانبي بصورة مستقلة عن قائمة صفحات العمل القابلة للتمرير.
- عرض الاسم والبريد والدور والحالة وعدد الصلاحيات ومعرف الحساب وتاريخ الإنشاء وآخر دخول وآخر تحديث ومزود تسجيل الدخول.
- عرض حالة توثيق البريد وحالة Authenticator وما إذا كانت الجلسة الحالية قد دخلت بعامل ثانٍ، مع تنبيه واضح عند بقاء اختبار الدخول مطلوبًا.
- نقل لوحة TOTP من صفحة أمان المنصة إلى إعدادات الحساب، وتحديث روابط متابعة توثيق البريد لتعود إلى `/admin/account`.
- إضافة إيقاف عامل Authenticator بخطوة تأكيد صريحة ثم إمكان إعادة تفعيله من الصفحة نفسها، مع تحديث حالة الحماية فورًا وعدم إظهار أي سر أو رمز.
- إضافة دخول رسمي برابط بريد لمرة واحدة في `/admin_login` باستخدام Firebase Email Link، محمي بتحقق Turnstile، مع استمرار تحدي TOTP للحسابات المسجلة فيه وعدم استبداله بالعامل الثاني.
- إضافة إرسال رابط تغيير كلمة المرور بخطوة تأكيد ثانية، مع منع الإرسال من البيئة المحلية وعدم عرض كلمة المرور أو حفظها.
- إبقاء صفحة `/admin/security` مخصصة لحماية المنصة والتقارير، مع رابط مباشر إلى إعدادات الحساب.
- نجاح بوابة الجودة الكاملة: فحص الأسرار، 0 ثغرات npm، 229 اختبارًا في 45 ملفًا، 9 اختبارات قواعد Firestore، بناء 41 صفحة ومنها `/admin/account`، و57 اختبار متصفح مع تجاوز واحد مقصود.
- نجاح الفحص المرئي المحلي من جلسة موثقة: ظهرت بيانات الحساب وحالة Authenticator وزر إيقاف المصادقة الثنائية، وظهر خيار إرسال رابط الدخول البريدي في صفحة الدخول دون تنفيذ إيقاف أو إرسال فعلي أثناء الفحص.
- نشر الإصدار `0.3.67` عبر commit `3b93ddb`، ونجاح `Quality gate #21` و`CodeQL #34`، ثم نجاح فحص الصحة الإنتاجي ورؤية رقم الإصدار وخيار رابط البريد على `date-tool.com`.
- نجاح الاختبار الإنتاجي البشري بتاريخ 2026-09-25 لحساب `platform_owner`: الدخول بكلمة المرور طلب رمز TOTP ونجح، والدخول برابط البريد طلب رمز TOTP ونجح، ثم عرضت صفحة الحساب حالة «دخلت بالعامل الثاني». لم تُسجل كلمة المرور أو رابط الدخول أو رمز Authenticator في المذكرة.

الأخطاء المكتشفة:

1. **جلسة Firebase القديمة لا تكفي لإنشاء عامل TOTP بعد توثيق البريد**
   - الأعراض: ظهرت رسالة انتهاء صلاحية جلسة التحقق عند بدء الإعداد.
   - السبب: Firebase يتطلب تسجيل دخول حديثًا قبل تغيير حساس في عوامل المصادقة.
   - الحل: تسجيل خروج ودخول جديدين قبل إنشاء رمز QR، دون تجاوز إعادة التحقق أو نقل كلمة المرور.
   - الحالة: محلول؛ تم تسجيل العامل ووصل تأكيد Firebase.

2. **بيئة التطوير المحلية لا تحمل جلسة الإدارة الإنتاجية**
   - الأعراض: فتح `/admin/account` محليًا أعاد التوجيه إلى `/admin_login`.
   - السبب: جلسات Firebase محفوظة لكل Origin على حدة، و`127.0.0.1` منفصل عن `date-tool.com`.
   - الحل: إبقاء العزل كما هو وعدم إضافة تجاوز مصادقة للتطوير؛ سجّل المستخدم الدخول محليًا صراحةً وأُنجز الفحص المرئي دون إضافة تجاوز أمني.
   - الحالة: سلوك أمني صحيح، والفحص المرئي المحلي مكتمل.

3. **Firebase لا يوفر رمز OTP رقميًا عبر البريد لتسجيل دخول الويب**
   - الأعراض: المطلوب تجربة طلب رمز بريدي للدخول، بينما واجهة Firebase الرسمية تعرض رابط دخول بريدي لمرة واحدة بدل رمز رقمي مستقل.
   - السبب: موفر Email Link هو مسار Firebase الرسمي لتسجيل الدخول دون كلمة مرور عبر البريد؛ إنشاء رمز رقمي مخصص يحتاج خدمة إرسال وخادم تحقق ومخزن محاولات وانتهاء صلاحية إضافيًا.
   - الحل: اعتماد رابط Firebase لمرة واحدة، محمي بـTurnstile، مع إبقاء TOTP عاملًا ثانيًا للحسابات الحساسة وعدم بناء نظام رموز مخصص غير ضروري.
   - الحالة: محلول ومختبر إنتاجيًا؛ وصل رابط الدخول وأكمل الحساب تحدي TOTP بنجاح.
...
- `app/components/admin/TotpMfaPanel.module.css`
- `app/(arabic)/auth/action/page.jsx`
- `tests/adminAccountSettings.test.js`
- `tests/firebaseMfa.test.js`
- `tests/mfaEnforcement.test.js`
- `app/version.js`
- `package.json`
...
```powershell
npm test -- --run tests/adminAccountSettings.test.js tests/firebaseMfa.test.js tests/adminNoReload.test.js tests/securityBoundaries.test.js
npm run lint
npm run build
npm run dev
npm run quality:local
npm test -- --run tests/mfaEnforcement.test.js tests/firebaseMfa.test.js tests/adminAccountSettings.test.js tests/firebaseAuthRetry.test.js
npm run health:production
git diff --check
```
...
- الإصدار `0.3.67` منشور ومراجع محليًا وإنتاجيًا، وفحص الصحة ناجح.
- عامل TOTP مضاف إلى حساب المالك واختُبر إنتاجيًا بنجاح عبر مساري كلمة المرور ورابط البريد، بينما فرض MFA العام ما زال معطلًا إلى أن تُنفذ خطوة الفرض التدريجي مستقلة.
- تحقق الفحص المرئي بتاريخ 2026-09-25 من أن Email/Password وEmail Link مفعّلان في Firebase Console دون إجراء أي تعديل أو حفظ جديد.

المتبقي المحفوظ:

- فرض MFA تدريجيًا على حساب `platform_owner` بعد مراجعة خطة الرجوع: ضبط `mfaRequired=true` للحساب، تفعيل علم الإصدار العام، نشر قواعد Firestore، ثم إعادة اختبار الدخول والمسارات الحساسة.

### تبسيط تحذير إيقاف MFA وفصل إصدار الإدارة - admin 0.1.56

تم إنجازه:

- تعديل تحذير الإيقاف إلى: «سيعود الحساب إلى عامل دخول واحد حتى تعيد التفعيل. قد يسجل خروجك لإتمام التغيير بأمان.» دون تغيير منطق إلغاء تسجيل TOTP.
- إضافة اختبار يثبت النص الجديد ويمنع عودة عبارة «قد يسجل Firebase خروجك» داخل التحذير.
- رفع إصدار الإدارة فقط إلى `0.1.56` مع إبقاء إصدار المنصة العامة ونسخة الحزمة عند `0.3.67`.
- نجاح الاختبار المخصص وESLint وبناء 41 صفحة، ثم نجاح `Quality gate #23` و`CodeQL #36` للـcommit التصحيحي `13183b6`.
- التحقق إنتاجيًا من ظهور `admin v0.1.56` والنص المطلوب، ومن بقاء `/api/app-version` على `0.3.67`. فُتحت مرحلة التأكيد للقراءة فقط ثم أُلغيت دون إيقاف العامل الثاني.

الأخطاء المكتشفة:

1. **رفع إصدار المنصة العامة بسبب تعديل يخص الإدارة فقط**
   - الأعراض: أصبح `APP_VERSION` و`package.json` يقرآن `0.3.68` رغم أن التغيير نص داخل صفحة إعدادات الإدارة.
   - السبب: عومل تعديل الإدارة كإصدار عام بدل استخدام رقم الإدارة المستقل.
   - الحل: إعادة `APP_VERSION` ونسخة الحزمة إلى `0.3.67`، والإبقاء على `ADMIN_VERSION=0.1.56`، وتثبيت قاعدة عدم رفع العامة لتغيير إداري صرف.
   - الحالة: محلول ومتحقق منه إنتاجيًا.
...
```powershell
npm test -- --run tests/adminAccountSettings.test.js
npm run lint
npm run build
npm test -- --run tests/adminAccountSettings.test.js tests/pwaVersionCheck.test.js tests/healthRoute.test.js
git diff --check
```

الحالة:

- التعديل منشور؛ العامة `0.3.67` والإدارة `0.1.56`.
- عامل TOTP ما زال مفعّلًا ولم تُنفذ عملية الإيقاف أثناء التحقق.

المتبقي المحفوظ:

- لا يوجد متبقٍ لهذا التعديل النصي؛ يبقى فرض MFA التدريجي بندًا أمنيًا مستقلًا كما هو موثق أعلاه.
